...
import {
  createJokerComApiLoginDTO,
  JokerComApiLoginDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiLoginDTO';
import {
  createJokerComApiDomainListDTO,
  JokerComApiDomainListDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiDomainListDTO';
import {
  createJokerComApiWhoisDTO,
  JokerComApiWhoisDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiWhoisDTO';
import {
  createJokerComApiRegisterDTO,
  JokerComApiRegisterDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiRegisterDTO';
import {
  createJokerComApiWhoisContactDTO,
  JokerComApiWhoisContactDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiWhoisContactDTO';
import {
  explainJokerComApiDomainPriceType,
  JokerComApiDomainPriceType,
  parseJokerComApiDomainPriceType,
} from '../../../../core/com/joker/dmapi/types/JokerComApiDomainPriceType';
import {JokerComApiPeriodType} from '../../../../core/com/joker/dmapi/types/JokerComApiPeriodType';
import {
  createJokerComApiDomainCheckDTO,
  JokerComApiDomainCheckDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiDomainCheckDTO';
import {parseJokerComApiDomainStatus} from '../../../../core/com/joker/dmapi/types/JokerComApiDomainStatus';
import {
  createJokerComApiPriceListDTO,
  JokerComApiPriceListDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiPriceListDTO';
import {parseJokerComApiPriceListItemListFromString} from '../../../../core/com/joker/dmapi/types/JokerComApiPriceListItem';
import {
  createJokerComApiDomainPrice,
  JokerComApiDomainPrice,
} from '../../../../core/com/joker/dmapi/types/JokerComApiDomainPrice';
import {
  explainJokerComApiPriceAmount,
  parseJokerComApiPriceAmount,
} from '../../../../core/com/joker/dmapi/types/JokerComApiPriceAmount';
import {
  explainJokerComApiCurrency,
  parseJokerComApiCurrency,
} from '../../../../core/com/joker/dmapi/types/JokerComApiCurrency';
import {
  explainJokerComApiDomainPeriod,
  parseJokerComApiDomainPeriod,
} from '../../../../core/com/joker/dmapi/types/JokerComApiDomainPeriod';
import {FiCsComJokerDomainManagementAPI} from '../../../../core/com/joker/dmapi/FiCsComJokerDomainManagementAPI';
import {
  createJokerComApiProfileDTO,
  JokerComApiProfileDTO,
} from '../../../../core/com/joker/dmapi/types/JokerComApiProfileDTO';
import {
  explainJokerComApiUserAccess,
  parseJokerComApiUserAccess,
} from '../../../../core/com/joker/dmapi/types/JokerComApiUserAccess';
import {isString} from '../../../../core/types/String';
import {parseInteger} from '../../../../core/types/Number';
import {keys} from '../../../../core/functions/keys';

const LOG = LogService.createLogger('FiCsComJokerDomainManagementService');
 ...
 */
export class FiCsComJokerDomainManagementService implements FiCsComJokerDomainManagementAPI {
  ...
  private async _login(
    ...
    if (apiKey && (username || password)) {
      throw new Error(
        'FiCsComJokerDomainManagementService._login: Use "api-key" or "username" and "password"; not both',
      );
    }
    if (!apiKey && !(username && password)) {
      throw new Error(
        'FiCsComJokerDomainManagementService._login: "username" or "password" missing',
      );
    }
    const args = {
    ...
    if (!userAccess) {
      throw new TypeError(
        `FiCsComJokerDomainManagementService._login: Could not parse user-access "${userAccessString}": ${explainJokerComApiUserAccess(userAccessString)}`,
      );
    }
    ...
    if (!accountCurrency) {
      throw new TypeError(
        `FiCsComJokerDomainManagementService._login: Could not parse account-currency "${accountCurrencyString}": ${explainJokerComApiCurrency(accountCurrencyString)}`,
      );
    }

    const accountBalance =
      parseJokerComApiPriceAmount(headers['account-balance']) ?? 0;
    const accountPendingAmount =
      parseJokerComApiPriceAmount(headers['account-pending_amount']) ?? 0;
    const accountRebate = parseFloat(headers['account-rebate']);
    const accountContractDate = headers['account-contract_date'];
    const statsNumberOfDomains =
      parseInteger(headers['stats-number-of-domains']) ?? 0;
    ...
    const statsNumberOfAutoRenew =
      parseInteger(headers['stats-number-of-autorenew']) ?? 0;
  ...
  public async logout() {
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.logout: No authSID. Try login first.',
      );
    await jokerPostRequest(this._url, 'logout', {
  ...
  public async queryDomainList(
    ...
    if (!this._authSID) {
      throw new Error(
        'FiCsComJokerDomainManagementService.queryDomainList: No authSID. Try login first.',
      );
    }
    const opts = {
      'auth-sid': this._authSID,
      ...
      ...(showJokerNS !== undefined
        ? {showjokerns: showJokerNS ? '1' : '0'}
        : {}),
    };
   ...
   * @param host
   */
  public async queryWhoisByHost(host: string): Promise<JokerComApiWhoisDTO> {
  ...
  private async _queryWhois(
    ...
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.queryWhois: No auth_id. Try login first.',
      );
    if (domain === undefined && contact === undefined && host === undefined) {
      throw new TypeError(
        'FiCsComJokerDomainManagementService.queryWhois: Exactly one of accepted options must be specified.',
      );
    }
    const opts = {
      'auth-sid': this._authSID,
    ...
    return createJokerComApiWhoisDTO(response.headers, body);
  }
  ...
  public async queryProfile(): Promise<JokerComApiProfileDTO> {
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.queryProfile: No auth_id. Try login first.',
      );
    const response = await jokerPostRequest(this._url, 'query-profile', {
      'auth-sid': this._authSID,
    });
    const headers = response.headers;
    const body = parseJokerStringObjectResponse(response.body);
  ...
  public async queryPriceList(): Promise<JokerComApiPriceListDTO> {
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.queryPriceList: No auth_id. Try login first.',
      );
    ...
    return createJokerComApiPriceListDTO(headers, body);
  }
  ...
  public async domainRenew(
    ...
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.domainRenew: No auth_id. Try login first.',
      );
    if (!domain)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRenew: Option "domain" is required.',
      );
    if (period === undefined && !expyear) {
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRenew: One of "period" or "expyear" is required.',
      );
    }
    if (period !== undefined && expyear) {
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRenew: Only one of "period" or "expyear" may be used, but not both.',
      );
    }
    if (maxPrice <= 0) {
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRenew: "max-price" must be above 0',
      );
    }
  ...
  public async domainCheck(
    ...
    if (periods !== undefined) {
      if (periodType === JokerComApiPeriodType.YEARS) {
        if (periods > 10 || periods <= 0) {
          throw new TypeError(
            'FiCsComJokerDomainManagementService.domainRenew: Option "periods" must be 1-10 when specified as years.',
          );
        }
      } else {
        if (periods < 12) {
          throw new TypeError(
            'FiCsComJokerDomainManagementService.domainRenew: Option "periods" must be over 12 when specified as months.',
          );
        }
      }
    }
    ...
    const response = await jokerPostRequest(this._url, 'domain-check', opts, [
      '0',
      '1000',
    ]);
    ...
    if (!domainStatus)
      throw new TypeError('domainCheck: Could not parse domain status');

    const prices: readonly JokerComApiDomainPrice[] | undefined =
      parseJokerComDomainPrices(body);
  ...
  public async domainRegister(
    ...
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.domainRegister: No auth_id. Try login first.',
      );
    if (!domain)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "domain" is required.',
      );
    if (!period)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "period" is required.',
      );
    if (period < 1)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "period" must be at least 1.',
      );
    if (!ownerContact)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "ownerContact" is required.',
      );
    if (!billingContact)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "billingContact" is required.',
      );
    if (!adminContact)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "adminContact" is required.',
      );
    if (!techContact)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "techContact" is required.',
      );
    if (!nsList)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: Option "nsList" is required.',
      );
    if ((nsList?.length ?? 0) < 2)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainModify: Option "nsList" must have at least 2 nameservers.',
      );
    if (maxPrice !== undefined && maxPrice <= 0) {
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainRegister: "max-price" must be above 0',
      );
    }
    const opts: JokerRequestArgumentObject = {
      'auth-sid': this._authSID,
    ...
    const headers = response.headers;
    return createJokerComApiRegisterDTO(headers['tracking-id'], headers);
  }

  /** grants-list
   * @see https://joker.com/faq/content/76/448/en/grants_list.html
   */
  public async grantsList(domain: string, showKey: string): Promise<string> {
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.grantsList: No auth_id. Try login first.',
      );
  ...
  public async grantsInvite(
    ...
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.grantsInvite: No auth_id. Try login first.',
      );
    if (!domain)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.grantsInvite: Option "domain" is required.',
      );
    if (!email)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.grantsInvite: Option "email" is required.',
      );
    if (!role)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.grantsInvite: Option "role" is required.',
      );
    const opts = {
      'auth-sid': this._authSID,
      domain,
  ...
  public async domainModify(
    ...
    if (!this._authSID)
      throw new Error(
        'FiCsComJokerDomainManagementService.domainModify: No auth_id. Try login first.',
      );
    if (!domain)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainModify: Option "domain" is required.',
      );
    if (dnssec === true && !ds)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainModify: Option "ds" is required when "dnssec" enabled.',
      );
    if (dnssec === true && (ds?.length ?? 0) < 2)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainModify: Option "ds" must have at least 2 items.',
      );
    if (nsList !== undefined && (nsList?.length ?? 0) < 2)
      throw new TypeError(
        'FiCsComJokerDomainManagementService.domainModify: Option "nsList" must have at least 2 nameservers.',
      );
    const opts = {
      ...
      ...(dnssec !== undefined && ds !== undefined && ds.length >= 1
        ? {'ds-1': ds[0]}
        : {}),
      ...(dnssec !== undefined && ds !== undefined && ds.length >= 2
        ? {'ds-2': ds[1]}
        : {}),
      ...(dnssec !== undefined && ds !== undefined && ds.length >= 3
        ? {'ds-3': ds[2]}
        : {}),
      ...(dnssec !== undefined && ds !== undefined && ds.length >= 4
        ? {'ds-4': ds[3]}
        : {}),
      ...(dnssec !== undefined && ds !== undefined && ds.length >= 5
        ? {'ds-5': ds[4]}
        : {}),
      ...(dnssec !== undefined && ds !== undefined && ds.length >= 6
        ? {'ds-6': ds[5]}
        : {}),
    };
...
async function jokerPostRequest(
  ...
  const responseString = await HttpService.postText(url, body, {
...
async function jokerGetRequest(
  ...
  const responseString = await HttpService.getText(url, {
    'Content-Type': ContentType.X_WWW_FORM_URLENCODED,
  });
...
function parseResponseObject(
  ...
  if (headersString === undefined) {
    throw new TypeError(
      `parseResponseBody: Could not parse headers from: "${data}"`,
    );
  }
  ...
  if (error !== undefined) {
    throw new Error(
      `FiCsComJokerDomainManagementService: Request failed: ${statusCode}: "${statusText}": ${error}`,
    );
  }

  if (warning !== undefined) {
    LOG.warn(
      `FiCsComJokerDomainManagementService: DMAPI Warning: "${warning}"`,
    );
  }

  if (statusCode !== undefined && !acceptedStatusCodes.includes(statusCode)) {
    throw new Error(
      `FiCsComJokerDomainManagementService: Request failed: ${statusCode}: "${statusText}"`,
    );
  }
  ...
  return reduce(
    ...
    (obj: JokerStringObject, line: string): JokerStringObject => {
      if (line.trim() && line.indexOf(': ') < 0) {
        throw new TypeError(
          `parseResponseHeaders: Could not parse line: "${line}"`,
        );
      }
      ...
      if (!name)
        throw new TypeError(
          `parseResponseHeaders: Could not parse name and value: "${line}"`,
        );
...
function parseJokerNS(line: string): boolean {
  if (!(line === '1' || line === '0')) {
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parseJokerNS: Could not parse "jokerns": "${line}"`,
    );
  }
  return line === '1';
}
...
function parseDomainLine(
  ...
  if (!domain)
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parse_domain: Could not parse domain name: "${line}"`,
    );

  const exp_date = tmp.shift();
  if (!exp_date)
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parse_domain: Could not parse domain exp_date: "${line}"`,
    );

  const statusString = tmp.shift();
  if (statusString === undefined)
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parse_domain: Could not parse statusString from "${line}"`,
    );

  const status = split(statusString, ',');
  if (!isString(status))
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parse_domain: Could not parse status: "${line}"`,
    );

  const jokerNsString = tmp.pop();
  if (jokerNsString === undefined) {
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parse_domain: Could not parse jokerNs string: "${line}"`,
    );
  }
  const jokerNS = parseJokerNS(jokerNsString);
  if (!isBoolean(jokerNS))
    throw new TypeError(
      `FiCsComJokerDomainManagementService.parse_domain: Could not parse jokerNs: "${line}"`,
    );
...
function parseJokerStringObjectResponse(body: string): JokerStringObject {
  ...
  return reduce(
    ...
    (data: JokerStringObject, line: string): JokerStringObject => {
      ...
      if (!key)
        throw new TypeError(
          `FiCsComJokerDomainManagementService.parseJokerStringObjectResponse: Could not parse key from line "${line}"`,
        );
      const value = parts.join(': ');
      return {
        ...data,
...
function parseJokerComDomainPrices(
  body: JokerStringObject,
): readonly JokerComApiDomainPrice[] | undefined {
  ...
  let results: readonly JokerComApiDomainPrice[] = reduce(
    keys(body),
    (
      list: readonly JokerComApiDomainPrice[],
      key: string,
    ): readonly JokerComApiDomainPrice[] => {
      if (key === 'domain-price-promo') {
        ...
        if (!startString)
          throw new TypeError(
            `FiCsComJokerDomainManagementService: parseJokerComDomainPrices: could not parse start promo date from: "${valueString}"`,
          );
        const endString = parts.shift();
        if (!endString)
          throw new TypeError(
            `FiCsComJokerDomainManagementService: parseJokerComDomainPrices: could not parse end promo date from: "${valueString}"`,
          );
        isPromo = true;
      ...
      if (startsWith(key, 'domain-price-')) {
        ...
        const type: JokerComApiDomainPriceType | undefined =
          parseJokerComApiDomainPriceType(typeString);
        if (!type)
          throw new TypeError(
            `FiCsComJokerDomainManagementService: parseJokerComDomainPrices: could not parse price type "${typeString}": ${explainJokerComApiDomainPriceType(typeString)}`,
          );
        ...
        const amount =
          amountString !== undefined
            ? parseJokerComApiPriceAmount(amountString)
            : undefined;
        if (!amount)
          throw new TypeError(
            `FiCsComJokerDomainManagementService: parseJokerComDomainPrices: could not parse price amount "${amountString}": ${explainJokerComApiPriceAmount(amountString)}`,
          );

        const currencyString = parts.shift();
        const currency =
          currencyString !== undefined
            ? parseJokerComApiCurrency(currencyString)
            : undefined;
        if (!currency)
          throw new TypeError(
            `FiCsComJokerDomainManagementService: parseJokerComDomainPrices: could not parse price currency "${currencyString}": ${explainJokerComApiCurrency(currencyString)}`,
          );

        const periodString = parts.shift();
        const period =
          periodString !== undefined
            ? parseJokerComApiDomainPeriod(periodString)
            : undefined;
        if (!period)
          throw new TypeError(
            `FiCsComJokerDomainManagementService: parseJokerComDomainPrices: could not parse price period "${periodString}": ${explainJokerComApiDomainPeriod(periodString)}`,
          );
