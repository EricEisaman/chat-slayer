...
import {isSseKeepalive} from '../SseKeepaliveResponse';
...
> {
  ...
  public static create(
    ...
    if (!has(this._protocolFactories, protocol)) {
      throw new TypeError(
        `RequestServer: Protocol "${protocol}" not yet supported`,
      );
    }
  ...
  public setHandler(
    newHandler: RequestHandler<IncomingMessage, ServerResponse> | undefined,
  ): void {
    LOG.debug(
      `Setting handler at ${this._getConnectionString()} as "${newHandler}", was "${this._handler}"`,
    );
  ...
  private async _callRequestHandler(
    req: IncomingMessage,
    res: ServerResponse,
  ): Promise<void> {
    if (this._handler !== undefined) {
      try {
        ...
        LOG.error(
          `"${req.method} ${req.url}": Response handler had an error: `,
          e,
        );
      }
      if (!res.writableEnded && !isSseKeepalive(res)) {
        LOG.warn(
          `"${req.method} ${req.url}": Warning! Request handler did not close the response.`,
        );
        res.end();
      }
    } else {
  ...
  private _onRequest(req: IncomingMessage, res: ServerResponse): void {
    this._callRequestHandler(req, res).catch((err: unknown) => {
