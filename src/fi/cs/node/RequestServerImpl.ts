...
} from '../core/request/types/RequestStatus';
import {
  RequestError,
  createRequestError,
  isRequestError,
...
import {applyCorsHeaders, getCorsAllowOrigin} from './CorsResponseContext';

const LOG = LogService.createLogger('RequestServerImpl');

export type RequestPreHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<'continue' | 'handled'>;

let requestPreHandler: RequestPreHandler | undefined;

export function setRequestPreHandler(
  handler: RequestPreHandler | undefined,
): void {
  requestPreHandler = handler;
}
...
export class RequestServerImpl implements RequestServer, Disposable {
  ...
  public static create(
    ...
    return new RequestServerImpl(server, router);
  }
   ...
   */
  public attachController(controller: any): void {
    if (isRequestController(controller)) {
      this._router.attachController(controller);
    } else {
      throw new TypeError(
        'RequestServer: The provided controller was not RequestController',
      );
    }
  ...
  private async _handleRequest(
    ...
    try {
      if (requestPreHandler) {
        const preResult = await requestPreHandler(req, res);
        if (preResult === 'handled') {
          return;
        }
      }
      const method = parseRequestMethod(reqMethod);
      const responseData: ResponseEntity<any> =
        await this._router.handleRequest(
          method,
          reqUrl,
          (headers: Headers) =>
            RequestServerImpl._requestBodyParser(req, headers),
  ...
  private static async _requestBodyParser(
    ...
    const contentType: string =
      headers.getFirst('content-type')?.toLowerCase() ?? 'application/json';
    switch (contentType) {
  ...
  private _writeResponseHeaders(
    ...
    if (!headers.isEmpty()) {
      headers.keySet().forEach((headerKey: string) => {
        ...
        LOG.debug(
          `_writeResponseHeaders: Setting response header as "${headerKey}": "${headerValue}"`,
        );
    ...
    const corsOrigin = getCorsAllowOrigin(res);
    if (corsOrigin) {
