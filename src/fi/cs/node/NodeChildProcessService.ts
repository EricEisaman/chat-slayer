...
import {
  ...
  CommandOptions,
  CommandResponse,
} from '../core/ChildProcessService';
import {
  ChildProcessError,
  isChildProcessError,
} from '../core/types/ChildProcessError';
...
export class NodeChildProcessService implements ChildProcessService {
  ...
  public destroy(): void {
    if (!this._destroyed) {
      ...
      LOG.debug(
        `destroying ${this._children?.length} children: ${map(this._children, item => item?.child?.pid).join(', ')}`,
      );
      this._shutdownChildProcesses().catch((err: unknown) => {
  ...
  public async countChildProcesses(): Promise<number> {
    LOG.debug(
      `countChildProcesses: ${this._children?.length}: ${map(this._children, item => item?.child?.pid).join(', ')}`,
    );
  ...
  public async waitAllChildProcessesStopped(): Promise<void> {
    if (this._children?.length) {
      LOG.debug(
        `start: waitAllChildProcessesStopped: ${map(this._children, item => item?.child?.pid).join(', ')}`,
      );
      ...
      const promises: Promise<void>[] = map(
        ...
        async (item: StoredChild): Promise<void> => {
          try {
            ...
            if (
              isChildProcessError(err) &&
              err.signal === (item.killSignal ?? 'SIGTERM')
            ) {
              const {
                name,
                pid,
              ...
              LOG.debug(
                `Child #${pid} (name=${name}, running=${running}, initializing=${initializing}, abort=${abort}, streamsOpen=${streamsOpen}, exitCode=${exitCode}, exitSignal=${exitSignal}) successfully shutdown with signal ${err.signal}`,
              );
            } else {
              const {
                name,
              ...
              LOG.debug(
                `Child #${pid} (name=${name}, running=${running}, initializing=${initializing}, abort=${abort}, streamsOpen=${streamsOpen}, exitCode=${exitCode}, exitSignal=${exitSignal}) failed to shutdown: `,
                err,
              );
            }
          }
      ...
      LOG.debug(
        `Waiting for ${promises.length} children to shutdown: ${map(this._children, item => item?.child?.pid).join(', ')}`,
      );
      // Wait for the children to shut down
      await Promise.allSettled(promises);
      LOG.debug(
        `end: waitAllChildProcessesStopped: ${map(this._children, item => item?.child?.pid).join(', ')}`,
      );

      const count = this._children?.length ?? 0;
      if (count) {
        LOG.warn(
          `Warning! ${count} children detected at the end of waitAllChildProcessesStopped(): ${map(this._children, item => item?.child?.pid).join(', ')}`,
        );
      }
    } else {
  ...
  public async shutdownChildProcesses(): Promise<void> {
    if (this._destroyed)
      throw new TypeError('shutdownChildProcesses: Service already destroyed');
  ...
  public async executeCommand(
    ...
    LOG.debug(
      ...
      args,
      opts,
      `Running: ${map(this._children, item => item?.child?.pid).join(', ')}`,
    );
  ...
  private async _execFile(
    ...
    if (this._destroyed)
      throw new TypeError('_execFile: Service already destroyed');
    ...
    const nodeOpts: SpawnOptions = {
      ...
      ...(serialization !== undefined
        ? staticSpawnOptionsTypeGuard({
            serialization: serialization as SerializationType,
          })
        : {}),
      ...(detached !== undefined
        ? staticSpawnOptionsTypeGuard({detached})
        : {}),
      ...
      ...(stdio !== undefined
        ? staticSpawnOptionsTypeGuard({stdio: stdio as StdioOptions})
        : {}),
      ...(killSignal !== undefined
        ? staticSpawnOptionsTypeGuard({
            killSignal: killSignal as NodeJS.Signals | number,
          })
        : {}),
    };
        ...
        try {
          ...
          child.on('error', (error: unknown) => {
            if (storedItem.running) {
              ...
              if (error) {
                ...
                reject(
                  new TypeError(
                    'Event "error" without error information detected',
                  ),
                );
              }
            } else {
          ...
          if (child.stdout) {
            child.stdout.on('data', (chunk: Buffer) => {
              if (this._destroyed) {
                LOG.debug(
                  "Event 'data': Service already destroyed; stdout data ignored.",
                );
                return;
              }
              ...
              const pid = child?.pid;
              LOG.debug(
                `stdout data on: child #${pid}, running=${running}, streamsOpen=${streamsOpen}, initializing=${initializing}`,
              );
          ...
          if (child.stderr) {
            child.stderr.on('data', (chunk: Buffer) => {
              if (this._destroyed) {
                LOG.debug(
                  "Event 'data' on stderr: Service already destroyed; stderr data ignored.",
                );
                return;
              }
              const {child, running, streamsOpen, initializing} = storedItem;
              const pid = child?.pid;
              LOG.debug(
                `stderr data on: child #${pid}, running=${running}, streamsOpen=${streamsOpen}, initializing=${initializing}`,
              );
          ...
          child.on('close', () => {
            ...
            if (this._destroyed) {
              ...
              const pid = child?.pid;
              LOG.debug(
                `close on destroyed service item: child #${pid}, running=${running}, streamsOpen=${streamsOpen}, initializing=${initializing}`,
              );
            } else {
              const {child, running, streamsOpen, initializing} = storedItem;
              const pid = child?.pid;
              LOG.debug(
                `close on item: child #${pid}, running=${running}, streamsOpen=${streamsOpen}, initializing=${initializing}`,
              );
            }
            this._onStoredChildClose(
              ...
              args ?? [],
              storedItem,
              parseInteger(storedItem.exitCode) ??
                parseInteger(child.exitCode) ??
                undefined,
          ...
          LOG.warn(
            `Exception handled from command "${name}${args?.length ? ' ' : ''}${(args ?? []).join(' ')}": `,
            err,
          );
  ...
  public sendShutdownToChildProcesses(): void {
    if (this._destroyed)
      throw new TypeError(
        'sendShutdownToChildProcesses: Service already destroyed',
      );
  ...
  private _sendShutdownToChildProcesses(): void {
    const children = this._children;
    LOG.debug(
      `Sending shutdown to ${children.length} children: ${map(this._children, item => item?.child?.pid).join(', ')}`,
    );
    forEach(children, (item: StoredChild): void => {
      try {
        const {child, initializing, abort} = item;
        LOG.debug(
          `#${child?.pid}: initializing=${initializing}, abort=${abort}`,
        );
        if (initializing) {
          if (!abort) {
  ...
  private _stopChild(item: StoredChild) {
    ...
    if (child && running) {
      ...
      if (!status) {
        LOG.warn(
          `Warning! Could not signal child process ${child?.pid} to stop`,
        );
      }
    } else {
      if (!child) LOG.warn('Warning! The child was not yet created');
      else if (!running)
        LOG.warn(`Warning! The child #${child?.pid} was not running`);
    }
  }
  ...
  private async _onStoredChildClose(
    ...
    const pid = child?.pid;
    LOG.debug(
      `_onStoredChildClose on child #${pid}, running=${running}, streamsOpen=${streamsOpen}, initializing=${initializing}`,
    );
    ...
    try {
      return await this._onChildProcessClose(
        name,
        args,
        exitCode,
        signalCode,
      ...
      LOG.warn(
        `Unexpected exception handled: "${name} ${(args ?? []).join(' ')}": `,
        err,
      );
  ...
  private async _onChildProcessClose(
    ...
    if (signalCode !== undefined) {
      LOG.debug(
        `Command failed: "${name}${args?.length ? ' ' : ''}${(args ?? []).join(' ')}": Signal received: ${signalCode}: `,
        stdout,
        stderr,
      );
    ...
    if (exitCode !== undefined && exitCode !== 0) {
      ...
      if (status >= 0) {
        LOG.debug(
          `Command failed: "${name}${args?.length ? ' ' : ''}${(args ?? []).join(' ')}": Exit code: ${status}: `,
          stdout,
          stderr,
        );
        throw new ChildProcessError(
      ...
      LOG.debug(
        `Command failed: "${name}${args?.length ? ' ' : ''}${(args ?? []).join(' ')}": `,
        stdout,
        stderr,
      );
    ...
    LOG.debug(
      `Command succeed: "${name}${args?.length ? ' ' : ''}${(args ?? []).join(' ')}": `,
      stdout,
      stderr,
    );
    return {
...
function staticSpawnOptionsTypeGuard(value: SpawnOptions): SpawnOptions {
  return value;
}
