...
import {
  RequestMethod,
  stringifyRequestMethod,
} from '../../../core/request/types/RequestMethod';
...
import {
  isRequestError,
  RequestError,
} from '../../../core/request/types/RequestError';
...
import {
  EntityStatusTypes,
  ResponseEntity,
} from '../../../core/request/types/ResponseEntity';
import {Headers} from '../../../core/request/types/Headers';

export const FsPromises = REQUEST_CLIENT_NODE_ENABLED
  ? require('fs').promises
  : undefined;
...
export class NodeRequestClient implements RequestClientAdapter {
  ...
  public static create(
    ...
    return new NodeRequestClient(http, https, defaultOptions);
  }
  ...
  public async bufferRequest(
    ...
    switch (method) {
      case RequestMethod.GET:
        return await this._bufferRequest(RequestMethod.GET, url, headers, data);
      case RequestMethod.POST:
        return await this._bufferRequest(
          RequestMethod.POST,
          url,
          headers,
          data,
      ...
      case RequestMethod.PUT:
        return await this._bufferRequest(RequestMethod.PUT, url, headers, data);
      ...
      default:
        throw new TypeError(`NodeRequestClient: Unsupported method: ${method}`);
    }
  }
  ...
  public async textRequest(
    ...
    switch (method) {
      case RequestMethod.GET:
        return await this._textRequest(
          RequestMethod.GET,
          url,
          headers,
          data,
        ).then(NodeRequestClient._successTextResponse);
      case RequestMethod.POST:
        return await this._textRequest(
          RequestMethod.POST,
          url,
  ...
  public async jsonRequest(
    ...
    switch (method) {
      case RequestMethod.GET:
        return await this._jsonRequest(
          RequestMethod.GET,
          url,
          headers,
          data,
        ).then(NodeRequestClient._successJsonResponse);
      case RequestMethod.POST:
        return await this._jsonRequest(
          RequestMethod.POST,
          url,
    ...
    switch (method) {
      case RequestMethod.GET:
        return await this._textRequest(
          ...
          headers,
          data,
        ).then(NodeRequestClient._successTextEntityResponse);
      case RequestMethod.POST:
        return await this._textRequest(
          RequestMethod.POST,
          url,
    ...
    switch (method) {
      case RequestMethod.GET:
        return await this._jsonRequest(
          ...
          headers,
          data,
        ).then(NodeRequestClient._successJsonEntityResponse);
      case RequestMethod.POST:
        return await this._jsonRequest(
          RequestMethod.POST,
          url,
  ...
  private async _textRequest(
    ...
    const result: string | undefined =
      await NodeHttpUtils.getRequestDataAsString(response);
  ...
  private async _jsonRequest(
    ...
    const result: JsonAny | undefined =
      await NodeHttpUtils.getRequestDataAsJson(response);
    const statusCode = response?.statusCode ?? 0;
    return {
      method,
      url,
  ...
  private async _httpRequest(
    ...
    if (protocol === 'unix:' || protocol === 'socket:') {
      const fullSocketPath: string = urlParsed?.pathname
        ? urlParsed?.pathname
        : '/';
      if (fullSocketPath === '/') {
        throw new TypeError(
          `No socket path found for unix protocol URL: ${url}`,
        );
      }
      const realSocketPath: string | undefined =
        await this._findSocketFile(fullSocketPath);
      if (!realSocketPath) {
        throw new TypeError(
          `No socket path found for unix protocol URL: ${url}`,
        );
      }
      const socketSuffix: string =
        realSocketPath.length < fullSocketPath.length
          ? fullSocketPath.substring(realSocketPath.length)
          : '';
    ...
    return await new Promise((resolve, reject) => {
      ...
      try {
        if (!httpModule) {
          throw new Error(
            'HTTP module not defined. This error should not happen.',
          );
        }
        const callback = (res: IncomingMessage) => {
          if (resolved) {
            LOG.warn(
              'Warning! Request had already ended when the response was received.',
            );
          } else {
            resolved = true;
            resolve(res);
        ...
        if (url) {
          options = {
            ...
            port:
              (urlParsed?.port ? parseInt(urlParsed.port, 10) : undefined) ??
  ...
  private static async _successJsonResponse(
    ...
    if (statusCode < 200 || statusCode >= 400) {
      throw NodeRequestClient._createUnsuccessfulResponseWithStatus(
        statusCode,
        response,
      );
    }
  ...
  private static async _successTextEntityResponse(
    ...
    const headers: Headers = new Headers(response.headers);
    const status: EntityStatusTypes = statusCode;
    return new ResponseEntity<string | undefined>(body, headers, status);
  }
  ...
  private async _checkSocketFile(path: string): Promise<boolean | undefined> {
    try {
      ...
      const code = isRegularObject(err) ? objectKey(err, 'code') : undefined;
      if (code === 'ENOTDIR') {
  ...
  private async _findSocketFile(fullPath: string): Promise<string | undefined> {
    ...
    const socketExists: boolean | undefined =
      await this._checkSocketFile(fullPath);
   ...
   * @private
   */
  private static _handleResponseStatusCode(response: TextHttpResponse): number {
    const statusCode: number = response?.statusCode;
    if (statusCode < 200 || statusCode >= 400) {
      throw NodeRequestClient._createUnsuccessfulResponseWithStatus(
        statusCode,
  ...
  private static _createUnsuccessfulResponseWithStatus(
    ...
    const headers: Headers = new Headers(response?.headers);
    const statusMessage: string = NodeRequestClient._stringifyErrorBodyJson(
      response?.body,
    );
