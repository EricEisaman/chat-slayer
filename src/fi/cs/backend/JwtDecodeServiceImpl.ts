...
export class JwtDecodeServiceImpl implements JwtDecodeService {
  ...
  public static decodePayloadAudience(token: string): string {
    ...
    if (!isString(aud)) {
      ...
      throw new TypeError(
        'decodePayloadAudience: Payload "aud" not string: ' + token,
      );
    }
    return aud;
  }
  ...
  public static decodePayloadSubject(token: string): string {
    ...
    if (!isString(sub)) {
      LOG.debug('payload: ', payload);
      throw new TypeError(
        'decodePayloadSubject: Payload "sub" not string: ' + token,
      );
    }
    return sub;
  }
  ...
  public static decodePayloadVerified(token: string): boolean {
    ...
    if (!isBoolean(verified)) {
      LOG.debug('payload: ', payload);
      throw new TypeError(
        'decodePayloadVerified: Payload "verified" not boolean: ' + token,
      );
    }
    return verified;
  }
  ...
  public static create(): JwtDecodeService {
