...
describe('EmailAuthControllerImpl', () => {
  ...
  describe('authenticateEmail', () => {
    ...
    it('should return internal server error if the email sending fails', async () => {
      ...
      (
        emailTokenService.createUnverifiedEmailToken as jest.Mock
      ).mockReturnValue({
        token: 'random_token',
        email: 'test@email.com',
        verified: false,
      ...
      (
        emailAuthMessageService.sendAuthenticationCode as jest.Mock
      ).mockImplementation(() => {
  ...
  describe('verifyEmailCode', () => {
    ...
    it('should return an verified email token if the code is correct', async () => {
      ...
      expect(emailTokenService.createVerifiedEmailToken).toHaveBeenCalledTimes(
        1,
      );
  ...
  describe('verifyTokenAndReturnSubject', () => {
    it('should have error if the token is not provided', async () => {
      ...
      // Then
      await expect(
        controller.verifyTokenAndReturnSubject(token),
      ).rejects.toThrow('Token was invalid: ');
    });
    ...
    it('should return subject information if the token is valid', async () => {
      ...
      (jwtDecodeService.decodePayloadSubject as jest.Mock).mockReturnValue(
        subjectInfo,
      );
