...
export class SmsAuthControllerImpl implements SmsAuthController {
    ...
    try {
      ...
      if (!isAuthenticateSmsDTO(body)) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Body not AuthenticateSmsDTO', 400))
          .status(400);
      }
      ...
      if (!sms) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('body.sms required', 400))
          .status(400);
      }
      sms = PhoneNumberUtils.normalizePhoneAddress(
        sms,
        this._defaultPhonePrefix,
      );

      const code: string =
        this._smsVerificationService.createVerificationCode(sms);
      const smsToken: SmsTokenDTO =
        this._smsTokenService.createUnverifiedSmsToken(sms);

      try {
        await this._smsAuthMessageService.sendAuthenticationCode(
          lang,
          sms,
          code,
        );
  ...
  public async verifySmsCode(
    ...
    try {
      if (!isVerifySmsCodeDTO(body)) {
        ...
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Body not VerifySmsCodeDTO', 400))
          .status(400);
      }
      ...
      if (
        !(sms && code && this._smsVerificationService.verifyCode(sms, code))
      ) {
      ...
      if (
        !(token && sms && this._smsTokenService.verifyToken(sms, token, false))
      ) {
      ...
      const smsToken: SmsTokenDTO =
        this._smsTokenService.createVerifiedSmsToken(sms);
  ...
  public async verifySmsToken(
    ...
    try {
      if (!isVerifySmsTokenDTO(body)) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Body not VerifySmsTokenDTO', 400))
          .status(400);
      }
      ...
      if (
        !(token && sms && this._smsTokenService.verifyToken(sms, token, true))
      ) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Access denied', 403))
          .status(403);
  ...
  public async verifyTokenAndReturnSubject(token: string): Promise<string> {
    ...
    if (!isString(token)) {
      throw new TypeError(
        'SmsAuthController.verifyTokenAndReturnSubject: Argument must be string',
      );
    }
    if (!this._smsTokenService.isTokenValid(token)) {
      throw new TypeError(
        'SmsAuthController.verifyTokenAndReturnSubject: Token was invalid: ' +
          token,
      );
    }
    ...
    if (!subject) {
      throw new TypeError(
        `SmsAuthController.verifyTokenAndReturnSubject: Token was not verified: ${token}`,
      );
    }
    return subject;
  }
