...
import {
  SmsTokenService,
  SmsTokenServiceAlgorithm,
} from '../core/auth/SmsTokenService';
...
export class SmsTokenServiceImpl implements SmsTokenService {
  ...
  protected constructor(
    ...
    this._unverifiedJwtTokenExpirationMinutes =
  ...
  public verifyToken(
    ...
    try {
      LOG.debug(
        `verifyToken: sms "${sms}", "${token}", ${requireVerifiedToken}`,
      );
      ...
      LOG.error(
        'verifyToken: Could not verify token: ',
        err,
        token,
        sms,
  ...
  public verifyValidTokenForSubject(
    ...
    try {
      ...
      if (payload?.sub !== sms) {
        LOG.debug(
          'verifyValidTokenForSubject: "sub" did not match: ',
          payload?.sub,
          sms,
        );
        return false;
      ...
      LOG.error(
        'verifyValidTokenForSubject: Could not verify token: ',
        err,
        token,
        sms,
      );
   ...
   * @param alg
   */
  public isTokenValid(token: string, alg?: SmsTokenServiceAlgorithm): boolean {
    try {
      if (!token) {
      ...
      LOG.error(
        'verifyValidToken: Exception: Could not verify token: ',
        err,
        token,
      );
      return false;
  ...
  public verifyTokenOnly(
    ...
    try {
      ...
      if (requireVerifiedToken) {
        if (!payload?.sub) {
          LOG.debug(
            'verifyTokenOnly: Property "sub" did not exists',
            payload?.sub,
          );
          return false;
        }
        ...
        if (!payload?.aud) {
          LOG.debug(
            'verifyTokenOnly: Property "aud" did not exist: ',
            payload?.aud,
          );
          return false;
        }
      ...
      LOG.error(
        'verifyTokenOnly: Could not verify token: ',
        err,
        token,
        requireVerifiedToken,
      );
  ...
  public createUnverifiedSmsToken(
    ...
    try {
      const signature = this._jwtEngine.sign(
        JwtUtils.createAudPayloadExpiringInMinutes(
          sms,
          this._unverifiedJwtTokenExpirationMinutes,
        ),
        alg,
      ...
      LOG.error(
        `createUnverifiedSmsToken: "${sms}": Could not sign JWT: `,
        err,
      );
  ...
  public createVerifiedSmsToken(
    ...
    try {
      const signature = this._jwtEngine.sign(
        JwtUtils.createSubPayloadExpiringInDays(
          sms,
          this._verifiedJwtTokenExpirationDays,
        ),
        alg,
      );
      return {
