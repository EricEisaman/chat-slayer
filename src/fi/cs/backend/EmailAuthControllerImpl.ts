...
import {
  createEmailTokenDTO,
  EmailTokenDTO,
} from '../core/auth/email/types/EmailTokenDTO';
...
import {
  ...
  sendEmailCodeDTO,
} from '../core/auth/email/types/SendEmailCodeDTO';
...
export class EmailAuthControllerImpl implements EmailAuthController {
    ...
    try {
      ...
      if (!isAuthenticateEmailDTO(body)) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Body not AuthenticateEmailDTO', 400))
          .status(400);
      }
      ...
      if (!email) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('body.email required', 400))
          .status(400);
      }

      const code: string =
        this._emailVerificationService.createVerificationCode(email);
      const emailToken: EmailTokenDTO =
        this._emailTokenService.createUnverifiedEmailToken(email);

      try {
        await this._emailAuthMessageService.sendAuthenticationCode(
          lang,
          email,
          code,
        );
        ...
        LOG.error(
          `authenticateEmail: Could not send email to '${email}': `,
          err,
        );
        return ResponseEntity.internalServerError<ErrorDTO>()
          .body(createErrorDTO('Internal error', 500))
          .status(500);
      }
      ...
      LOG.error('ERROR: ', err);
      return ResponseEntity.internalServerError<ErrorDTO>()
        .body(createErrorDTO('Internal Server Error', 500))
        .status(500);
    }
  }
  ...
  public async verifyEmailCode(
    ...
    try {
      if (!isVerifyEmailCodeDTO(body)) {
        ...
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Body not VerifyEmailCodeDTO', 400))
          .status(400);
      }
      ...
      if (
        !(
          email &&
          code &&
          this._emailVerificationService.verifyCode(email, code)
        )
      ) {
        LOG.info(`Access denied for "${email}" since code is not correct`);
        return ResponseEntity.internalServerError<ErrorDTO>()
          .body(createErrorDTO('Access denied', 403))
          .status(403);
      }
      ...
      if (
        !(
          token &&
          email &&
          this._emailTokenService.verifyToken(email, token, false)
        )
      ) {
      ...
      const emailToken: EmailTokenDTO =
        this._emailTokenService.createVerifiedEmailToken(email);
  ...
  public async verifyEmailToken(
    ...
    try {
      if (!isVerifyEmailTokenDTO(body)) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Body not VerifyEmailTokenDTO', 400))
          .status(400);
      }
      ...
      if (
        !(
          token &&
          email &&
          this._emailTokenService.verifyToken(email, token, true)
        )
      ) {
        return ResponseEntity.badRequest<ErrorDTO>()
          .body(createErrorDTO('Access denied', 403))
   ...
   */
  public async verifyTokenAndReturnSubject(token: string): Promise<string> {
    LOG.debug('verifyTokenAndReturnSubject: token = ', token);
    if (!isString(token)) {
      throw new TypeError(
        'EmailAuthController.verifyTokenAndReturnSubject: Argument must be string',
      );
    }
    if (!this._emailTokenService.isTokenValid(token)) {
      throw new TypeError(
        'EmailAuthController.verifyTokenAndReturnSubject: Token was invalid: ' +
          token,
      );
    }
    ...
    if (!subject) {
      throw new TypeError(
        `EmailAuthController.verifyTokenAndReturnSubject: Token was not verified: ${token}`,
      );
    }
    return subject;
  }
  ...
  public async authenticateEmailWithoutLogin(
    ...
    try {
      ...
      const emailResponse = createSendEmailCodeDTO(emailToken, code, lang);
