...
export class JwtEncodeServiceImpl implements JwtEncodeService {
  ...
  protected constructor(defaultAlgorithm: string = 'HS256') {
    this._defaultAlgorithm = defaultAlgorithm;
  }

  public static create(defaultAlgorithm: string = 'HS256') {
   ...
   */
  public createJwtEngine(secret: string, defaultAlgorithm?: string): JwtEngine {
    let _defaultAlgorithm: string | undefined = defaultAlgorithm;
    return {
      getDefaultAlgorithm: (): string =>
        _defaultAlgorithm ?? this.getDefaultAlgorithm(),
      ...
      sign: (payload: ReadonlyJsonObject, alg?: string): string =>
        jwsSign({
          header: {
            alg: (alg ?? this.getDefaultAlgorithm()) as unknown as Algorithm,
          },
          payload: payload,
          secret: secret,
        }),
      verify: (token: string, alg?: string): boolean =>
        jwsVerify(
          ...
          (alg ??
            _defaultAlgorithm ??
            this.getDefaultAlgorithm()) as unknown as Algorithm,
          secret,
        ),
    };
  }
