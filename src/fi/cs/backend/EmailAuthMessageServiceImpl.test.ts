...
describe('EmailAuthMessageServiceImpl', () => {
  ...
  describe('sendAuthenticationCode', () => {
    it('calls the translation service with the correct parameters', async () => {
      const service = EmailAuthMessageServiceImpl.create(
        mockEmailService,
        mockTranslationService,
      );
    ...
    it('sends an email with the correct parameters', async () => {
      ...
      (mockTranslationService.translateKeys as any).mockResolvedValue(
        translations,
      );
      ...
      const expectedEmail = {
        ...
        content:
          translations[T_M_AUTH_CODE_HEADER_TEXT] +
          translations[T_M_AUTH_CODE_BODY_TEXT] +
          translations[T_M_AUTH_CODE_FOOTER_TEXT],
        htmlContent:
          translations[T_M_AUTH_CODE_HEADER_HTML] +
          translations[T_M_AUTH_CODE_BODY_HTML] +
          translations[T_M_AUTH_CODE_FOOTER_HTML],
      };
      expect(mockEmailService.sendEmailMessage).toHaveBeenCalledWith(
        expectedEmail,
      );
    });
  });
