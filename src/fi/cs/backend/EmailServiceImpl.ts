...
export class EmailServiceImpl implements EmailService {
   ...
   * @param transporter
   */
  protected constructor(from: string, transporter?: any | undefined) {
    this._from = from;
  ...
  public static create(
    ...
    return new EmailServiceImpl(from, transporter);
  }
  ...
  public initialize(config?: string): void {
    ...
    function parseBooleanParam(u: URL, key: string): boolean | undefined {
      return u?.searchParams?.has(key)
        ? parseBoolean(u?.searchParams?.get(key))
        : undefined;
    }

    function parseNonEmptyStringParam(u: URL, key: string): string | undefined {
      return u?.searchParams?.has(key)
        ? parseNonEmptyString(u?.searchParams?.get(key))
        : undefined;
    }
    ...
    const tlsServerName: string | undefined = parseNonEmptyStringParam(
      u,
      'tls-server-name',
    );
    const tlsRejectUnauthorized: boolean | undefined = parseBooleanParam(
      u,
      'tls-server-name',
    );

    LOG.debug(
      `Config "${config}" parsed as ${hostname}:${port} ${
        username && password
  ...
  public async sendEmailMessage(message: EmailMessage) {
    if (!this._transporter)
      throw new TypeError('EmailServiceImpl not initialized');
    ...
    const recipientList: string[] = uniq([...toList, ...ccList]);
    ...
    LOG.debug(
      `Sending message "${subject}" to "${recipientString}" from "${from}"`,
    );
