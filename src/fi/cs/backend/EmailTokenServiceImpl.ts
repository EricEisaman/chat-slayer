...
import {
  EmailTokenService,
  EmailTokenServiceAlgorithm,
} from '../core/auth/EmailTokenService';
...
export class EmailTokenServiceImpl implements EmailTokenService {
  ...
  public verifyToken(
    ...
    try {
      LOG.debug(
        `verifyToken: email "${email}", "${token}", ${requireVerifiedToken}`,
      );
  ...
  public createUnverifiedEmailToken(
    ...
    try {
      ...
      LOG.error(
        `createUnverifiedEmailToken: "${email}": Could not sign JWT: `,
        err,
      );
  ...
  public createVerifiedEmailToken(
    ...
    try {
      ...
      LOG.error(
        `createVerifiedEmailToken: "${email}": Could not sign JWT: `,
        err,
      );
      throw new TypeError(`Could not sign JWT for "${email}"`);
    }
