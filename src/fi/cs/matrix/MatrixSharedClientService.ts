...
import {
  SimpleSharedClientService,
  SharedClientServiceDestructor,
} from '../core/simpleRepository/types/SimpleSharedClientService';
...
export class MatrixSharedClientService implements SimpleSharedClientService {
  ...
  public constructor() {
    this._observer = new Observer<SimpleSharedClientServiceEvent>(
  ...
  public async login(url: string): Promise<void> {
    ...
    LOG.debug(
      `Logging in to "https://${hostname}" as "${username}" with "${password}"`,
    );
  ...
  public async initialize(url: string): Promise<void> {
    ...
    if (
      this._observer.hasCallbacks(SimpleSharedClientServiceEvent.INITIALIZED)
    ) {
