...
export class DeviceRepositoryService implements SimpleRepositoryService {
   ...
   * @param id
   */
  public async findDeviceByDeviceId(
    id: string,
  ): Promise<DeviceRepositoryItem | undefined> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('Repository uninitialized');
    const foundItem:
      | SimpleRepositoryEntry<StoredDeviceRepositoryItem>
      | undefined = await this._repository.findByProperty('deviceId', id);
    if (!foundItem) return undefined;
    return this._toDeviceRepositoryItem(foundItem);
  }
  ...
  public async saveDevice(
    ...
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('Repository uninitialized');
    const storedRepositoryItem: StoredDeviceRepositoryItem =
      toStoredDeviceRepositoryItem(item);
    const foundItem =
      await this._repository.updateOrCreateItem(storedRepositoryItem);
    return this._toDeviceRepositoryItem(foundItem);
  }
  ...
  private _toDeviceRepositoryItem(
    storedItem: SimpleRepositoryEntry<StoredDeviceRepositoryItem>,
  ): DeviceRepositoryItem {
    const id = storedItem.id;
    const target = storedItem.data?.target;
    LOG.debug(`Device with id "${id}": `, storedItem, target);
    const item = parseDeviceRepositoryItem(id, target);
    LOG.debug(`Device "${id}" parsed as: `, item);
    if (!item) {
      throw new TypeError(
        `DeviceRepositoryService: Could not parse "${storedItem.id}" and ${JSON.stringify(target)}`,
      );
    }
    return item;
  }
