...
import {
  ...
  EventRepositoryItem,
  toStoredEventRepositoryItem,
} from './EventRepositoryItem';
...
export class EventRepositoryService implements SimpleRepositoryService {
  ...
  protected _repository:
    | SimpleRepository<StoredEventRepositoryItem>
    | undefined;
  ...
  public async getAllEvents(): Promise<readonly EventRepositoryItem[]> {
    const list: readonly SimpleRepositoryEntry<StoredEventRepositoryItem>[] =
      await this._getAllEvents();
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredEventRepositoryItem>,
      ): EventRepositoryItem => {
  ...
  public async getSomeEvents(
    ...
    const list: readonly SimpleRepositoryEntry<StoredEventRepositoryItem>[] =
      await this._getSomeEvents(idList);
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredEventRepositoryItem>,
  ...
  public async findEventById(
    id: string,
  ): Promise<EventRepositoryItem | undefined> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('No this._repository');
    const foundItem:
      | SimpleRepositoryEntry<StoredEventRepositoryItem>
  ...
  public async deleteSomeEvents(idList: readonly string[]): Promise<void> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('No this._repository');
    const list: readonly SimpleRepositoryEntry<StoredEventRepositoryItem>[] =
      await this._getSomeEvents(idList);
  ...
  public async createEvent(
    ...
    if (!this._repository) throw new TypeError('No this._repository');
    const createdItem: SimpleRepositoryEntry<StoredEventRepositoryItem> =
      await this._repository.createItem(storedItemTemplate);
  ...
  public async saveEvent(
    ...
    const foundItem = await this._repository.updateOrCreateItem(
      toStoredEventRepositoryItem(item),
    );
    return this._toEventRepositoryItem(foundItem);
  }
  ...
  private async _getAllEvents(): Promise<
    readonly SimpleRepositoryEntry<StoredEventRepositoryItem>[]
  > {
    if (!this._repository) throw new TypeError('No this._repository');
  ...
  private _toEventRepositoryItem(
    storedItem: SimpleRepositoryEntry<StoredEventRepositoryItem>,
  ): EventRepositoryItem {
    ...
    if (!isEventEntity(data)) {
      ...
      throw new TypeError(
        `EventRepositoryService: Could not parse repository item "${id}" because ${explainEventEntity(data)}`,
      );
    }
