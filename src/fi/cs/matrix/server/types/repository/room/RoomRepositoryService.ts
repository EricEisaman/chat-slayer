...
import {
  RoomRepositoryItem,
  parseRoomRepositoryItem,
  toStoredRoomRepositoryItem,
} from './RoomRepositoryItem';
...
export class RoomRepositoryService implements SimpleRepositoryService {
  ...
  protected _repositoryInitializer:
    | SimpleRepositoryInitializer<StoredRoomRepositoryItem>
    | undefined;
  ...
  public async getAllRooms(): Promise<readonly RoomRepositoryItem[]> {
    const list: readonly SimpleRepositoryEntry<StoredRoomRepositoryItem>[] =
      await this._getAllRooms();
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredRoomRepositoryItem>,
      ): RoomRepositoryItem => {
  ...
  public async getSomeRooms(
    ...
    const list: readonly SimpleRepositoryEntry<StoredRoomRepositoryItem>[] =
      await this._getSomeRooms(idList);
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredRoomRepositoryItem>,
  ...
  public async findRoomById(
    id: string,
  ): Promise<RoomRepositoryItem | undefined> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('Repository uninitialized');
    const foundItem:
      | SimpleRepositoryEntry<StoredRoomRepositoryItem>
  ...
  public async deleteSomeRooms(idList: readonly string[]): Promise<void> {
    await this._sharedClientService.waitForInitialization();
    const list: readonly SimpleRepositoryEntry<StoredRoomRepositoryItem>[] =
      await this._getSomeRooms(idList);
    if (!this._repository) throw new TypeError('Repository uninitialized');
  ...
  public async createRoom(
    ...
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('Repository uninitialized');
    const createdItem = await this._repository.createItem(
      toStoredRoomRepositoryItem(item),
    );
    return this._toRoomRepositoryItem(createdItem);
  }

  public async saveRoom(item: RoomRepositoryItem): Promise<RoomRepositoryItem> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('Repository uninitialized');
  ...
  private async _getAllRooms(): Promise<
    readonly SimpleRepositoryEntry<StoredRoomRepositoryItem>[]
  > {
    if (!this._repository) throw new TypeError('Repository uninitialized');
  ...
  private _toRoomRepositoryItem(
    storedItem: SimpleRepositoryEntry<StoredRoomRepositoryItem>,
  ): RoomRepositoryItem {
    const id = storedItem.id;
    const target = storedItem.data?.target;
    LOG.debug(`Room with id "${id}": `, storedItem, target);
    const item = parseRoomRepositoryItem(id, target);
    LOG.debug(`Room "${id}" as: `, item);
    if (!item) {
      throw new TypeError(
        `RoomRepositoryService: Could not parse "${storedItem.id}" and ${JSON.stringify(target)}`,
      );
    }
    return item;
  }
