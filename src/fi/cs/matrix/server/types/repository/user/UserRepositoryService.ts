...
import {
  UserRepositoryItem,
  parseUserRepositoryItem,
  toStoredUserRepositoryItem,
} from './UserRepositoryItem';
...
export class UserRepositoryService implements SimpleRepositoryService {
  ...
  public async getAllUsers(): Promise<readonly UserRepositoryItem[]> {
    const list: readonly SimpleRepositoryEntry<StoredUserRepositoryItem>[] =
      await this._getAllUsers();
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredUserRepositoryItem>,
      ): UserRepositoryItem => {
  ...
  public async getSomeUsers(
    ...
    const list: readonly SimpleRepositoryEntry<StoredUserRepositoryItem>[] =
      await this._getSomeUsers(idList);
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredUserRepositoryItem>,
  ...
  public async findUserById(
    ...
    const foundItem:
      | SimpleRepositoryEntry<StoredUserRepositoryItem>
  ...
  public async findByUsername(
    ...
    const foundItem:
      | SimpleRepositoryEntry<StoredUserRepositoryItem>
      | undefined = await this._repository.findByProperty(
      'username',
      toLower(username),
    );
    if (!foundItem) return undefined;
  ...
  public async deleteSomeUsers(idList: readonly string[]): Promise<void> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('No this._repository');
    const list: readonly SimpleRepositoryEntry<StoredUserRepositoryItem>[] =
      await this._getSomeUsers(idList);
  ...
  public async createUser(
    ...
    const createdStoredItem: SimpleRepositoryEntry<StoredUserRepositoryItem> =
      await this._repository.createItem(toStoredUserRepositoryItem(item));
    return this._toUserRepositoryItem(createdStoredItem);
  }

  public async saveUser(item: UserRepositoryItem): Promise<UserRepositoryItem> {
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('No this._repository');
    const foundItem = await this._repository.updateOrCreateItem(
      toStoredUserRepositoryItem(item),
    );
    return this._toUserRepositoryItem(foundItem);
  }
  ...
  private async _getAllUsers(): Promise<
    readonly SimpleRepositoryEntry<StoredUserRepositoryItem>[]
  > {
    if (!this._repository) throw new TypeError('No this._repository');
  ...
  private _toUserRepositoryItem(
    storedItem: SimpleRepositoryEntry<StoredUserRepositoryItem>,
  ): UserRepositoryItem {
    const id = storedItem.id;
    const target = storedItem.data?.target;
    LOG.debug(`User with id "${id}": `, storedItem, target);
    const item = parseUserRepositoryItem(id, target);
    LOG.debug(`User "${id}" as: `, item);
    if (!item) {
      throw new TypeError(
        `UserRepositoryService: Could not parse "${storedItem.id}" and ${JSON.stringify(target)}`,
      );
    }
    return item;
  }
