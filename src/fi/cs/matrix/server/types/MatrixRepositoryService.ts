...
import {
  ...
  parseDeviceRepositoryItem,
  toStoredDeviceRepositoryItem,
} from './repository/device/DeviceRepositoryItem';
import {map} from '../../../core/functions/map';
import {parseJson} from '../../../core/Json';
import {isDevice} from './repository/device/Device';

const LOG = LogService.createLogger('CsMatrixDeviceRepositoryService');

export type CsMatrixDeviceRepositoryServiceDestructor = ObserverDestructor;

export class CsMatrixDeviceRepositoryService implements SimpleRepositoryService {
  ...
  protected _repository:
    | SimpleRepository<StoredDeviceRepositoryItem>
    | undefined;
  protected _repositoryInitializer:
    | SimpleRepositoryInitializer<StoredDeviceRepositoryItem>
    | undefined;
  ...
  public constructor(
    ...
    this._observer = new Observer<SimpleRepositoryServiceEvent>(
      'CsMatrixDeviceRepositoryService',
    );
  ...
  ): CsMatrixDeviceRepositoryServiceDestructor {
  ...
  public async initialize(): Promise<void> {
    ...
    if (!this._repositoryInitializer)
      throw new TypeError('Repository uninitialized');
    const client = this._sharedClientService.getClient();
    if (!client) throw new TypeError('Client uninitialized');
    this._repository =
      await this._repositoryInitializer.initializeRepository(client);
  ...
  public async getAllDevices(): Promise<readonly DeviceRepositoryItem[]> {
    const list: readonly SimpleRepositoryEntry<StoredDeviceRepositoryItem>[] =
      await this._getAllDevices();
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredDeviceRepositoryItem>,
      ): DeviceRepositoryItem => {
        const data = parseJson(item.data);
        if (!isDevice(data))
          throw new TypeError(
            `MatrixRepositoryService: Could not parse data: ${item.data}`,
          );
        return createDeviceRepositoryItem(item.id, data);
      },
    );
  }
  ...
  public async getSomeDevices(
    ...
    const list: readonly SimpleRepositoryEntry<StoredDeviceRepositoryItem>[] =
      await this._getSomeDevices(idList);
    return map(
      list,
      (
        item: SimpleRepositoryEntry<StoredDeviceRepositoryItem>,
  ...
  public async getDeviceById(
    ...
    const foundItem:
      | SimpleRepositoryEntry<StoredDeviceRepositoryItem>
      | undefined = await this._repository.findById(id);
  ...
  public async deleteSomeDevices(idList: readonly string[]): Promise<void> {
    await this._sharedClientService.waitForInitialization();
    const list: readonly SimpleRepositoryEntry<StoredDeviceRepositoryItem>[] =
      await this._getSomeDevices(idList);
    if (!this._repository) throw new TypeError('Repository uninitialized');
  ...
  public async saveDevice(
    ...
    await this._sharedClientService.waitForInitialization();
    if (!this._repository) throw new TypeError('Repository uninitialized');
    const foundItem = await this._repository.updateOrCreateItem(
      toStoredDeviceRepositoryItem(item),
    );
  ...
  private async _getAllDevices(): Promise<
    readonly SimpleRepositoryEntry<StoredDeviceRepositoryItem>[]
  > {
    if (!this._repository) throw new TypeError('Repository uninitialized');
