...
} from '../core/simpleRepository/types/SimpleRepository';
...
} from './types/event/roomJoinRules/RoomJoinRulesAllowConditionDTO';
...
import {
  isStoredRepositoryItem,
  SimpleStoredRepositoryItem,
  StoredRepositoryItemExplainCallback,
  StoredRepositoryItemTestCallback,
} from '../core/simpleRepository/types/SimpleStoredRepositoryItem';
 ...
 */
export class MatrixCrudRepository<
...
> implements SimpleRepository<T> {
  ...
  public constructor(
    ...
    if (!isMatrixType(stateType)) {
      throw new TypeError(
        'MatrixCrudRepository: stateType invalid: ' + stateType,
      );
    }
    ...
    this._deletedType =
      parseNonEmptyString(deletedType) ?? MatrixType.FI_NOR_DELETED;
    ...
    this._explainT =
      ...
      ((value: unknown): string =>
        this._isT(value) ? explainOk() : explainNot(this._tName));
  ...
  public async getAll(): Promise<readonly SimpleRepositoryEntry<T>[]> {
    ...
    if (!this.isRepositoryEntryList(list)) {
      throw new TypeError(
        `MatrixCrudRepository.getAll: Illegal data from database: ${this.explainRepositoryEntryList(list)}`,
      );
    }
    return list;
  }
  ...
  public async getSome(
    ...
    const list = filter(
      allList,
      (item: SimpleRepositoryEntry<T>): boolean =>
        !!item?.id && idList.includes(item?.id),
    );
    if (!this.isRepositoryEntryList(list)) {
      throw new TypeError(
        `MatrixCrudRepository.getSome: Illegal data from database: ${this.explainRepositoryEntryList(list)}`,
      );
    }
    return list;
  }
  ...
  public async getAllByProperty(
    ...
    const list = map(
      filter(
        items,
        (item: SimpleRepositoryEntry<T>): boolean =>
          get(item?.data, propertyName) === propertyValue,
      ),
    ...
    if (!this.isRepositoryEntryList(list)) {
      throw new TypeError(
        `MatrixCrudRepository.getAllByProperty: Illegal data from database: ${this.explainRepositoryEntryList(list)}`,
      );
    }
    return list;
  }
  ...
  public async createItem(
    ...
    const invitedMembers: readonly MatrixUserId[] = filter(
      uniq(
        concat(
    ...
    const allowedGroups: readonly MatrixRoomId[] | undefined =
      this._allowedGroups;
    ...
    let initialState: readonly MatrixStateEventOf<any>[] = [
      ...
      createMatrixStateEvent(this._stateType, this._stateKey, content),
        ...
        ...[
          createRoomJoinRulesStateEventDTO(
            createRoomJoinRulesStateContentDTO(
              ...
              map(
                allowedGroups,
                (item: MatrixRoomId): RoomJoinRulesAllowConditionDTO =>
                  createRoomJoinRulesAllowConditionDTO(
    ...
    const inviteOptions: Partial<MatrixCreateRoomDTO> = invitedMembers.length
    ...
    const response: MatrixCreateRoomResponseDTO =
      await this._client.createRoom(options);
    ...
    if (
      ...
      clientUserId &&
      clientUserId !== this._serviceAccount.getUserId()
    ) {
      try {
        await this._serviceAccount.joinRoom(room_id);
      } catch (err: unknown) {
        LOG.warn(
          `Warning! Could not join service account to room "${room_id}": `,
          err,
        );
      }
    }
  ...
  public async findById(
    ...
    const response: GetRoomStateByTypeResponseDTO | undefined =
      await this._client.getRoomStateByType(
        id,
        this._stateType,
        this._stateKey,
      );
    ...
    if (!isJsonObject(data)) {
      throw new TypeError(
        `MatrixCrudRepository.findById: data was not JsonObject: ${data}`,
      );
    }
    ...
    if (!isInteger(version)) {
      throw new TypeError(
        `MatrixCrudRepository.findById: version was not integer: ${version}`,
      );
    }

    let members: readonly SimpleRepositoryMember[] | undefined = undefined;
    if (includeMembers) {
      const dto: MatrixRoomJoinedMembersDTO =
        await this._client.getJoinedMembers(id);
      members = map(
        keys(dto.joined),
        (memberId: string): SimpleRepositoryMember => {
  ...
  public async findByProperty(
    ...
    if (resultCount !== 1)
      throw new TypeError(
        `MemoryRepository.findByProperty: Multiple items found by property "${propertyName}" as: ${propertyValue}`,
      );
    return result[0];
  }
  ...
  public async findByIdAndUpdate(
    ...
    if (rItem === undefined)
      throw new TypeError(`findByIdAndUpdate: Could not find item for "${id}"`);
  ...
  public async update(
    ...
    if (!isJsonObject(jsonData)) {
      throw new TypeError(
        `MatrixCrudRepository.update: jsonData was not JsonObject: ${jsonData}`,
      );
    }
    ...
    if (!isInteger(newVersion)) {
      throw new TypeError(
        `MatrixCrudRepository.update: newVersion was not integer: ${newVersion}`,
      );
    }
    ...
    const response: PutRoomStateWithEventTypeResponseDTO =
      await this._client.setRoomStateByType(
        id,
        this._stateType,
        this._stateKey,
        content,
  ...
  public async updateOrCreateItem(item: T): Promise<SimpleRepositoryEntry<T>> {
    if (!isJsonObject(item)) {
      throw new TypeError(
        `MatrixCrudRepository.updateOrCreateItem: jsonData was not JsonObject: ${item}`,
      );
    }
    const id = item.id;
    const foundItem: SimpleRepositoryEntry<T> | undefined =
      id !== REPOSITORY_NEW_IDENTIFIER ? await this.findById(id) : undefined;
    if (foundItem) {
  ...
  public async deleteById(id: string): Promise<SimpleRepositoryEntry<T>> {
    ...
    try {
      ...
      if (!isInteger(newVersion)) {
        throw new TypeError(
          `MatrixCrudRepository.deleteById: newVersion was not integer: ${newVersion}`,
        );
      }
      ...
      const deletedResponse: PutRoomStateWithEventTypeResponseDTO =
        await this._client.setRoomStateByType(
          id,
          this._deletedType,
          this._deletedKey,
      ...
      if (this._serviceAccount) {
        try {
          ...
          LOG.warn(
            `Warning! Service account could not leave from the room ${id}: `,
            err,
          );
        }
        ...
        try {
          ...
          LOG.warn(
            `Warning! Service account could not forget the room ${id}: `,
            err,
          );
        }
      }
      ...
      if (
        ...
        [401, 403, 404].includes(err.getStatusCode())
      ) {
        throw err;
      }
  ...
  public async deleteByIdList(
    ...
    if (!this.isRepositoryEntryList(results)) {
      throw new TypeError(
        `MatrixCrudRepository.getAllByProperty: Illegal data from database: ${this.explainRepositoryEntryList(results)}`,
      );
    }
    return results;
  }
  ...
  public async deleteByList(
    list: readonly SimpleRepositoryEntry<T>[],
  ): Promise<readonly SimpleRepositoryEntry<T>[]> {
    ...
    await reduce(
      ...
      async (p: Promise<void>, item: string) => {
        ...
        try {
          ...
          if (
            this._client.isAlreadyInTheRoom(
              SimpleMatrixClient.getErrorBody(err),
            )
          )
            return;

          LOG.error(
            `Warning! Could not invite user ${item} to room ${id}: `,
            err,
          );
          throw err;
        }
  ...
  public async waitById(
    ...
    if (!id)
      throw new TypeError(
        `MatrixCrudRepository.waitById: id is required: ${id}`,
      );
  ...
  public explainRepositoryEntryList(list: unknown): string {
    return SimpleRepositoryUtils.explainRepositoryEntryList(
      list,
      this._isT,
      this._explainT,
      this._tName,
  ...
  private async _getAll(): Promise<readonly SimpleRepositoryEntry<T>[]> {
      ...
      await reduce(
        ...
        async (p, roomId: MatrixRoomId): Promise<void> => {
          ...
          try {
            ...
            LOG.warn(
              `Warning! Could not join client "${this._client.getUserId()}" to room: ${roomId}`,
            );
          }
        },
        Promise.resolve(),
    ...
    return reduce(
      joinedRooms,
      (
        result: readonly SimpleRepositoryEntry<T>[],
        roomId: MatrixRoomId,
      ): readonly SimpleRepositoryEntry<T>[] => {
        ...
        return concat(
          ...
          map(
            events,
            (
              item: MatrixSyncResponseRoomEventDTO,
