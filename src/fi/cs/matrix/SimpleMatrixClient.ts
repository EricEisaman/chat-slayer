...
import {
  createMatrixPasswordLoginRequestDTO,
  MatrixPasswordLoginRequestDTO,
} from './types/request/passwordLogin/MatrixPasswordLoginRequestDTO';
import {
  createMatrixTextMessageDTO,
  MatrixTextMessageDTO,
} from './types/message/textMessage/MatrixTextMessageDTO';
...
} from './types/response/sync/types/MatrixSyncResponseInvitedRoomDTO';
import {
  MatrixSyncResponseLeftRoomDTO,
  getEventsFromMatrixSyncResponseLeftRoomDTO,
} from './types/response/sync/types/MatrixSyncResponseLeftRoomDTO';
...
import {
  ...
  isMatrixJoinRoomResponseDTO,
} from './types/response/joinRoom/MatrixJoinRoomResponseDTO';
import {
...
import {
  ...
  isMatrixRegisterResponseDTO,
} from './types/response/register/MatrixRegisterResponseDTO';
...
} from './types/synapse/SynapseRegisterResponseDTO';
...
import {
  isMatrixWhoAmIResponseDTO,
  MatrixWhoAmIResponseDTO,
} from './types/response/whoami/MatrixWhoAmIResponseDTO';
import {createMatrixIdentifierDTO} from './types/request/login/types/MatrixIdentifierDTO';
import {
  GetRoomStateByTypeResponseDTO,
  isGetRoomStateByTypeResponseDTO,
} from './types/response/getRoomStateByType/GetRoomStateByTypeResponseDTO';
...
export class SimpleMatrixClient implements SimpleRepositoryClient {
  ...
  public constructor(
    ...
    this._state = accessToken
      ? SimpleMatrixClientState.AUTHENTICATED
      : SimpleMatrixClientState.UNAUTHENTICATED;
    this._originalUrl = url;
    this._homeServerUrl = SimpleMatrixClient._normalizeUrl(
      homeServerUrl ?? url,
    );
    ...
    this._observer = new Observer<SimpleMatrixClientEvent>(
      'SimpleMatrixClient',
    );
    ...
    try {
      ...
      const response: any = await this._postJson(
        ...
        access_token
          ? {
              [MATRIX_AUTHORIZATION_HEADER_NAME]:
                AuthorizationUtils.createBearerHeader(access_token),
            }
          : undefined,
      );
      if (!isMatrixRegisterResponseDTO(response)) {
        ...
        throw new TypeError(
          `${this._observer.getName()}.register: Response was invalid`,
        );
      }
      ...
      if (err instanceof RequestError) {
        ...
        if (statusCode === 400) {
          const errorBody: any = SimpleMatrixClient.getErrorBody(err);
          if (isMatrixErrorDTO(errorBody)) {
            switch (errorBody.errcode) {
              ...
              case MatrixErrorCode.M_EXCLUSIVE:
                throw new RequestError(
                  RequestStatus.Conflict,
                  'User name conflicts with exclusive namespace',
                );
              default:
                throw new RequestError(
  ...
  public async whoamiDTO(): Promise<MatrixWhoAmIResponseDTO | undefined> {
    ...
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}.whoamiDTO: Client did not have access token`,
      );
    }
    ...
    const response: any = await this._getJson(
      ...
      {
        [MATRIX_AUTHORIZATION_HEADER_NAME]:
          AuthorizationUtils.createBearerHeader(accessToken),
      },
    );
    LOG.debug('whoamiDTO: response = ', response);
    if (!isMatrixWhoAmIResponseDTO(response)) {
      throw new TypeError(
        `${this._observer.getName()}.whoamiDTO: Response was not MatrixWhoAmIResponseDTO: ${JSON.stringify(response)}`,
      );
    }
  ...
  public async getRegisterNonce(): Promise<string> {
    try {
      ...
      const nonceResponse: any = await this._getJson(
        this._homeServerUrl + SYNAPSE_REGISTER_URL,
      );
      const nonce = nonceResponse?.nonce ?? undefined;
      if (!nonce)
        throw new TypeError(
          `${this._observer.getName()}.getRegisterNonce: No nonce detected`,
        );
      return nonce;
    } catch (err: unknown) {
      LOG.debug('Could not fetch nonce for registration: ', err);
      throw new TypeError(
        `${this._observer.getName()}.getRegisterNonce: Could not fetch nonce for the register request. Is it Synapse?`,
      );
    }
  }
  ...
  public async registerWithSharedSecret(
    ...
    try {
      ...
      if (!isSynapseRegisterResponseDTO(response)) {
        LOG.debug(
          'registerWithSharedSecret: Invalid response received: ',
          response,
        );
        throw new TypeError(
          `${this._observer.getName()}.registerWithSharedSecret: Response was invalid`,
        );
      }
      LOG.debug(
        'registerWithSharedSecret: RegisterResponseDTO received: ',
        response,
      );
      return response;
    } catch (err: unknown) {
  ...
  public async login(
    ...
    try {
      ...
      if (!isMatrixLoginResponseDTO(response)) {
        LOG.debug('Invalid response received: ', response);
        throw new TypeError(
          `${this._observer.getName()}.login: Response was invalid`,
        );
      }
      ...
      if (response?.well_known) {
        const responseHomeServerUrl =
          response.well_known[MatrixType.M_HOMESERVER]?.base_url;
        if (responseHomeServerUrl) {
          homeServerUrl = responseHomeServerUrl;
        } else {
          homeServerUrl = originalUrl;
        }
        const responseIdentityServerUrl =
          response.well_known[MatrixType.M_IDENTITY_SERVER]?.base_url;
      ...
      if (!access_token) {
        throw new TypeError(
          `${this._observer.getName()}.login: Response did not have access_token`,
        );
      }
      const user_id = response?.user_id;
      if (!user_id) {
        throw new TypeError(
          `${this._observer.getName()}.login: Response did not have user_id`,
        );
      }
   ...
   *     state
   */
  public async authenticate(access_token: string): Promise<SimpleMatrixClient> {
    try {
      return new SimpleMatrixClient(
        this._originalUrl,
        undefined,
  ...
  public async resolveRoomId(name: string): Promise<string | undefined> {
    try {
      ...
      if (!isGetDirectoryRoomAliasResponseDTO(response)) {
        LOG.debug(
          'resolveRoomId: response was not GetDirectoryRoomAliasResponseDTO: ',
          response,
        );
        throw new TypeError(
          `${this._observer.getName()}.resolveRoomId: Response was not GetDirectoryRoomAliasResponseDTO: ${response}`,
        );
      }
      ...
      if (
        ...
        err.getStatusCode() === RequestStatus.NotFound
      ) {
        return undefined;
      } else {
  ...
  public async getJoinedMembers(
    roomId: MatrixRoomId,
  ): Promise<MatrixRoomJoinedMembersDTO> {
    const accessToken: string | undefined = this._accessToken;
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}.getJoinedMembers: Client did not have access token`,
      );
    }
    ...
    if (!isMatrixRoomJoinedMembersDTO(response)) {
      LOG.debug(
        'getJoinedMembers: response was not MatrixRoomJoinedMembersDTO: ',
        response,
      );
      throw new TypeError(
        `${this._observer.getName()}.getJoinedMembers: Response was not MatrixRoomJoinedMembersDTO: ${response}`,
      );
    }
  ...
  public async getRoomStateByType(
    ...
    try {
      ...
      if (!accessToken) {
        throw new TypeError(
          `${this._observer.getName()}.getRoomStateByType: Client did not have access token`,
        );
      }

      LOG.debug(
        `getRoomStateByType: roomId="${roomId}", eventType="${eventType}", stateKey="${stateKey}" `,
      );

      const response: any = await this._getJson(
        this._homeServerUrl +
          MATRIX_ROOM_EVENT_STATE_FETCH_URL(roomId, eventType, stateKey),
        {
          [MATRIX_AUTHORIZATION_HEADER_NAME]:
            AuthorizationUtils.createBearerHeader(accessToken),
        },
      ...
      if (!isGetRoomStateByTypeResponseDTO(response)) {
        LOG.debug(
          'getRoomStateByType: response was not GetRoomStateByTypeResponseDTO: ',
          response,
        );
        throw new TypeError(
          `${this._observer.getName()}.getRoomStateByType: Response was not GetRoomStateByTypeResponseDTO: ${JSON.stringify(response)}`,
        );
      }
  ...
  public async setRoomStateByType(
    ...
    try {
      ...
      if (!accessToken) {
        throw new TypeError(
          `${this._observer.getName()}.setRoomStateByType: Client did not have access token`,
        );
      }

      const response: JsonAny | undefined = await this._putJson(
        this._homeServerUrl +
          MATRIX_ROOM_EVENT_STATE_UPDATE_URL(roomId, eventType, stateKey),
      ...
      if (!isPutRoomStateWithEventTypeResponseDTO(response)) {
        LOG.debug(
          'setRoomStateByType: response was not PutRoomStateWithEventTypeDTO: ',
          response,
        );
        throw new TypeError(
          `${this._observer.getName()}.setRoomStateByType: Response was not PutRoomStateWithEventTypeDTO: ${JSON.stringify(response)}`,
        );
      }
   ...
   * @param roomId
   */
  public async forgetRoom(roomId: string): Promise<void> {
    try {
      const accessToken: string | undefined = this._accessToken;
      if (!accessToken) {
        throw new TypeError(
          `${this._observer.getName()}.forgetRoom: Client did not have access token`,
        );
      }
  ...
  public async leaveRoom(roomId: string): Promise<void> {
    try {
      ...
      if (!accessToken) {
        throw new TypeError(
          `${this._observer.getName()}.leaveRoom: Client did not have access token`,
        );
      }
  ...
  public async inviteToRoom(
    ...
    try {
      if (!isMatrixRoomId(roomId)) {
        throw new TypeError(
          `${this._observer.getName()}.inviteToRoom: roomId invalid: ${roomId}`,
        );
      }

      if (!isMatrixUserId(userId)) {
        throw new TypeError(
          `${this._observer.getName()}.inviteToRoom: userId invalid: ${userId}`,
        );
      }
      ...
      if (!accessToken) {
        throw new TypeError(
          `${this._observer.getName()}.inviteToRoom: Client did not have access token`,
        );
      }
      ...
      if (this.isAlreadyInTheRoom(SimpleMatrixClient.getErrorBody(err))) return;
  ...
  public async sendTextMessage(roomId: string, body: string): Promise<void> {
    ...
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}.sendTextMessage: Client did not have access token`,
      );
    }
    ...
    const response: Json | undefined = await this._postJson(
      this._homeServerUrl +
        MATRIX_ROOM_SEND_EVENT_URL(roomId, MatrixType.M_ROOM_MESSAGE),
      requestBody as unknown as JsonAny,
      {
        [MATRIX_AUTHORIZATION_HEADER_NAME]:
          AuthorizationUtils.createBearerHeader(accessToken),
  ...
  public async createRoom(
    ...
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}.createRoom: Client did not have access token`,
      );
    }
    ...
    if (!isMatrixCreateRoomResponseDTO(response)) {
      ...
      throw new TypeError(
        `${this._observer.getName()}.createRoom: Response was not MatrixCreateRoomResponseDTO: ` +
          response,
      );
    }
  ...
  public async joinRoom(
    ...
    try {
      ...
      if (!accessToken) {
        throw new TypeError(
          `${this._observer.getName()}.joinRoom: Client did not have access token`,
        );
      }
      ...
      if (!isMatrixJoinRoomResponseDTO(response)) {
        LOG.debug('response = ', response);
        throw new TypeError(
          `${this._observer.getName()}.joinRoom: Could not join to "${roomId}": Response was not MatrixJoinRoomResponseDTO: ` +
            response,
        );
      }
      ...
      if (this.isAlreadyInTheRoom(SimpleMatrixClient.getErrorBody(err))) {
        return {room_id: roomId};
      }

      const body = SimpleMatrixClient.getErrorBody(err);
      if (
        isMatrixErrorDTO(body) &&
        body.errcode === MatrixErrorCode.M_FORBIDDEN
      ) {
        LOG.warn(
          `joinRoom: Passing on error: Could not join to room "${roomId}": ${body?.errcode}: ${body?.error}`,
        );
        throw err;
      } else {
        LOG.warn(
          `joinRoom: Passing on error: Could not join to room "${roomId}": `,
          err,
        );
        throw err;
      }
  ...
  public async sync(options: {
    ...
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}.sync: Client ${this._userId} did not have access token`,
      );
    }

    const {filter, since, full_state, set_presence, timeout} = options;
    ...
    if (filter !== undefined) {
      if (isString(filter)) {
        ...
        throw new TypeError(
          `${this._observer.getName()}.sync: Invalid value for filter option: ${filter}`,
        );
      }
    }
    ...
    if (!isMatrixSyncResponseDTO(response)) {
      LOG.debug(
        'sync: response not MatrixSyncResponseDTO: ',
        JSON.stringify(response, null, 2),
      );
      throw new TypeError(
        `${this._observer.getName()}.sync: Response was not MatrixSyncResponseDTO: ${explainMatrixSyncResponseDTO(response)}`,
      );
    }
  ...
  public async waitForEvents(
    ...
    if (onlyInRooms === undefined) {
      ...
      LOG.debug(
        `Waiting for events ${events.join(' | ')} in rooms ${onlyInRooms.join(', ')}`,
      );
    }
    ...
    return await new Promise((resolve, reject) => {
      try {
        ...
        const onEvent = (
          ...
          if (
            onlyInRooms !== undefined &&
            !(roomId && onlyInRooms.includes(roomId))
          ) {
            LOG.debug(
              'waitForEvents: Event was not in watched room list: ',
              type,
              roomId,
              data,
            );
  ...
  private async _postJson(
    ...
    try {
      LOG.debug(
        `_postJson: Executing POST request ${url} with `,
        body,
        headers,
      );
      const result = await RequestClientImpl.postJson(url, body, headers);
      LOG.debug(
        `_postJson: Response received for POST request ${url} as `,
        result,
      );
      return result;
    } catch (err: unknown) {
      LOG.warn('_postJson: Error: ', err);
      const responseBody = SimpleMatrixClient.getErrorBody(err);
      if (isMatrixErrorDTO(responseBody)) {
        ...
        if (errCode === MatrixErrorCode.M_LIMIT_EXCEEDED) {
          ...
          LOG.warn(
            '_postJson: Limit reached, retrying: ',
            retry_after_ms,
            url,
            body,
            headers,
          ...
          LOG.warn(
            `_postJson: Passing on error code ${errCode}: ${responseBody?.error}`,
          );
        }
      } else {
  ...
  private async _putJson(
    ...
    try {
      ...
      LOG.debug(
        `_putJson: Response received for PUT request ${url} as `,
        result,
      );
      return result;
    } catch (err: unknown) {
      ...
      if (isMatrixErrorDTO(responseBody)) {
        ...
        if (responseBody?.errcode === MatrixErrorCode.M_LIMIT_EXCEEDED) {
          const retry_after_ms = responseBody?.retry_after_ms ?? 1000;
          LOG.warn(
            '_putJson: Limit reached, retrying: ',
            retry_after_ms,
            url,
            body,
            headers,
          ...
          LOG.warn(
            `Passing on: Error with code ${errCode}: ${responseBody.error}`,
          );
        }
      } else {
  ...
  private async _getJson(
    ...
    try {
      ...
      LOG.debug(
        `_getJson: Response received for PUT request ${url} as `,
        result,
      );
      return result;
    } catch (err: unknown) {
      ...
      if (isMatrixErrorDTO(responseBody)) {
        ...
        if (responseBody?.errcode === MatrixErrorCode.M_LIMIT_EXCEEDED) {
          const retry_after_ms = responseBody?.retry_after_ms ?? 1000;
          LOG.error(
            '_getJson: Limit reached, retrying: ',
            retry_after_ms,
            url,
            headers,
          );
          ...
          LOG.warn(
            `_getJson: Passing on: Error with code ${errCode}: ${responseBody?.error}`,
          );
        }
      } else {
  ...
  public static getErrorBody(err: unknown): unknown {
    try {
      if (!isRegularObject(err)) return undefined;
      const getBody = objectKey(err, 'getBody');
      if (typeof getBody === 'function') {
        return getBody();
      }
      return objectKey(err, 'body');
    } catch (err2) {
  ...
  private _normalizeRoomName(name: string) {
    if (!name || !isString(name)) {
      throw new TypeError(
        `${this._observer.getName()}._normalizeRoomName: name is invalid: ${name}`,
      );
    }
  ...
  private _setState(value: SimpleMatrixClientState) {
    LOG.debug(
      '_setState: ',
      value,
      stringifySimpleMatrixClientState(value),
      this._stopSyncOnNext,
    );
    this._state = value;
  }
  ...
  public _startSyncing(triggerEvents: boolean) {
    switch (this._state) {
      ...
      case SimpleMatrixClientState.AUTHENTICATED_AND_SYNCING:
        if (this._stopSyncOnNext) {
          this._stopSyncOnNext = false;
          LOG.debug(
            `_startSyncing: Cancelled previous stop request (state was ${stringifySimpleMatrixClientState(this._state)})`,
          );
        } else {
          LOG.warn(
            `_startSyncing: Warning! Client was already started (was ${stringifySimpleMatrixClientState(this._state)})`,
          );
        }
        return;
      ...
      case SimpleMatrixClientState.AUTHENTICATING:
        throw new TypeError(
          `${this._observer.getName()}._startSyncing: Client was ${stringifySimpleMatrixClientState(this._state)}`,
        );
    }

    if (this.isStopping()) {
      LOG.warn(
        '_startSyncing: Warning! Cancelled previous stop request, although state was AUTHENTICATED.',
      );
      this._stopSyncOnNext = false;
    }
    ...
    this._initSync(triggerEvents).catch((err: unknown) => {
  ...
  public _stopSyncing() {
    switch (this._state) {
      ...
      case SimpleMatrixClientState.AUTHENTICATED:
        LOG.warn(
          `_stopSyncing: Warning! Client was not started (was ${stringifySimpleMatrixClientState(this._state)})`,
        );
        return;
      ...
      case SimpleMatrixClientState.AUTHENTICATED_AND_SYNCING:
        if (!this._stopSyncOnNext) {
          LOG.debug(
            `_stopSyncing: Scheduled stop (state was ${stringifySimpleMatrixClientState(this._state)})`,
          );
          this._stopSyncOnNext = true;
        } else {
          LOG.warn(
            `_stopSyncing: Warning! Stop was already scheduled (state was ${stringifySimpleMatrixClientState(this._state)})`,
          );
        }
        return;
  ...
  private _startSyncAgainTimer() {
    if (this._state !== SimpleMatrixClientState.AUTHENTICATED_AND_STARTED) {
      throw new TypeError(
        `${this._observer.getName()}._startSyncAgainTimer: Client was not AUTHENTICATED_AND_STARTED (was ${stringifySimpleMatrixClientState(this._state)})`,
      );
    }

    this._clearSyncAgainTimer();

    this._syncAgainTimer = setTimeout(
      this._syncAgainTimeoutCallback,
      this._syncAgainTimeMs,
    );
  }
  ...
  private _startInitSyncAgainLater(triggerEvents: boolean) {
    if (this._state !== SimpleMatrixClientState.AUTHENTICATED_AND_RESTARTING) {
      throw new TypeError(
        `${this._observer.getName()}._startInitSyncAgainLater: Client was not AUTHENTICATED_AND_RESTARTING (${stringifySimpleMatrixClientState(this._state)})`,
      );
    }
  ...
  private _onSyncAgainTimeout() {
    ...
    if (this._stopSyncOnNext) {
      this._stopSyncOnNext = false;
      LOG.debug(
        '_onSyncRetryTimeout: Sync cancelled by previous stop request.',
      );
      return;
    }
    ...
    if (this._state !== SimpleMatrixClientState.AUTHENTICATED_AND_STARTED) {
      LOG.error(
        `_onSyncRetryTimeout: Client was not AUTHENTICATED_AND_STARTED (was ${stringifySimpleMatrixClientState(this._state)})`,
      );
    } else {
  ...
  private _onInitSyncAgain(triggerEvents: boolean) {
    ...
    if (this._state !== SimpleMatrixClientState.AUTHENTICATED_AND_RESTARTING) {
      LOG.error(
        `_onInitSyncAgain: Client was not AUTHENTICATED_AND_RESTARTING (${stringifySimpleMatrixClientState(this._state)})`,
      );
      return;
    }
  ...
  private _triggerMatrixEventList(
    events: readonly MatrixSyncResponseAnyEventDTO[],
  ...
  private _triggerMatrixEvent(
    event: MatrixSyncResponseAnyEventDTO,
    room_id: string | undefined,
  ) {
    if (this._observer.hasCallbacks(SimpleMatrixClientEvent.EVENT)) {
      this._observer.triggerEvent(
        SimpleMatrixClientEvent.EVENT,
        room_id ? {...event, room_id} : event,
      );
    } else {
      LOG.warn(
        'Warning! Client received an event but nothing was listening it: ',
        event,
        room_id,
      );
    }
  ...
  private async _syncNextBatch() {
    switch (this._state) {
      ...
      case SimpleMatrixClientState.AUTHENTICATED_AND_SYNCING:
        throw new TypeError(
          `${this._observer.getName()}._syncNextBatch: State was ${stringifySimpleMatrixClientState(this._state)}`,
        );
    }
    ...
    if (!nextBatch)
      throw new TypeError(
        `${this._observer.getName()}._syncNextBatch: No previous nextBatch defined`,
      );
   ...
   * @private
   */
  private async _initSync(triggerEvents: boolean) {
    if (this._state !== SimpleMatrixClientState.AUTHENTICATED) {
      throw new TypeError(
        `${this._observer.getName()}._initSync: Client was not authenticated (${stringifySimpleMatrixClientState(this._state)})`,
      );
    }
    ...
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}._initSync: Client did not have access token`,
      );
    }
    ...
    try {
      ...
      if (this._stopSyncOnNext) {
        ...
        this._setState(SimpleMatrixClientState.AUTHENTICATED);
        LOG.debug(
          '_initSync: Started successfully, but stop was already scheduled.',
        );
      ...
      if (!next_batch) {
        LOG.warn(
          '_initSync: Warning! No next_batch in the response: ',
          response,
        );
  ...
  private async _syncSinceBatch(next: string) {
    ...
    if (!accessToken) {
      throw new TypeError(
        `${this._observer.getName()}._syncSinceBatch: Client did not have access token`,
      );
    }
  ...
  private _triggerSyncEvents(response: MatrixSyncResponseDTO) {
    const nonRoomEvents: readonly MatrixSyncResponseEventDTO[] = concat(
      response?.presence
        ? getEventsFromMatrixSyncResponsePresenceDTO(response?.presence)
        : [],
      response?.account_data
        ? getEventsFromMatrixSyncResponseAccountDataDTO(response?.account_data)
        : [],
      response?.to_device
        ? getEventsFromMatrixSyncResponseToDeviceDTO(response?.to_device)
        : [],
    );
