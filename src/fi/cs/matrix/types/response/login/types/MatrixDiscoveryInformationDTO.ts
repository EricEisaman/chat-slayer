...
import {
  isMatrixHomeServerDTO,
  MatrixHomeServerDTO,
} from './MatrixHomeServerDTO';
...
export function isMatrixDiscoveryInformationDTO(
  value: unknown,
): value is MatrixDiscoveryInformationDTO {
  return (
    isRegularObject(value) &&
    hasNoOtherKeysInDevelopment(value, [
      MatrixType.M_HOMESERVER,
      MatrixType.M_IDENTITY_SERVER,
    ]) &&
    isMatrixHomeServerDTO(value[MatrixType.M_HOMESERVER]) &&
    (isUndefined(value[MatrixType.M_IDENTITY_SERVER]) ||
      isMatrixIdentityServerInformationDTO(value[MatrixType.M_IDENTITY_SERVER]))
  );
}

export function stringifyMatrixDiscoveryInformationDTO(
  value: MatrixDiscoveryInformationDTO,
): string {
  if (!isMatrixDiscoveryInformationDTO(value))
    throw new TypeError(`Not MatrixWellKnownDTO: ${value}`);
  return `MatrixWellKnownDTO(${value})`;
}

export function parseMatrixDiscoveryInformationDTO(
  value: unknown,
): MatrixDiscoveryInformationDTO | undefined {
