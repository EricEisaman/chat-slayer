...
export function isMatrixSyncResponseToDeviceDTO(
  value: unknown,
): value is MatrixSyncResponseToDeviceDTO {
  return (
    isRegularObject(value) &&
    hasNoOtherKeysInDevelopment(value, ['events']) &&
    isArrayOf<MatrixSyncResponseEventDTO>(
...
export function stringifyMatrixSyncResponseToDeviceDTO(
  value: MatrixSyncResponseToDeviceDTO,
): string {
  return `MatrixSyncResponseToDeviceDTO(${value})`;
}

export function parseMatrixSyncResponseToDeviceDTO(
  value: unknown,
): MatrixSyncResponseToDeviceDTO | undefined {
