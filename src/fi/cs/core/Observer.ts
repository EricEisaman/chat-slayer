...
// NOTICE! THIS LIBRARY INTENTIONALLY DOES NOT USE CS LOGGER BECAUSE IT IS USED
 ...
 */
export type ObserverCallbackArray<EventName extends keyof any> = Array<
  ObserverCallback<EventName>
>;
 ...
 */
export type ObserverRecord<EventName extends keyof any> = Record<
  EventName,
  ObserverCallbackArray<EventName>
>;
...
export class Observer<EventName extends keyof any> implements Disposable {
  ...
  public static create<EventName extends keyof any>(
    name: string,
  ): Observer<EventName> {
  ...
  public triggerEvent(eventName: EventName, ...args: Array<any>) {
    if (!this.hasCallbacks(eventName)) {
      console.warn(
        `Warning! The observer for "${this._name}" did not have anything listening "${eventName.toString()}"`,
      );
      return;
    }
    ...
    forEach(callbacks, (callback: ObserverCallback<EventName>) => {
      try {
        callback(eventName, ...args);
      } catch (e) {
        console.error(
          `Observer "${this._name}" and the event handler for "${eventName.toString()}" returned an exception: `,
          e,
        );
      }
    });
  ...
  public listenEvent(
    eventName: EventName,
    callback: ObserverCallback<EventName>,
  ): ObserverDestructor {
    if (!this.hasCallbacks(eventName)) {
  ...
  public removeListener(
    ...
    if (!this.hasCallbacks(eventName)) {
      console.warn(
        `Warning! Could not remove callback since the observer for "${this._name}" did not have anything listening "${eventName.toString()}"`,
      );
      return;
    }
    ...
    this._callbacks[eventName] = filter(
      this._callbacks[eventName],
      (item: ObserverCallback<EventName>) => {
   ...
   */
  public async waitForEvent(eventName: EventName, time: number): Promise<void> {
    await new Promise<void>((resolve, reject) => {
      try {
        ...
        listener = this.listenEvent(eventName, () => {
          try {
            if (timeout) {
