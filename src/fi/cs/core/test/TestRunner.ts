...
export class TestRunner {
  ...
  public static testFileInDir(dir: string, file: string) {
    ...
    if (file.endsWith(TestRunner._testFileEnding)) {
      ...
      const testNames: string[] = Object.keys(test).filter(file =>
        file.endsWith(TestRunner._testMethodEnding),
      );
      ...
      forEach(testNames, (testName: string) => {
        ...
        forEach(testMethodNames, (methodName: string) => {
          ...
          TestRunner._results = [...TestRunner._results, testResult];
          ...
          function testFailed(err: unknown) {
            updateTestResult({
              ...testResult,
  ...
  public static printResults() {
    ...
    if (failedCount >= 1) {
      ...
      forEach(errorResults, (testResult: TestResult) => {
        console.error(
          `[${testResult.file}] ${testResult.className}.${testResult.methodName} failed: `,
          testResult.result,
          '\n',
        );
      });
