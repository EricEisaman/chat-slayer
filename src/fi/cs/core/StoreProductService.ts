...
export class StoreProductService {
  ...
  private static _observer: Observer<StoreProductServiceEvent> =
    new Observer<StoreProductServiceEvent>(SERVICE_NAME);
  ...
  private static async _updateProducts(): Promise<void> {
    this._loading = true;
    try {
      const response: StoreIndexDTO = await StoreClientService.getStoreIndex(
        this._apiUrl,
      );
