...
export class HttpService {
  ...
  private static _observer: Observer<HttpServiceEvent> =
    new Observer<HttpServiceEvent>('HttpService');
  ...
  public static async waitUntilNoOpenRequests(): Promise<void> {
    ...
    return await new Promise((resolve, reject) => {
      try {
        let destructor: any | undefined = this.on(
          HttpServiceEvent.REQUEST_STOPPED,
          () => {
            try {
              if (!this.hasOpenRequests()) {
                LOG.debug(
                  "waitUntilNoOpenRequests: No requests anymore. We're ready!",
                );
                destructor();
                ...
                LOG.debug(
                  `waitUntilNoOpenRequests: We still have ${this.getRequestCount()} requests`,
                );
              }
            } catch (err) {
              LOG.debug(
                'waitUntilNoOpenRequests: Canceling waiting: error: ',
                err,
              );
              reject(err);
            }
  ...
  private static async _request<T>(
    ...
    attempt = attempt ?? 0;
    retryDelay =
      retryDelay ?? retryPolicy?.baseDelay ?? this._defaultRetryDelay;
    if (attempt === 0 && this._requestCount >= this._requestLimit) {
      throw new TypeError(
        `${context}: Too many requests: ${this._requestCount}`,
      );
    }
    try {
      if (attempt === 0) {
        ...
        if (this._observer.hasCallbacks(HttpServiceEvent.REQUEST_STARTED)) {
          this._observer.triggerEvent(
            HttpServiceEvent.REQUEST_STARTED,
            url,
            method,
          );
        }
        LOG.debug(
          `Started ${method} request to "${url} "(${this._requestCount} requests)`,
        );
      } else {
        LOG.debug(
          `Started attempt ${attempt} for ${method} request to "${url} "(${this._requestCount} requests)`,
        );
      }
      ...
      if (retryPolicy) {
        if (shouldRetry(retryPolicy, attempt, method, status, code)) {
          ...
          LOG.debug(
            `Waiting next attempt for ${method} request to "${url} "(${this._requestCount} requests)`,
          );
      ...
      if (attempt === 0) {
        ...
        LOG.debug(
          `Stopped ${method} request to "${url}" (${this._requestCount} requests)`,
        );
      }
    }
  }
    ...
    return this._request(
      ...
      async () => {
        const response: JsonAny | undefined =
          await RequestClientImpl.deleteJson(url, headers);
        return response as ReadonlyJsonAny | undefined;
      },
      retryPolicy,
    );
  ...
  public static async postJsonEntity(
    ...
    return this._request(
      ...
      url,
      async () => {
        return await RequestClientImpl.postJsonEntity(
          url,
          data as JsonAny,
          headers,
        );
