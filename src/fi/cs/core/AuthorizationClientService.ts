...
export function isAuthorizationResultDTO(
  value: unknown,
): value is AuthorizationResultDTO {
  return (
    !!value &&
    isString(objectKey(value, 'email')) &&
    !!objectKey(value, 'email')
  );
}
...
export class AuthorizationClientService {
  ...
  public async verifySessionJwt(
    token: string,
  ): Promise<AuthorizationResultDTO | undefined> {
    try {
      const result = await RequestClientImpl.postJson(
        `${this._serviceUrl}/verify`,
        {
          token,
        },
      );
  ...
  public static async verifySessionAuthorizationHeader(
    ...
    if (!jwt) {
      LOG.debug(
        'verifySessionAuthorizationHeader: Unsupported header value: ',
        header,
      );
    ...
    const result: AuthorizationResultDTO | undefined =
      await authService.verifySessionJwt(jwt);
