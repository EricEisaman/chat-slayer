...
export class BufferedLogWriter implements LogWriter {
  ...
  public constructor(
    ...
    const minimumChunkSize =
      prefix.length + lineBreak.length + suffix.length + 1;
    if (bufferSize < minimumChunkSize)
      throw new TypeError(
        `Chunk size must be greater than ${minimumChunkSize}`,
      );
  }
  ...
  private _drainLoop(forceFlush: boolean): void {
    while (this._drain(forceFlush)) {}
  }

  private _drain(forceFlush: boolean): boolean {
    let drainableString: string;
    if (forceFlush) {
      drainableString = this._data;
      this._data = '';
    } else {
      const lastBreak = this._data.lastIndexOf(this._lineBreak);
    ...
    const origRows: string[] = drainableString.split('\n');

    const rows: string[] = LogUtils.splitStringArray(
      origRows,
      this._chunkSize,
      this._prefix,
      this._suffix,
    ...
    const chunks: string[] = LogUtils.mergeStringArray(
      rows,
      this._chunkSize,
      this._lineBreak,
    );
