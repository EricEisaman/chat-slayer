...
export class DiscordLogger implements Logger {
  ...
  private _sendMessageAsStringSync(content: string): void {
    this._sendMessageAsString(`[${this._name}] ${content}`).catch(
      (err: unknown) => {
        console.warn(
          `Warning! Error "${err}" while sending the log message: "${content}"`,
        );
  ...
  private async _sendMessageAsString(content: string): Promise<void> {
    const body = {
      content,
      options: {},
    };
    await HttpService.postJson(this._url, body, undefined, this._retryPolicy);
  }
}
