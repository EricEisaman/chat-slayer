...
describe('HostsService', () => {
  ...
  describe('#toString', () => {
    it('can stringify model with single entry', () => {
      expect(service.toString()).toBe('127.0.0.1\tlocalhost\n');
    });

    it('can stringify model with multiple entries', () => {
      service.setAddress('10.0.0.1', 'something.local');
    ...
    it('can stringify model with multiple entries and more hostnames', () => {
      service.setAddress('10.0.0.1', 'something.local');
      service.setAddress('10.0.0.5', ['fs.local', 'fs.test', 'fs.dev']);
      expect(service.toString()).toBe(
        '127.0.0.1\tlocalhost\n' +
          '10.0.0.1\tsomething.local\n' +
  ...
  describe('#setAddress', () => {
    ...
    it('can replace model entry', () => {
      service.setAddress('127.0.0.1', ['localhost', 'something.local']);

      expect(service.getModel()).toStrictEqual({
        entries: [
          {
            address: '127.0.0.1',
            hostnames: ['localhost', 'something.local'],
          },
        ],
      });
    });
  ...
  describe('#replaceByHostnames', () => {
    it('can set a new model entry for new address', () => {
      service.replaceByHostnames('10.0.0.1', 'something.local');
    ...
    it('can replace model entry for existing address', () => {
      service.replaceByHostnames('127.0.0.1', ['localhost', 'something.local']);
    ...
    it('can add new hostname to existing address', () => {
      service.replaceByHostnames('127.0.0.1', 'something.local');
