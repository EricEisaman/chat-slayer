...
export class HostsServiceImpl implements HostsService {
  ...
  protected constructor(model: HostsModel) {
    this._model = model;
    this._observer = new Observer<HostsServiceEvent>('HostsServiceImpl');
  }

  public static create(model: HostsModel): HostsServiceImpl {
  ...
  public setAddress(
    ...
    this._setModel(
      HostsServiceImpl._setAddress(this._model, address, hostnames),
    );
  }
  ...
  public replaceByHostnames(
    ...
    forEach(hostnames, (hostname: string) => {
  ...
  public toString(): string {
    return (
      map(
        this._model.entries,
        (item: HostEntryModel): string =>
          `${item.address}\t${item.hostnames.join(' ')}`,
      ).join('\n') + '\n'
    );
  }
  ...
  public static _replaceByHostname(
    ...
    const oldEntryByHostname: HostEntryModel | undefined =
      HostsServiceImpl._getEntryByHostname(model, hostname);
    const oldEntryByAddress: HostEntryModel | undefined =
      HostsServiceImpl._getEntryByAddress(model, address);

    if (oldEntryByHostname !== undefined) {
      const newHostnames = filter(
        oldEntryByHostname.hostnames,
        (item: string): boolean => item !== hostname,
      );
      if (newHostnames.length) {
        model = HostsServiceImpl._setAddress(
          model,
          oldEntryByHostname.address,
          newHostnames,
        );
      } else {
        model = HostsServiceImpl._removeAddress(
          model,
          oldEntryByHostname.address,
        );
      }
    ...
    if (oldEntryByAddress !== undefined) {
      model = HostsServiceImpl._setAddress(
        model,
        address,
        uniq(concat([], oldEntryByAddress.hostnames, [hostname])),
      );
    } else {
  ...
  private _setModel(newHostsModel: HostsModel): void {
  ...
  private static _getEntryByAddress(
    model: HostsModel,
    address: string,
  ...
  private static _getEntryByHostname(
    model: HostsModel,
    hostname: string,
  ): HostEntryModel | undefined {
    return find(model.entries, (entry: HostEntryModel): boolean =>
      some(entry.hostnames, (item: string): boolean => item === hostname),
    );
  }
}
