...
export class EnumUtils {
  public static getKeyValuePairs<T>(
    ...
    return map(
      ...
      (key: string): EnumKeyValuePair<T> => [key, obj[key]],
    );
  }

  public static getValues<T>(obj: EnumObject<T>): readonly T[] {
    return map(EnumUtils.getKeys<T>(obj), (key: string): T => obj[key]);
  }

  public static getKeys<T>(obj: EnumObject<T>): readonly string[] {
    return filter(Object.keys(obj), (k: string): boolean =>
      k ? '0123456789'.indexOf(k[0]) < 0 : true,
    );
  }
