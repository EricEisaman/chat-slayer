...
const CAPITAL_INCOME_TAX_PERCENT_BELOW_RELIEF = 0.3;
const CAPITAL_INCOME_TAX_PERCENT_ABOVE_RELIEF = 0.34;
const TAX_BASIS_AFTER_10_YEARS = 0.4;
const TAX_BASIS_STANDARD = 0.2;
...
export class FiStockTradeTaxCalculator {
  public static calculate(
    ...
    const hankintaHintaOlettamaOsuus = yli10Years
      ? TAX_BASIS_AFTER_10_YEARS
      : TAX_BASIS_STANDARD;
    const hankintaHintaOlettama = kauppaSummaValue * hankintaHintaOlettamaOsuus;
    const hankintaKulut = ostinOsakkeet
      ? hankintaHintaValue + hankintaVarainsiirtoVeroValue + valitysPalkkioValue
      : hankintaHintaValue;
    const veroVahennys =
      hankintaHintaOlettama > hankintaKulut
        ? hankintaHintaOlettama
        : hankintaKulut;
    const verotettavaSumma = isTaxFree
      ? 0
      : kauppaSummaValue >= veroVahennys
        ? kauppaSummaValue - veroVahennys
        : 0;
  ...
  public static reverseCalculate(
    ...
    const minBruttoRange: number = kauppaSummaNetto;
    const maxBruttoRange: number =
      kauppaSummaNetto / (1 - CAPITAL_INCOME_TAX_PERCENT_ABOVE_RELIEF);
    ...
    for (; current <= maxBruttoRange; current += 0.01) {
      ...
      if (
        Math.round(result.kauppaSummaNetto * 100) ===
        Math.round(kauppaSummaNetto * 100)
      ) {
