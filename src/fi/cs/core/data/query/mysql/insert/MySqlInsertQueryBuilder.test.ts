...
export const mockQueryBuilderFactory = (): QueryBuilder => {
  const me: QueryBuilder = {
    ...
    build: jest
      .fn<any>()
      .mockImplementation(() => [me.buildQueryString(), me.buildQueryValues()]),
    buildQueryString: jest.fn<any>().mockReturnValue('query'),
    buildQueryValues: jest
      .fn<any>()
      .mockImplementation(() =>
        map(me.getQueryValueFactories(), item => item()),
      ),
    getQueryValueFactories: jest
      .fn<any>()
      .mockReturnValue([() => 'value of query']),
  };
  return me;
};
...
describe('MySqlInsertQueryBuilder', () => {
  ...
  beforeAll(() => {
    ...
    (dateOnlyListBuilder.getQueryValueFactories as any).mockReturnValue([
      () => 'value of date query',
    ]);

    nameAndDateListBuilder = mockQueryBuilderFactory();
    (nameAndDateListBuilder.buildQueryString as any).mockReturnValue(
      'name query, date query',
    );
    (nameAndDateListBuilder.getQueryValueFactories as any).mockReturnValue([
      () => 'value of name query',
      () => 'value of date query',
    ]);
  });
  ...
  describe('#build', () => {
    ...
    it('can build insert query for one row with name and date columns', () => {
      ...
      expect(queryString).toBe(
        'INSERT INTO ?? (??, ??) VALUES (name query, date query)',
      );
    ...
    it('can build insert query for two rows with name and date columns', () => {
      ...
      nameAndDateListBuilder = mockQueryBuilderFactory();
      (nameAndDateListBuilder.buildQueryString as any).mockReturnValue(
        'name query 2, date query 2',
      );
      (nameAndDateListBuilder.getQueryValueFactories as any).mockReturnValue([
        () => 'value of name query 2',
        () => 'value of date query 2',
      ]);
      ...
      const [queryString, values] = builder.build();
      expect(queryString).toBe(
        'INSERT INTO ?? (??, ??) VALUES (name query, date query), (name query 2, date query 2)',
      );
    ...
    it('can build insert query from an object', () => {
      ...
      builder.addColumnName(nameColumn);
      builder.addColumnName(ageColumn);
      builder.appendValueObject(['car_name', 'car_age'], {
        car_name: 'hello',
        car_age: 13,
      });
      builder.appendValueObject(['car_name', 'car_age'], {
