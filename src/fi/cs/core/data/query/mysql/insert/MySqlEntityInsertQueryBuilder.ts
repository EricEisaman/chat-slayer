...
export class MySqlEntityInsertQueryBuilder implements EntityInsertQueryBuilder {
  ...
  public appendEntity<T extends Entity>(
    ...
    const filteredFields: EntityField[] = filter(
      ...
      (field: EntityField): boolean => {
        ...
        return (
          insertable &&
          !ignoreProperties.includes(propertyName) &&
          fieldType !== EntityFieldType.JOINED_ENTITY
        );
      },
    );

    const itemBuilder = MySqlListQueryBuilder.create();
    forEach(filteredFields, (field: EntityField) => {
      ...
      const value: any =
        EntityUtils.getPropertyFromEntity(entity, propertyName) ?? null;
  ...
  public appendEntityList<T extends Entity>(
    ...
    forEach(list, item =>
      this.appendEntity(item, fields, temporalProperties, ignoreProperties),
    );
  }
