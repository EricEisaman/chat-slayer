...
import {
  ChainQueryBuilder,
  ChainQueryBuilderFactory,
} from '../types/ChainQueryBuilder';
...
export class ChainQueryBuilderUtils {
  ...
  public static buildChain(
    ...
    forEach(where.getConditions(), (item: Condition) => {
      ...
      if (isPropertyNameTarget(target)) {
        ...
        if (!field)
          throw new TypeError(
            `Could not find field info for property "${propertyName}" from table "${completeTableName}"`,
          );
        const columnName = field?.columnName;
        if (!columnName)
          throw new TypeError(
            `Could not find column name for property "${propertyName}" from table "${completeTableName}"`,
          );

        const {columnDefinition} = field;

        const temporalProperty = find(
          temporalProperties,
        ...
        const isJson: boolean = isTime
          ? false
          : isJsonColumnDefinition(columnDefinition);
        ...
        if (isEqualCondition(item)) {
          ...
          if (isTime) {
            builder.setColumnBetweenAsTime(
              completeTableName,
              columnName,
              start,
              end,
