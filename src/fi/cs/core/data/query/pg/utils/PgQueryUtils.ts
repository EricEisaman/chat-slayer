...
const qtc = (table: string, column: string) =>
  PgQueryUtils.quoteTableAndColumn(table, column);

export const PG_PH_AS = (expression: string, columnName: string) =>
  `${assertExists(
    expression,
    ...
    assertExists(
      columnName,
      `Column name required for expression: "${expression}"`,
    ),
  )}`;
...
export const PG_AS_COLUMN_NAME = (value: string, asColumnName: string) =>
  `${value} AS ${qc(asColumnName)}`;
...
export const PG_ESCAPE_TABLE_OR_COLUMN = (value: string): string => {
  const doubleQuote = '"';
  const escapedIdentifier = value
    .split(doubleQuote)
    .join(doubleQuote + doubleQuote);
...
export class PgQueryUtils {
  ...
  public static quoteTableAndColumn(
    tableName: string,
    columnName: string,
  ): string {
  ...
  public static quoteTableAndColumnAsJsonB(
    tableName: string,
    columnName: string,
  ): string {
  ...
  public static quoteTableAndColumnAsText(
    tableName: string,
    columnName: string,
  ): string {
  ...
  public static quoteTableAndColumnAsTimestampString(
    tableName: string,
    columnName: string,
  ): string {
    return PG_TO_CHAR_TIMESTAMP(
      PgQueryUtils.quoteTableAndColumn(tableName, columnName),
    );
  }

  public static quoteTableAndColumnAsColumnName(
    tableName: string,
    columnName: string,
    asColumnName: string,
  ): string {
    return PG_AS_COLUMN_NAME(
      PgQueryUtils.quoteTableAndColumnAsText(tableName, columnName),
      asColumnName,
    );
  }

  public static quoteTableAndColumnAsTextAsColumnName(
    tableName: string,
    columnName: string,
    asColumnName: string,
  ): string {
    return PG_AS_COLUMN_NAME(
      PG_TO_TEXT(PgQueryUtils.quoteTableAndColumn(tableName, columnName)),
      asColumnName,
    );
  }

  public static quoteTableAndColumnAsTimestampStringAsColumnName(
    tableName: string,
    columnName: string,
    asColumnName: string,
  ): string {
    return PG_AS_COLUMN_NAME(
      PG_TO_CHAR_TIMESTAMP(
        PgQueryUtils.quoteTableAndColumn(tableName, columnName),
      ),
      asColumnName,
    );
