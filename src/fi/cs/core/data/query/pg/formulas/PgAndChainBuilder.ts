...
export class PgAndChainBuilder implements ChainQueryBuilder {
  ...
  public setColumnInList(
    ...
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} IN (${builder.buildQueryString()})`,
    );
    forEach(builder.getQueryValueFactories(), f => this._formulaValues.push(f));
  }

  public setColumnEquals(tableName: string, columnName: string, value: any) {
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} = ${PgQueryUtils.getValuePlaceholder()}`,
    );
  ...
  public setColumnEqualsAsJson(
    ...
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumnAsJsonB(tableName, columnName)} = ${PgQueryUtils.getValuePlaceholderAsJsonB()}`,
    );
    this._formulaValues.push(() => value);
  }

  public setColumnIsNull(tableName: string, columnName: string): void {
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} IS NULL`,
    );
  }
  ...
  public setColumnBetween(
    ...
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} BETWEEN ${PgQueryUtils.getValuePlaceholder()} AND ${PgQueryUtils.getValuePlaceholder()}`,
    );
    this._formulaValues.push(() => start);
    this._formulaValues.push(() => end);
  }

  public setColumnBefore(tableName: string, columnName: string, value: any) {
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} < ${PgQueryUtils.getValuePlaceholder()}`,
    );
  ...
  public setColumnAfter(tableName: string, columnName: string, value: any) {
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} > ${PgQueryUtils.getValuePlaceholder()}`,
    );
  ...
  public setColumnEqualsAsTime(
    ...
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} = ${PgQueryUtils.getValuePlaceholderAsTimestamp()}`,
    );
    this._formulaValues.push(() => value);
  }
  ...
  public setColumnBetweenAsTime(
    ...
    this._formulaQuery.push(
      () =>
        `${PgQueryUtils.quoteTableAndColumn(tableName, columnName)} BETWEEN ${PgQueryUtils.getValuePlaceholderAsTimestamp()} AND ${PgQueryUtils.getValuePlaceholderAsTimestamp()}`,
    );
    this._formulaValues.push(() => start);
    this._formulaValues.push(() => end);
  }
  ...
  public setFromQueryBuilder(builder: QueryBuilder) {
  ...
  public toString(): string {
    return `PgAndChainBuilder "${this.buildQueryString()}" with ${this.buildQueryValues()
      .map(item => item())
      .join(' ')}`;
  }
