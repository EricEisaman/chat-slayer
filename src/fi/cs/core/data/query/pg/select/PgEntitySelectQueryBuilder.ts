...
import {
  ...
  TableFieldInfoCallback,
} from '../../sql/select/EntitySelectQueryBuilder';
...
export class PgEntitySelectQueryBuilder implements EntitySelectQueryBuilder {
  ...
  public setOneToManyRelations(
    ...
    forEach(relations, (oneToManyRelation: EntityRelationOneToMany): void => {
      const {propertyName} = oneToManyRelation;
      const mappedTable = oneToManyRelation?.mappedTable;
      if (!mappedTable)
        throw new TypeError(
          `The relation "${propertyName}" did not have table defined`,
        );

      const [mappedFields, mappedTemporalProperties] =
        resolveMappedFieldInfo(mappedTable);

      const groupByColumn = this.getGroupByColumn();
      if (!groupByColumn)
        throw new TypeError(
          `Could not find column to group by for property "${propertyName}"`,
        );
  ...
  public setManyToOneRelations(
    ...
    forEach(relations, (relation: EntityRelationManyToOne): void => {
      ...
      const mappedField: EntityField | undefined = find(
        fields,
        field =>
          field?.propertyName === propertyName &&
          field?.fieldType === EntityFieldType.JOINED_ENTITY,
      );
      if (!mappedField)
        throw new TypeError(
          `Could not find field definition for property "${propertyName}"`,
        );
      const mappedColumnName: string = mappedField.columnName;
      if (!mappedColumnName)
        throw new TypeError(
          `Could not find column name for property "${propertyName}"`,
        );
      this.setManyToOne(
        propertyName,
        mappedFields,
  ...
  public appendResultExpression(
    ...
    this._builder.appendResultExpression(queryFactory, ...valueFactories);
  }
  ...
  public appendOrderExpression(
    ...
    return this._builder.appendOrderExpression(queryFactory, ...valueFactories);
  }
  ...
  public appendLeftJoinExpression(
    ...
    this._builder.appendLeftJoinExpression(queryFactory, ...valueFactories);
  }
  ...
  public leftJoinTable(
    ...
    return this._builder.leftJoinTable(
      fromTableName,
      fromColumnName,
      sourceTableName,
      sourceColumnName,
  ...
  public toString(): string {
    return `PgEntitySelectQueryBuilder "${this.buildQueryString()}" with ${this.buildQueryValues()
      .map(item => item())
      .join(' ')}`;
  }
