...
export class FunctionQueryBuilder implements QueryBuilder {
  ...
  public toString(): string {
    return `PgFunctionBuilder "${this.buildQueryString()}" with ${this.buildQueryValues()
      .map(item => item())
      .join(' ')}`;
  }
  ...
  public buildQueryString(): string {
    if (!this._builder)
      throw new TypeError(
        `Could not build ${this._name}() query string: Query builder not initialized`,
      );
  ...
  public buildQueryValues(): readonly any[] {
    if (!this._builder)
      throw new TypeError(
        `Could not build ${this._name}() query values: Query builder not initialized`,
      );
  ...
  public getQueryValueFactories(): readonly QueryValueFactory[] {
    if (!this._builder)
      throw new TypeError(
        `Could not build ${this._name}() query factories: Query builder not initialized`,
      );
