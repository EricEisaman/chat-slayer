...
export abstract class BaseUpdateQueryBuilder implements UpdateQueryBuilder {
  ...
  public getTableName(): string {
    if (!this._tableName)
      throw new TypeError(
        'The table name where to update entities has not been configured in the query builder',
      );
  ...
  public getCompleteTableName(): string {
    if (!this._tableName)
      throw new TypeError(
        'The table where to update rows has not been initialized yet',
      );
  ...
  public buildQueryString(): string {
    ...
    if (!prefixes.length)
      throw new TypeError(
        'No prefix factories detected for update query builder! This must be an error.',
      );
    const setQuery = map(this._setQueries, f => f());
    if (!setQuery.length)
      throw new TypeError(
        'No value placeholders detected for update query builder! This must be an error.',
      );
    const where = this._where ? this._where.buildQueryString() : '';
    return `${prefixes.join(' ')} SET ${setQuery.join(', ')}${
      where ? ` WHERE ${where}` : ''
    }`;
  }
