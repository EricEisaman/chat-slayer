...
export abstract class BaseInsertQueryBuilder implements InsertQueryBuilder {
   ...
   * @protected
   */
  protected constructor(columnNameSeparator: string) {
  ...
  public addColumnFactory(
    ...
    forEach(valueFactories, factory => {
      this._columnNameValues.push(factory);
    });
  }

  public abstract addColumnName(name: string): void;
  ...
  public getTableName(): string {
    if (!this._intoTableName)
      throw new TypeError(
        'The table name where to insert entities has not been configured in the query builder',
      );
  ...
  public getFullTableName(): string {
    if (!this._intoTableName)
      throw new TypeError(
        'The table where to insert rows has not been initialized yet',
      );
  ...
  public buildQueryString(): string {
    ...
    if (!prefixes.length)
      throw new TypeError(
        'No prefix factories detected for insert query builder! This must be an error.',
      );
    const columnNameValues = map(this._columnNameQueries, f => f());
    if (!columnNameValues.length)
      throw new TypeError(
        'No value placeholders detected for insert query builder! This must be an error.',
      );
