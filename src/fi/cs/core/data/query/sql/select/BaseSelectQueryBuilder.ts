...
export abstract class BaseSelectQueryBuilder implements SelectQueryBuilder {
   ...
   */
  abstract includeColumnAsTimestamp(
    tableName: string,
    columnName: string,
    asColumnName: string,
  ): void;
   ...
   */
  abstract includeColumnFromQueryBuilder(
    builder: QueryBuilder,
    asColumnName: string,
  ): void;
  ...
  public getTableName(): string {
    if (!this._mainTableName)
      throw new TypeError('From table has not been initialized yet');
...
export function isBaseSelectQueryBuilder(
  value: unknown,
): value is BaseSelectQueryBuilder {
