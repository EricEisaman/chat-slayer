...
export class PersisterMetadataManagerImpl implements PersisterMetadataManager {
  ...
  public getMetadataByTable(tableName: string): EntityMetadata | undefined {
    return has(this._metadata, tableName)
      ? this._metadata[tableName]
      : undefined;
  }
  ...
  private _updateRelations(): void {
    ...
    LOG.debug(
      `_updateRelations: Updating metadata for all configured tables (${tables.length}`,
    );
    forEach(tables, (metadata: EntityMetadata): void => {
      ...
      forEach(manyToOneRelations, (manyToOne: EntityRelationManyToOne) => {
        ...
        const manyToOneField: EntityField | undefined = find(
          fields,
          (item: EntityField): boolean =>
            item.propertyName === manyToOnePropertyName,
        );
        if (!manyToOneField) {
          LOG.warn(
            `Warning! Property "${manyToOnePropertyName}" had @ManyToOne annotation but no field configuration found.`,
          );
          return;
        }
        if (manyToOneField.fieldType !== EntityFieldType.JOINED_ENTITY) {
          LOG.warn(
            `Warning! Property "${manyToOnePropertyName}" had @ManyToOne annotation but was not joined column type (it was "${manyToOneField.fieldType}").`,
          );
          return;
        }
        ...
        if (!oneToManyTableName) {
          LOG.debug(
            `Property "${manyToOnePropertyName}" had @ManyToOne annotation but we could not find remote table name. This might be because the remote entity has not been setup yet.`,
          );
          return;
        }
        ...
        if (!oneToManyMetadata) {
          LOG.debug(
            `Property "${manyToOnePropertyName}" had @ManyToOne annotation but we could not find metadata for remote table "${oneToManyTableName}". This might be because the remote entity has not been setup yet.`,
          );
          return;
        }
        ...
        if (!manyToOneField.metadata) {
          LOG.debug(
            `Updated metadata for property "${manyToOne.propertyName}" as `,
            oneToManyMetadata,
          );
          manyToOneField.metadata = oneToManyMetadata;
        } else {
          LOG.debug(
            `Metadata was already defined for "${manyToOne.propertyName}" as `,
            manyToOneField.metadata,
          );
        }
      });
    });
