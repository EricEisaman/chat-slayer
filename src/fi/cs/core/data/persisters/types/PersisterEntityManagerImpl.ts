...
export class PersisterEntityManagerImpl implements PersisterEntityManager {
   ...
   * @inheritDoc
   */
  public saveLastEntityListState<T extends EntityLike>(
    list: readonly T[],
  ): void {
    forEach(list, (item: T): void => this.saveLastEntityState<T>(item));
  }
  ...
  public getChangedFields<T extends EntityLike>(
    ...
    if (prevState === undefined) {
      ...
      return filter(fields, (field: EntityField): boolean => {
    ...
    return filter(fields, (field: EntityField): boolean => {
      ...
      const prevValue: any = has(prevState, propertyName)
        ? (prevState as any)[propertyName]
        : undefined;
      const nextValue: any = has(item, propertyName)
