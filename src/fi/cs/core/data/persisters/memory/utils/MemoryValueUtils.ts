...
import {
  isPropertyNameTarget,
  PropertyNameTarget,
} from '../../../conditions/types/PropertyNameTarget';
import {
  BetweenCondition,
  isBetweenCondition,
} from '../../../conditions/BetweenCondition';
import {
  EqualCondition,
  isEqualCondition,
} from '../../../conditions/EqualCondition';
...
import {
  BeforeCondition,
  isBeforeCondition,
} from '../../../conditions/BeforeCondition';
import {
  AfterCondition,
  isAfterCondition,
} from '../../../conditions/AfterCondition';
...
export class MemoryValueUtils {
      ...
      (
        ...
        if (isWhereConditionTarget(target)) {
          ...
          if (isOrCondition(item))
            return this.buildMatcherFunctionFromWhereUsingOr(item.getWhere());
          if (isAndCondition(item))
            return this.buildMatcherFunctionFromWhereUsingAnd(item.getWhere());
          //LOG.debug(`reduce and: item = `, item);
          throw new TypeError(
            `Unsupported condition target for where clause: ${item}`,
          );
        }
        if (!isPropertyNameTarget(target)) {
          if (isOrCondition(item))
            throw new TypeError(
              `Unsupported condition target for or conditions: ${target}`,
            );
          if (isAndCondition(item))
            throw new TypeError(
              `Unsupported condition target for and conditions: ${target}`,
            );
          //LOG.debug(`reduce and: target = `, target);
    ...
    return reduce(
      ...
      (
        ...
        if (!isPropertyNameTarget(target)) {
          ...
          throw new TypeError(
            `Unsupported condition target or item: ${target} / ${item}`,
          );
        }
        if (isBetweenCondition(item)) {
  ...
  public static buildBetweenRangeConditionAndTest<T = any>(
    ...
    const stagingTest = this.buildRangeTest<T>(
      propertyName,
      rangeStart,
      rangeEnd,
    );
  ...
  public static equalTest<T = any>(value: T, testValue: T): boolean {
    return isEqual(value, testValue);
  }

  public static beforeTest<T = any>(value: T, testValue: T): boolean {
    return value < testValue;
  }

  public static afterTest<T = any>(value: T, testValue: T): boolean {
    return value > testValue;
  }
}
