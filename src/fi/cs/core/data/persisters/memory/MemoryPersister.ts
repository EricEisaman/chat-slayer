...
export class MemoryPersister implements Persister {
   ...
   */
  constructor(idType?: MemoryIdType) {
    this._data = {};
  ...
  private async _deleteAll<T extends Entity>(
    ...
    const hasPreRemoveCallbacks = EntityCallbackUtils.hasCallbacks(
      callbacks,
      EntityCallbackType.PRE_REMOVE,
    );
    const hasPostRemoveCallbacks = EntityCallbackUtils.hasCallbacks(
      callbacks,
    ...
    if (hasPreRemoveCallbacks || hasPostRemoveCallbacks) {
      ...
      if (hasPreRemoveCallbacks && entities?.length) {
        await EntityCallbackUtils.runPreRemoveCallbacks(entities, callbacks);
      }
    }
    ...
    if (!hasPreRemoveCallbacks && !hasPostRemoveCallbacks) {
      ...
      if (hasPostRemoveCallbacks) {
        await EntityCallbackUtils.runPostRemoveCallbacks(entities, callbacks);
      }
    }
  }
  ...
  private async _findAll<T extends Entity>(
    ...
    const matchedItems =
      matcher !== undefined
        ? filter(allItems, (item: MemoryItem): boolean => matcher(item.value))
        : allItems;
    const items: T[] = this._prepareItemList(
      matchedItems,
      metadata,
      true,
      sort,
    ...
    if (ret?.length) {
      await EntityCallbackUtils.runPostLoadCallbacks(ret, callbacks);
    }
  ...
  private async _findBy<T extends Entity>(
    ...
    if (entity) {
      await EntityCallbackUtils.runPostLoadCallbacks([entity], callbacks);
    }
    return entity;
  }
  ...
  private async _insert<T extends Entity>(
    ...
    await EntityCallbackUtils.runPrePersistCallbacks(list, callbacks);
    ...
    const newItems: MemoryItem[] = map(list, (item: T): MemoryItem => {
      // Prepare the ID field
      if (!(has(item, idPropertyName) && (item as any)[idPropertyName])) {
        const newId: number = ++ID_SEQUENCER;
        (item as any)[idPropertyName] =
          this._idType === MemoryIdType.STRING ? `${newId}` : newId;
      }
      ...
      forEach(creationTimestamps, (propertyName): void => {
        (item as any)[propertyName] = now;
      });

      // Prepare the update timestamp
      forEach(updateTimestamps, (propertyName): void => {
        (item as any)[propertyName] = now;
      });
    ...
    const addedEntity = this._populateRelations(
      db,
      this._prepareItem<T>(firstItem, metadata, true),
      metadata,
    );
    this._entityManager.saveLastEntityState<T>(addedEntity);

    await EntityCallbackUtils.runPostLoadCallbacks([addedEntity], callbacks);

    await EntityCallbackUtils.runPostPersistCallbacks([addedEntity], callbacks);
  ...
  private async _update<T extends Entity>(
    ...
    const {tableName, fields, idPropertyName, callbacks, updateTimestamps} =
      metadata;

    const idField = find(fields, item => item.propertyName === idPropertyName);
    if (!idField)
      throw new TypeError(
        `Could not find id field using property "${idPropertyName}"`,
      );
    const entityId = has(entity, idPropertyName)
      ? (entity as any)[idPropertyName]
      : undefined;
    if (!entityId)
      throw new TypeError(
        `Could not find entity id column using property "${idPropertyName}"`,
      );

    const updateFields = this._entityManager.getChangedFields(entity, fields);

    if (updateFields.length === 0) {
      LOG.debug(
        'Entity did not any updatable properties changed. Saved nothing.',
      );
      ...
      if (!item) {
        throw new TypeError(
          `Entity was not stored in this persister for ID: ${entityId}`,
        );
      }

      await EntityCallbackUtils.runPostUpdateCallbacks([item], callbacks);
    ...
    await EntityCallbackUtils.runPreUpdateCallbacks([entity], callbacks);
    ...
    if (savedItem) {
      ...
      forEach(updateFields, (field: EntityField): void => {
    ...
    await EntityCallbackUtils.runPostUpdateCallbacks([savedEntity], callbacks);
  ...
  private _count(
    ...
    const matcher =
      where !== undefined
        ...
        : undefined;
    if (matcher) {
      return filter(db[tableName].items, (item: MemoryItem): boolean =>
        matcher(item.value),
      ).length;
    }
  ...
  private _existsBy(
    ...
    const matcher =
      MemoryValueUtils.buildMatcherFunctionFromWhereUsingAnd(where);
    return some(db[tableName].items, (item: MemoryItem): boolean =>
      matcher(item.value),
    );
  }
  ...
  private _prepareItemList<T extends Entity>(
    ...
    const list = map(
      items,
      (item: MemoryItem): T => this._prepareItem(item, metadata, simplify),
    );
    if (sortFunction) {
  ...
  private _populateRelationsToList<T extends Entity>(
    ...
    return map(list, item => this._populateRelations(db, item, metadata));
  }
  ...
  private _populateOneToManyRelations<T extends Entity>(
    ...
    const entityId: string | number | undefined = has(entity, idPropertyName)
      ? ((entity as any)[idPropertyName] as string | number)
      : undefined;
    LOG.debug(
      '_populateOneToManyRelations: 0. entityId = ',
      entityId,
      entity,
      idPropertyName,
      tableName,
    ...
    if (oneToManyRelations?.length) {
      forEach(oneToManyRelations, (oneToMany: EntityRelationOneToMany) => {
        const {propertyName, mappedBy, mappedTable} = oneToMany;
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": 1. oneToMany = `,
          mappedBy,
          mappedTable,
        );
        ...
        if (!(mappedTable && mappedBy)) {
          throw new TypeError(
            `No link to table exists to populate property "${propertyName}" in table "${tableName}"`,
          );
        }

        const manyToOneMetadata =
          this._metadataManager.getMetadataByTable(mappedTable);
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": 2. mappedToMetadata = `,
          manyToOneMetadata,
        );
        if (!manyToOneMetadata) {
          throw new TypeError(
            `Could not find metadata for linked table "${mappedTable} to populate property "${propertyName}" in table "${tableName}"`,
          );
        }

        const manyToOneFieldInfo: EntityField | undefined = find(
          manyToOneMetadata.fields,
          (field: EntityField): boolean => field.propertyName === mappedBy,
        );
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": 3. joinColumn = `,
          manyToOneFieldInfo,
        );
        if (!manyToOneFieldInfo) {
          LOG.debug(
            `_populateOneToManyRelations: "${propertyName}": No field definition found for matching ManyToOne relation`,
          );
          return;
        }
        ...
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": 4. joinColumnName = `,
          joinColumnName,
          metadata.fields,
        );
        const joinPropertyName = EntityUtils.getPropertyName(
          joinColumnName,
          manyToOneMetadata.fields,
        );
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": 5. joinPropertyName = `,
          joinPropertyName,
        );

        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": 6. Searching related items for column name "${joinColumnName}" and inner property "${joinPropertyName}" mapped to table "${mappedTable}" by id "${entityId}"`,
        );
        const linkedEntities: MemoryItem[] = this._filterItems(
          ...
          (relatedItem: MemoryItem): boolean => {
            ...
            LOG.debug(
              `_populateOneToManyRelations: "${propertyName}": 7. relatedEntity = `,
              relatedEntity,
            );
            LOG.debug(
              `_populateOneToManyRelations: "${propertyName}": 7. joinPropertyName = `,
              joinPropertyName,
            );
            const innerId: string | number | undefined = has(
              relatedEntity,
              joinPropertyName,
            )
              ? (relatedEntity as any)[joinPropertyName]
              : undefined;
            LOG.debug(
              `_populateOneToManyRelations: "${propertyName}": 10. innerId = `,
              innerId,
            );
            LOG.debug(
              `_populateOneToManyRelations: "${propertyName}": 10. entityId = `,
              entityId,
            );
        ...
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": linkedEntities = `,
          linkedEntities,
        );
        ...
        LOG.debug(
          `_populateOneToManyRelations: "${propertyName}": PREPARED: linkedEntities = `,
          linkedEntities,
        );
  ...
  private _populateManyToOneRelations<T extends Entity>(
    ...
    LOG.debug(
      'ManyToOneRelations: 0. tableName = ',
      tableName,
      manyToOneRelations,
    );

    if (manyToOneRelations?.length) {
      forEach(manyToOneRelations, (manyToOne: EntityRelationManyToOne) => {
        const {propertyName, mappedTable} = manyToOne;
        LOG.debug(
          'ManyToOneRelations: 1. propertyName = ',
          propertyName,
          mappedTable,
        );

        const joinColumn: EntityField | undefined = find(
          metadata.fields,
          (field: EntityField): boolean => field.propertyName === propertyName,
        );
        LOG.debug('ManyToOneRelations: 2. joinColumn = ', joinColumn);
        if (joinColumn) {
          const joinColumnName = joinColumn.columnName;
          LOG.debug(
            'ManyToOneRelations: 3. joinColumnName = ',
            joinColumnName,
            metadata.fields,
          );
          ...
          const mappedToMetadata =
            this._metadataManager.getMetadataByTable(mappedTable);
          LOG.debug(
            'ManyToOneRelations: 4. mappedToMetadata = ',
            mappedToMetadata,
          );
          if (!mappedToMetadata) {
            throw new TypeError(
          ...
          LOG.debug(
            'ManyToOneRelations: 5. joinPropertyName = ',
            joinPropertyName,
          );
          LOG.debug('ManyToOneRelations: 5. entity = ', entity);

          const mappedId: string = has(entity, joinPropertyName)
            ? (entity as any)[joinPropertyName]
            : undefined;
          if (!mappedId)
            throw new TypeError(
              `Could not find related entity id ("${joinPropertyName}" from "${joinColumnName}") by property "${propertyName}"`,
            );
          LOG.debug('ManyToOneRelations: 5. mappedId = ', mappedId);

          const relatedMemoryItem: MemoryItem | undefined = find(
            db[mappedTable].items,
          ...
          LOG.debug(
            'ManyToOneRelations: 5. relatedMemoryItem = ',
            relatedMemoryItem,
          );
          ...
          if (!relatedEntity)
            throw new TypeError(
              `Could not find related entity by property "${propertyName}"`,
            );

          LOG.debug('ManyToOneRelations: 6. Entity = ', entity);
          LOG.debug(
            'ManyToOneRelations: 6. Related Entity = ',
            relatedMemoryItem,
          );
          ...
          LOG.debug(
            `ManyToOneRelations: 8. Searching for #${mappedId} from tableq ${mappedTable}`,
          );
          ...
          LOG.debug(
            'ManyToOneRelations: 9. storedRelatedItem = ',
            storedRelatedItem,
          );
          if (!storedRelatedItem)
            throw new TypeError(
              `Could not find related entity by id "${mappedId}" from table "${mappedTable}"`,
            );
