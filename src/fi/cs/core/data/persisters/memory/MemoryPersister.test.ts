  ...
  describe('instance', () => {
    ...
    describe('#count', () => {
      ...
      it('can count items by property when there is none', async () => {
        expect(
          await persister.count(
            fooMetadata,
            Where.propertyEquals('name', 'Foo 123'),
          ),
        ).toBe(0);
      });
      ...
      it('can count items by property when there is three', async () => {
        expect(
          await persister.count(
            barMetadata,
            Where.propertyEquals('name', 'Bar 456'),
          ),
        ).toBe(1);
      });
    });
    ...
    describe('#deleteAll', () => {
      ...
      it('can delete items by property when there is none', async () => {
        ...
        await persister.deleteAll(
          fooMetadata,
          Where.propertyEquals(
            fooMetadata.idPropertyName,
            barEntity2.id as string,
          ),
        );
        expect(await persister.count(fooMetadata, undefined)).toBe(0);
        expect(await persister.count(barMetadata, undefined)).toBe(3);
      ...
      it('can delete items by property list when there is none', async () => {
        ...
        await persister.deleteAll(
          fooMetadata,
          Where.propertyListEquals(fooMetadata.idPropertyName, [
            barEntity2.id as string,
          ]),
        );
        expect(await persister.count(fooMetadata, undefined)).toBe(0);
      ...
      it('can delete items by property list when there is three', async () => {
        ...
        await persister.deleteAll(
          barMetadata,
          Where.propertyListEquals(barMetadata.idPropertyName, [
            barEntity2.id as string,
          ]),
        );
        expect(await persister.count(fooMetadata, undefined)).toBe(0);
    ...
    describe('#findAll', () => {
      it('can detect there is no matches', async () => {
        expect(
          await persister.findAll(fooMetadata, undefined, undefined),
        ).toStrictEqual([]);
      });

      it('can detect there is three matches', async () => {
        const list: BarEntity[] = await persister.findAll(
          barMetadata,
          undefined,
          undefined,
        );
    ...
    describe('#findBy', () => {
      ...
      it('can find by id when there is one match', async () => {
        expect(barEntity2.id).toBeDefined();
        const item: BarEntity | undefined = await persister.findBy(
          barMetadata,
          Where.propertyEquals(
            barMetadata.idPropertyName,
            barEntity2.id as string,
    ...
    describe('#insert', () => {
      it('can insert new item', async () => {
        expect(await persister.count(fooMetadata, undefined)).toBe(0);
        const entity = await persister.insert(
          fooMetadata,
          new FooEntity({name: 'Hello world'}),
        );
    ...
    describe('#update', () => {
      ...
      it('cannot update an item without update call', async () => {
        expect(barEntity2.id).toBeDefined();
        barEntity2.name = 'Hello world';
        const entity: BarEntity | undefined = await persister.findBy(
          barMetadata,
          Where.propertyEquals(
            barMetadata.idPropertyName,
            barEntity2.id as string,
