...
export class MySqlUtils {
  public static getDateTimeStringFromDate(time: Date): string {
    ...
    const year =
      isNumber(utcFullYear) && utcFullYear >= 0 ? `${utcFullYear}` : '';
    const month =
      isNumber(utcMonth) && utcMonth >= 0
        ? padStart(`${1 + utcMonth}`, 2, '0')
        : '';
    const date =
      isNumber(utcDate) && utcDate >= 0 ? padStart(`${utcDate}`, 2, '0') : '';
    const hours =
      isNumber(utcHours) && utcHours >= 0
        ? padStart(`${utcHours}`, 2, '0')
        : '';
    const minutes =
      isNumber(utcMinutes) && utcMinutes >= 0
        ? padStart(`${utcMinutes}`, 2, '0')
        : '';
    const seconds =
      isNumber(utcSeconds) && utcSeconds >= 0
        ? padStart(`${utcSeconds}`, 2, '0')
        : '';
  ...
  public static getDateTimeStringFromISOString(isoDate: string): string {
