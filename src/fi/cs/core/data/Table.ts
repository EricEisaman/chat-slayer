...
export const Table = (tableName: string) => {
  return (
    ...
    EntityMetadataUtils.updateMetadata(target, (metadata: EntityMetadata) => {
      ...
      if (TargetEntity) {
        const createEntity = (dto?: unknown) => new TargetEntity(dto);

        // We'll test if it creates correct entity object
        const ret: any = createEntity() as unknown as any;
        if (!isEntity(ret)) {
          LOG.warn(
            `Warning! @Table(${JSON.stringify(tableName)}): Your entity class was not extended from the Entity base class. Functionality will be broken.`,
          );
        } else {
