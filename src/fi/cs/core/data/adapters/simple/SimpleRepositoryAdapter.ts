...
} from '../../../simpleRepository/types/SimpleStoredRepositoryItem';
import {
  SimpleRepository as SimpleBaseRepository,
  REPOSITORY_NEW_IDENTIFIER,
} from '../../../simpleRepository/types/SimpleRepository';
import {
  createRepositoryEntry,
  SimpleRepositoryEntry,
} from '../../../simpleRepository/types/SimpleRepositoryEntry';
 ...
 * framework.
 */
export class SimpleRepositoryAdapter<
...
> implements SimpleBaseRepository<T> {
  ...
  public constructor(
    ...
    this._isT = isT;
    // this._tName      = tName ?? 'T';
    // this._explainT   = explainT ?? ( (value: unknown) : string => isT(value) ? explainOk() : explainNot(this._tName) );
  ...
  public async deleteById(id: string): Promise<SimpleRepositoryEntry<T>> {
    const entity: SimpleEntityT | undefined =
  ...
  public async deleteByIdList(
    ...
    if (list.length <= 0)
      throw new TypeError('deleteByIdList: The list argument was empty');
  ...
  public async deleteByList(
    ...
    if (list.length <= 0)
      throw new TypeError('deleteByList: The list argument was empty');
  ...
  public async findById(
    ...
    const entity: SimpleEntity | undefined =
      await this._repository.findById(id);
  ...
  public async getAllByProperty(
    ...
    const items: SimpleEntity[] =
      await this._repository.findAllByEntityDeleted(false);
    const filteredEntities = filter(
      items,
      (item: SimpleEntity): boolean =>
        get(SimpleEntity.parseData(item), propertyName) === propertyValue,
    );
  ...
  public async getSome(
    ...
    if (idList.length <= 0)
      throw new TypeError('getSome: The list argument was empty');
  ...
  public async inviteToItem(
    ...
    const newInvited = filter(
      uniq(concat([], prevInvited, members)),
  ...
  public async updateOrCreateItem(item: T): Promise<SimpleRepositoryEntry<T>> {
    ...
    const foundItem: SimpleRepositoryEntry<T> | undefined =
      id && id !== REPOSITORY_NEW_IDENTIFIER
  ...
  private _createRepositoryEntryFromEntity(
    ...
    return createRepositoryEntry<T>(
      ...
      dto.version,
      dto.deleted,
      includeMembers && dto.members
        ? map(dto.members, (id: string) => createSimpleRepositoryMember(id))
        : undefined,
    );
  }

  private _createRepositoryEntryArrayFromEntityArray(
    list: readonly SimpleEntity[],
  ): SimpleRepositoryEntry<T>[] {
    return map(list, (item: SimpleEntity) =>
      this._createRepositoryEntryFromEntity(item),
    );
  }
}
