...
export const basicCrudTests = (context: RepositoryTestContext): void => {
  ...
  interface FooRepository extends Repository<FooEntity, string> {
    ...
    findAllByFooDateBetween(
      start: string,
      end: string,
      sort?: Sort,
    ): Promise<FooEntity[]>;
    findByFooDateBetween(
      start: string,
      end: string,
      sort?: Sort,
    ): Promise<FooEntity | undefined>;
  ...
  interface BarRepository extends Repository<BarEntity, string> {
    ...
    findAllByBarDateBetween(
      start: string,
      end: string,
      sort?: Sort,
    ): Promise<BarEntity[]>;
    findByBarDateBetween(
      start: string,
      end: string,
      sort?: Sort,
    ): Promise<BarEntity | undefined>;
  ...
  const barEntityName1: string = 'Bar 123';
  const barEntityName2: string = 'Bar 456';
  const barEntityName3: string = 'Bar 789';
  const barEntityName4: string = 'Bar 123';

  const dateBeforeEntity1: string = '2023-04-04T14:58:59Z';
  const barEntityDate1: string = '2023-04-30T10:03:12Z';
  const dateBetweenEntity1And2: string = '2023-05-02T17:44:14Z';
  const barEntityDate2: string = '2023-05-11T17:12:03Z';
  const dateBetweenEntity2And3: string = '2023-05-11T17:57:00Z';
  const barEntityDate3: string = '2023-05-12T07:44:12Z';
  const dateBetweenEntity3And4: string = '2023-05-12T15:30:42Z';
  const barEntityDate4: string = '2023-05-12T15:55:39Z';
  const dateAfterEntity4: string = '2023-05-12T15:55:59Z';

  beforeEach(async () => {
    persister = context.getPersister();

    // Will be initialized with no entities
    fooRepository = createCrudRepositoryWithPersister<
      FooEntity,
      string,
      FooRepository
    >(new FooEntity(), persister);
    await fooRepository.deleteAll();

    // Will be initialized with four entities
    barRepository = createCrudRepositoryWithPersister<
      BarEntity,
      string,
      BarRepository
    >(new BarEntity(), persister);
    ...
    if (barEntity1.barDate !== barEntityDate1)
      throw new TypeError(
        `barEntity1 date did not initialize correctly: ${barEntity1.barDate}`,
      );
    ...
    if (barEntityId1 === barEntityId2)
      throw new TypeError(
        `barEntity2 failed to initialize (not unique ID with barEntityId1 and barEntityId2): ${barEntityId1}`,
      );
    if (barEntity2.barDate !== barEntityDate2)
      throw new TypeError(
        `barEntity1 date did not initialize correctly: ${barEntity2.barDate}`,
      );
    ...
    if (barEntityId1 === barEntityId3)
      throw new TypeError(
        `barEntityId3 failed to initialize (not unique ID with entity 1): ${barEntityId1}`,
      );
    if (barEntityId2 === barEntityId3)
      throw new TypeError(
        `barEntityId3 failed to initialize (not unique ID with entity 2): ${barEntityId2}`,
      );
    if (barEntity3.barDate !== barEntityDate3)
      throw new TypeError(
        `barEntity1 date did not initialize correctly: ${barEntity3.barDate}`,
      );
    ...
    if (barEntityId1 === barEntityId4)
      throw new TypeError(
        `barEntityId4 failed to initialize (not unique ID with entity 1): ${barEntityId1}`,
      );
    if (barEntityId2 === barEntityId4)
      throw new TypeError(
        `barEntityId4 failed to initialize (not unique ID with entity 2): ${barEntityId2}`,
      );
    if (barEntityId3 === barEntityId4)
      throw new TypeError(
        `barEntityId4 failed to initialize (not unique ID with entity 3): ${barEntityId3}`,
      );
    if (barEntity4.barDate !== barEntityDate4)
      throw new TypeError(
        `barEntity1 date did not initialize correctly: ${barEntity4.barDate}`,
      );
  });
  ...
  describe('#delete', () => {
    it('can delete entity by entity object', async () => {
      ...
      const entity: BarEntity | undefined =
        await barRepository.findByBarId(barEntityId2);
  ...
  describe('#deleteAll', () => {
    ...
    it('can delete all entities with few ids', async () => {
      expect(await barRepository.count()).toBe(4);
      await barRepository.deleteAll([barEntity2, barEntity3]);
      expect(await barRepository.count()).toBe(2);

      const entity1: BarEntity | undefined =
        await barRepository.findByBarId(barEntityId1);
      expect(entity1).toBeDefined();

      const entity2: BarEntity | undefined =
        await barRepository.findByBarId(barEntityId2);
      expect(entity2).not.toBeDefined();

      const entity3: BarEntity | undefined =
        await barRepository.findByBarId(barEntityId3);
      expect(entity3).not.toBeDefined();

      const entity4: BarEntity | undefined =
        await barRepository.findByBarId(barEntityId4);
  ...
  describe('#deleteAllById', () => {
    ...
    it('can delete all entities by few ids', async () => {
      expect(await barRepository.count()).toBe(4);
      await barRepository.deleteAllById([barEntityId2, barEntityId3]);
      expect(await barRepository.count()).toBe(2);
  ...
  describe('#findAll', () => {
    ...
    it('can find all entities sorted by name and id in desc order', async () => {
      const items = await barRepository.findAll(
        Sort.by(Sort.Direction.DESC, 'barName', 'barId'),
      );
      expect(items).toBeArray();
      expect(items?.length).toBe(4);
  ...
  describe('#findAllById', () => {
    it('can find all entities by id unsorted', async () => {
      const items = await barRepository.findAllById([
        barEntityId2,
        barEntityId3,
      ]);
      expect(items).toBeArray();
    ...
    it('can find all entities by id in desc order', async () => {
      const items = await barRepository.findAllById(
        [barEntityId2, barEntityId3],
        Sort.by(Sort.Direction.DESC, 'barName'),
      );
      expect(items).toBeArray();
      expect(items?.length).toBe(2);
      expect(items[1]).toBeDefined();
  ...
  describe('#findById', () => {
    ...
    it('can find entity by id by asc order', async () => {
      const item = await barRepository.findById(
        barEntityId2,
        Sort.by('barName'),
      );
      expect(item).toBeDefined();
  ...
  describe('#find', () => {
    ...
    it('can find entities by property in asc order', async () => {
      const items = await barRepository.find(
        'barName',
        barEntityName2,
        Sort.by('barName'),
      );
  ...
  describe('#save', () => {
    ...
    it('cannot save older entity with non-updatable field when no other field has changed', async () => {
      ...
      let foundItem: FooEntity | undefined =
        await fooRepository.findById(addedId);
      expect(foundItem).toBeDefined();
      expect(foundItem?.fooId).toBe(addedId);
      expect(foundItem?.fooName).toBe('Hello world');
      expect(foundItem?.nonUpdatable).toBe('hello');
  ...
  describe('#saveAll', () => {
    it('can save fresh entities', async () => {
      ...
      const newEntity1 = new FooEntity({
        fooName: 'Hello world 1',
        fooDate: '2023-05-12T10:22:32+03:00',
        nonUpdatable: 'hello',
      });
      const newEntity2 = new FooEntity({
        fooName: 'Hello world 2',
        fooDate: '2023-05-12T15:42:09+03:00',
        nonUpdatable: 'hello',
      });
  ...
  describe('#findAllByBarName', () => {
    ...
    it('can fetch single entity by barName property in asc order', async () => {
      const items = await barRepository.findAllByBarName(
        barEntityName2,
        Sort.by('barName'),
      );
      expect(items).toBeArray();
    ...
    it('can fetch multiple entities by barId property in desc order', async () => {
      const items = await barRepository.findAllByBarName(
        barEntityName1,
        Sort.by(Sort.Direction.DESC, 'barId'),
      );
      expect(items).toBeArray();
      expect(items?.length).toBe(2);
  ...
  describe('#findByBarName', () => {
    it('can find entity by barName property unsorted', async () => {
      const entity: BarEntity | undefined =
        await barRepository.findByBarName(barEntityName2);
    ...
    it('can find entity by barName property in asc order', async () => {
      const entity: BarEntity | undefined = await barRepository.findByBarName(
        barEntityName2,
        Sort.by('barName'),
      );
      expect(entity).toBeDefined();
  ...
  describe('#findAllByBarId', () => {
    ...
    it('can find all entities by barId in asc order', async () => {
      const items = await barRepository.findAllByBarId(
        barEntityId2,
        Sort.by('barName'),
      );
      expect(items).toBeArray();
  ...
  describe('#findByBarId', () => {
    ...
    it('can find an entity by barId in asc order', async () => {
      const item = await barRepository.findByBarId(
        barEntityId2,
        Sort.by('barName'),
      );
      expect(item?.barId).toBe(barEntityId2);
  ...
  describe('#findAllByBarDateBetween', () => {
    it('can find all entities between values by date unordered', async () => {
      const items = await barRepository.findAllByBarDateBetween(
        barEntityDate2,
        barEntityDate3,
      );
    ...
    it('can find all entities by barId in desc order', async () => {
      const items = await barRepository.findAllByBarDateBetween(
        barEntityDate2,
        barEntityDate3,
        Sort.by(Sort.Direction.DESC, 'barDate'),
      );
      expect(items).toHaveLength(2);
      expect(items[0]?.barId).toBe(barEntityId3);
      expect(items[0]?.barName).toBe(barEntityName3);
  ...
  describe('#findByBarDateBetween', () => {
    it('can find an entity between times in unsorted order', async () => {
      const item = await barRepository.findByBarDateBetween(
        dateBetweenEntity3And4,
        dateAfterEntity4,
      );
  ...
  describe('#deleteAllByBarDateBetween', () => {
    it('can delete all entities by barDate between range', async () => {
      await barRepository.deleteAllByBarDateBetween(
        dateBeforeEntity1,
        dateBetweenEntity3And4,
      );
  ...
  describe('#existsByBarDateBetween', () => {
    it('can find if entities exist between range', async () => {
      expect(
        await barRepository.existsByBarDateBetween(
          dateBetweenEntity1And2,
          dateBetweenEntity3And4,
        ),
      ).toBe(true);
  ...
  describe('#countByBarDateBetween', () => {
    it('can count entities by barId', async () => {
      expect(
        await barRepository.countByBarDateBetween(
          dateBetweenEntity1And2,
          dateAfterEntity4,
        ),
      ).toBe(3);
  ...
  describe('#findAllByBarDateBefore', () => {
    it('can find all entities before time, unordered', async () => {
      ...
      const items = await barRepository.findAllByBarDateBefore(
        dateBetweenEntity2And3,
      );
      expect(items).toHaveLength(2);
  ...
  describe('#findByBarDateBefore', () => {
    ...
    it('can find an entity before time in unsorted order', async () => {
      // Matches entity 1
      const item = await barRepository.findByBarDateBefore(
        dateBetweenEntity1And2,
      );
      expect(item?.barId).toBe(barEntityId1);
      expect(item?.barName).toBe(barEntityName1);
  ...
  describe('#existsByBarDateBefore', () => {
    it('can find if entities exist before time', async () => {
      expect(
        await barRepository.existsByBarDateBefore(dateBetweenEntity1And2),
      ).toBe(true);
  ...
  describe('#countByBarDateBefore', () => {
    it('can count entities before time', async () => {
      ...
      expect(
        await barRepository.countByBarDateBefore(dateBetweenEntity3And4),
      ).toBe(3);
  ...
  describe('#findAllByBarDateAfter', () => {
    it('can find all entities after time, unordered', async () => {
      ...
      const items = await barRepository.findAllByBarDateAfter(
        dateBetweenEntity2And3,
      );
      expect(items).toHaveLength(2);
  ...
  describe('#findByBarDateAfter', () => {
    it('can find an entity after time in unsorted order', async () => {
      // Matches 4
      const item = await barRepository.findByBarDateAfter(
        dateBetweenEntity3And4,
      );
      expect(item?.barId).toBe(barEntityId4);
      expect(item?.barName).toBe(barEntityName4);
  ...
  describe('#existsByBarDateAfter', () => {
    it('can find if entities exist after barDate', async () => {
      expect(
        await barRepository.existsByBarDateAfter(dateBetweenEntity1And2),
      ).toBe(true);
      await barRepository.deleteAllByBarDateBetween(
        dateBetweenEntity1And2,
        dateAfterEntity4,
  ...
  describe('#countByBarDateAfter', () => {
    it('can count entities after barDate', async () => {
      expect(
        await barRepository.countByBarDateAfter(dateBetweenEntity1And2),
      ).toBe(3);
      await barRepository.deleteAllByBarDateAfter(dateBetweenEntity1And2);
      expect(await barRepository.countByBarDateAfter(dateBeforeEntity1)).toBe(
        1,
      );
    });
  });
