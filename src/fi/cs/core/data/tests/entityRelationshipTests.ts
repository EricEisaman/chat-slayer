...
export const entityRelationshipTests = (
  ...
  let cartB_id: string;
  const cartB_name: string = 'Cart B';

  let cartB_item1: CartItemEntity;
  let cartB_item1_id: string;
  const cartB_item1_name: string = 'Cart B item 1';
  let cartB_item2: CartItemEntity;
  let cartB_item2_id: string;
  const cartB_item2_name: string = 'Cart B item 2';
  let cartB_item3: CartItemEntity;
  let cartB_item3_id: string;
  const cartB_item3_name: string = 'Cart B item 3';

  let cartB_contact1: ContactEntity;
  let cartB_contact1_id: string;
  const cartB_contact1_name: string = 'Cart B contact 1';
  let cartB_contact2: ContactEntity;
  let cartB_contact2_id: string;
  const cartB_contact2_name: string = 'Cart B contact 2';
  let cartB_contact3: ContactEntity;
  let cartB_contact3_id: string;
  const cartB_contact3_name: string = 'Cart B contact 3';

  // This cart must not have items
  let cartC: CartEntity;
  let cartC_id: string;
  const cartC_name: string = 'Cart C';

  beforeEach(async () => {
    persister = context.getPersister();

    cartRepository = createCrudRepositoryWithPersister<
      CartEntity,
      string,
      CartRepository
    >(new CartEntity(), persister);

    cartItemRepository = createCrudRepositoryWithPersister<
      CartItemEntity,
      string,
      CartItemRepository
    >(new CartItemEntity(), persister);

    contactRepository = createCrudRepositoryWithPersister<
      ContactEntity,
      string,
      ContactRepository
    >(new ContactEntity(), persister);
    ...
    cartA = await persister.insert(cartA.getMetadata(), cartA);
    cartA_id = cartA?.id as string;
    if (!isString(cartA_id))
      throw new TypeError(
        `cartA_id failed to initialize: ${JSON.stringify(cartA_id)}`,
      );

    // LOG.debug(`Step 2`)
    cartA_item1 = new CartItemEntity({
      cartId: cartA_id,
      name: cartA_item1_name,
    });
    ...
    if (!isString(cartA_item1_id))
      throw new TypeError(
        `cartItemA1_id failed to initialize: ${JSON.stringify(cartA_item1_id)}`,
      );

    // LOG.debug(`Step 3`)
    cartA_item2 = new CartItemEntity({
      cartId: cartA_id,
      name: cartA_item2_name,
    });
    ...
    if (!isString(cartA_item2_id))
      throw new TypeError(
        `cartItemA2_id failed to initialize: ${JSON.stringify(cartA_item2_id)}`,
      );

    // LOG.debug(`Step 4`)
    cartA_item3 = new CartItemEntity({
      cartId: cartA_id,
      name: cartA_item3_name,
    });
    ...
    if (!isString(cartA_item3_id))
      throw new TypeError(
        `cartItemA3_id failed to initialize: ${JSON.stringify(cartA_item3_id)}`,
      );

    // LOG.debug(`Step 2`)
    cartA_contact1 = new ContactEntity({
      cartId: cartA_id,
      name: cartA_contact1_name,
    });
    ...
    if (!isString(cartA_contact1_id))
      throw new TypeError(
        `contactA1_id failed to initialize: ${JSON.stringify(cartA_contact1_id)}`,
      );

    // LOG.debug(`Step 3`)
    cartA_contact2 = new ContactEntity({
      cartId: cartA_id,
      name: cartA_contact2_name,
    });
    ...
    if (!isString(cartA_contact2_id))
      throw new TypeError(
        `contactA2_id failed to initialize: ${JSON.stringify(cartA_contact2_id)}`,
      );

    // LOG.debug(`Step 4`)
    cartA_contact3 = new ContactEntity({
      cartId: cartA_id,
      name: cartA_contact3_name,
    });
    ...
    if (!isString(cartA_contact3_id))
      throw new TypeError(
        `contactA3_id failed to initialize: ${JSON.stringify(cartA_contact3_id)}`,
      );

    // LOG.debug(`Step 5`)
    cartB = new CartEntity({name: cartB_name});
    cartB = await persister.insert(cartB.getMetadata(), cartB);
    cartB_id = cartB?.id as string;
    if (!isString(cartB_id))
      throw new TypeError(
        `cartB_id failed to initialize: ${JSON.stringify(cartB_id)}`,
      );

    // LOG.debug(`Step 6`)
    cartB_item1 = new CartItemEntity({
      cartId: cartB_id,
      name: cartB_item1_name,
    });
    ...
    if (!isString(cartB_item1_id))
      throw new TypeError(
        `cartB_item1_id failed to initialize: ${JSON.stringify(cartB_item1_id)}`,
      );

    // LOG.debug(`Step 7`)
    cartB_item2 = new CartItemEntity({
      cartId: cartB_id,
      name: cartB_item2_name,
    });
    ...
    if (!isString(cartB_item2_id))
      throw new TypeError(
        `cartB_item2_id failed to initialize: ${JSON.stringify(cartB_item2_id)}`,
      );

    // LOG.debug(`Step 8`)
    cartB_item3 = new CartItemEntity({
      cartId: cartB_id,
      name: cartB_item3_name,
    });
    ...
    if (!isString(cartB_item3_id))
      throw new TypeError(
        `cartB_item3_id failed to initialize: ${JSON.stringify(cartB_item3_id)}`,
      );

    // LOG.debug(`Step 6`)
    cartB_contact1 = new ContactEntity({
      cartId: cartB_id,
      name: cartB_contact1_name,
    });
    ...
    if (!isString(cartB_contact1_id))
      throw new TypeError(
        `cartB_contact1_id failed to initialize: ${JSON.stringify(cartB_contact1_id)}`,
      );

    // LOG.debug(`Step 7`)
    cartB_contact2 = new ContactEntity({
      cartId: cartB_id,
      name: cartB_contact2_name,
    });
    ...
    if (!isString(cartB_contact2_id))
      throw new TypeError(
        `cartB_contact2_id failed to initialize: ${JSON.stringify(cartB_contact2_id)}`,
      );

    // LOG.debug(`Step 8`)
    cartB_contact3 = new ContactEntity({
      cartId: cartB_id,
      name: cartB_contact3_name,
    });
    ...
    if (!isString(cartB_contact3_id))
      throw new TypeError(
        `cartB_contact3_id failed to initialize: ${JSON.stringify(cartB_contact3_id)}`,
      );

    // LOG.debug(`Step 1`)
    cartC = new CartEntity({name: cartC_name});
    cartC = await persister.insert(cartC.getMetadata(), cartC);
    cartC_id = cartC?.id as string;
    if (!isString(cartC_id))
      throw new TypeError(
        `cartC_id failed to initialize: ${JSON.stringify(cartC_id)}`,
      );
  ...
  describe('#findAll', () => {
    it('returns related cart items mapped by @OneToMany', async () => {
      ...
      const cart1 = find(
        items,
        (item: CartEntity): boolean => item.id === cartA_id,
      );
      const cart2 = find(
        items,
        (item: CartEntity): boolean => item.id === cartB_id,
      );
      const cart3 = find(
        items,
        (item: CartEntity): boolean => item.id === cartC_id,
      );
      ...
      const cart1_item1 = find(
        cart1?.items,
        item => item.id === cartA_item1_id,
      );
      const cart1_item2 = find(
        cart1?.items,
        item => item.id === cartA_item2_id,
      );
      const cart1_item3 = find(
        cart1?.items,
        item => item.id === cartA_item3_id,
      );
      ...
      const cart2_item1 = find(
        cart2?.items,
        item => item.id === cartB_item1_id,
      );
      const cart2_item2 = find(
        cart2?.items,
        item => item.id === cartB_item2_id,
      );
      const cart2_item3 = find(
        cart2?.items,
        item => item.id === cartB_item3_id,
      );
  ...
  describe('#findById', () => {
    it('returns related cart mapped by @OneToMany to cart item entities', async () => {
      const entity: CartEntity | undefined =
        await cartRepository.findById(cartA_id);
    ...
    it('returns related cart item mapped by @ManyToOne to cart entity', async () => {
      const entity: CartItemEntity | undefined =
        await cartItemRepository.findById(cartA_item1_id);
      expect(entity).toBeDefined();
  ...
  describe('#findByName', () => {
    it('returns related cart mapped by @OneToMany to cart item entities', async () => {
      const cartEntity: CartEntity | undefined =
        await cartRepository.findByName(cartA_name);
      expect(cartEntity).toBeDefined();
      expect(cartEntity?.id).toBe(cartA_id);
      expect(cartEntity?.name).toBe(cartA_name);
      expect(cartEntity?.items).toBeArray();
    ...
    it('returns related cart items mapped by @ManyToOne to the cart entity', async () => {
      const cartItemEntity: CartItemEntity | undefined =
        await cartItemRepository.findByName(cartA_item1_name);
