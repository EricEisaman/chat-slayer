...
import {
  isJsonColumnDefinition,
  isTimeColumnDefinition,
...
export class EntityUtils {
  ...
  public static getColumnName(
    ...
    const field = fields.find(
      (x: EntityField) => x.propertyName === propertyName,
    );
    if (!field)
      throw new RepositoryError(
        RepositoryError.Code.COLUMN_NAME_NOT_FOUND,
        `Column name not found for property: "${propertyName}"`,
      );
  ...
  public static getPropertyName(
    ...
    throw new RepositoryError(
      RepositoryError.Code.PROPERTY_NAME_NOT_FOUND,
  ...
  public static toEntity<T extends Entity>(
    ...
    if (!createEntity)
      throw new TypeError('Could not create entity: No create function');

    const ret: T = createEntity() as unknown as T;
    if (!isEntity(ret)) {
      LOG.debug('The created entity was not extended from Entity class: ', ret);
      throw new TypeError(
        'Could not create an entity object. This is probably because your entity class was not extended from the Entity base class. See debug log for extra information.',
      );
    }

    forEach(fields, (field: EntityField): void => {
      const {
        fieldType,
        propertyName,
        columnName,
      ...
      if (fieldType === EntityFieldType.JOINED_ENTITY) {
        ...
        const isTime: boolean =
          !!temporalType || isTimeColumnDefinition(columnDefinition);
        if (isTime) {
          LOG.debug(
            `toEntity: "${propertyName}": as string "${dbEntity[columnName]}": `,
            dbEntity[columnName],
          );
          (ret as any)[propertyName] = EntityUtils.parseDateAsString(
            dbEntity[columnName],
          );
          return;
        }
        ...
        if (isJson) {
          ...
          (ret as any)[propertyName] = EntityUtils.parseJsonObject(
            dbEntity[columnName],
          );
          return;
        }
    ...
    // OneToMany relations
    forEach(oneToManyRelations, (relation: EntityRelationOneToMany): void => {
      ...
      if (!mappedTable) {
        LOG.warn(
          `Warning: @oneToMany: The property "${propertyName}" did not have table mapped`,
        );
        return;
      }
      ...
      if (!mappedMetadata) {
        LOG.debug(
          `oneToMany: "${propertyName}": mappedMetadata=`,
          mappedMetadata,
        );
        throw new TypeError(
          `Could not find metadata for table "${mappedTable}" (for property ${propertyName}) -- Check that your table names in @OneToMany and @Table match exactly`,
        );
      }

      let dbValue: any = has(dbEntity, propertyName)
        ? dbEntity[propertyName]
        : undefined;
      if (dbValue !== undefined) {
        if (isString(dbValue)) {
          ...
          if (dbValue === undefined)
            throw new TypeError(`Failed to parse JSON: "${jsonString}"`);
        }
        if (isNull(dbValue)) {
        ...
        if (!isArray(dbValue)) {
          ...
          throw new TypeError(
            `Expected the dbValue for property "${propertyName}" to be an array: ${dbValue}`,
          );
        }
        (ret as any)[propertyName] = map(
          filter(dbValue, value => !isNull(value)),
          (value: unknown) => {
            if (!value)
              throw new TypeError(
                `Unexpected undefined item in an array for property "${propertyName}"`,
              );
    ...
    // ManyToOne relations
    forEach(manyToOneRelations, (relation: EntityRelationManyToOne): void => {
      const {propertyName} = relation;
      ...
      if (!mappedTable) {
        ...
        throw new TypeError(
          `The property "${propertyName}" did not have table configured`,
        );
      }
      const mappedMetadata = metadataManager.getMetadataByTable(mappedTable);
      if (!mappedMetadata) {
        LOG.debug(
          `manyToOne: ${propertyName}: mappedMetadata=`,
          mappedMetadata,
        );
        throw new TypeError(
          `Could not find metadata for property "${propertyName}" from table "${mappedTable}"`,
        );
      }
      ...
      if (dbValue !== undefined) {
        ...
        (ret as any)[propertyName] = this.toEntity(
          dbValue,
          mappedMetadata,
          metadataManager,
        );
  ...
  public static getPropertyFromEntity<T extends Entity>(
    value: T,
    propertyName: string,
  ): any {
  ...
  public static getId<T extends Entity, ID extends EntityIdTypes>(
    ...
    throw new RepositoryError(
      RepositoryError.Code.ID_NOT_FOUND_FOR_TABLE,
      `Id property not found for table: "${tablePrefix}${metadata.tableName}"`,
    );
  }

  public static isIdField(field: EntityField, metadata: EntityMetadata) {
  ...
  public static parseBoolean(input: unknown): boolean {
  ...
  public static parseNumber(input: unknown): number | undefined {
    return isNumber(input) ? input : undefined;
  }

  public static parseJsonObject(
    input: unknown,
  ): ReadonlyJsonObject | undefined {
    return parseReadonlyJsonObject(input);
  }

  public static parseIntegerAsString(
    input: string | number | undefined,
  ): string | undefined {
    if ((isString(input) && trim(input)) === '' || input === undefined)
      return undefined;
    return `${input}`;
  }
  ...
  public static parseMySQLDateAsIsoString(value: unknown): string | undefined {
    const parsed = MySqlDateTime.parse(
      EntityUtils.parseDateAsString(value as string | Date | undefined),
    );
    return parsed ? parsed.getISOString() : undefined;
  }

  public static parseDateAsIsoString(value: unknown): string | undefined {
    const parsed = EntityUtils.parseDateAsString(
      value as string | Date | undefined,
    );
    return parsed ? parsed : undefined;
  }

  public static toJsonString(value: unknown): string | undefined {
    return JSON.stringify(value);
  }

  public static parseIsoStringAsMySQLDateString(
    value: unknown,
  ): string | undefined {
    const parsed = MySqlDateTime.parse(value);
  ...
  public static areEntitiesSameType<T extends Entity>(
    ...
    if (list.length < 1)
      throw new TypeError(
        'Cannot check empty array of entities for their type',
      );
    if (!metadata) metadata = list[0].getMetadata();
    return every(
      list,
      (entity: T): boolean => entity.getMetadata() === metadata,
    );
  }
  ...
  public static removeEntityRelations<T extends Entity>(
    ...
    forEach(metadata.manyToOneRelations, relation => {
      const {propertyName} = relation;
      (clonedEntity as any)[propertyName] = undefined;
    });

    forEach(metadata.oneToManyRelations, relation => {
      const {propertyName} = relation;
