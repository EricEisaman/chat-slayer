...
export class RepositoryUtils {
  ...
  >(proto: any, entityMetadata: EntityMetadata) {
    forEach(entityMetadata.fields, (item: EntityField) => {
      ...
      if (!this._isReservedRepositoryMethodName(proto, findAllByMethodName)) {
        ...
        LOG.debug(
          `The property "${findAllByMethodName}" was already defined. Not extending it.`,
        );
      }
      ...
      if (!this._isReservedRepositoryMethodName(proto, findByMethodName)) {
        ...
        LOG.debug(
          `The property "${findByMethodName}" was already defined. Not extending it.`,
        );
      }
      ...
      if (!this._isReservedRepositoryMethodName(proto, deleteAllByMethodName)) {
        ...
        LOG.debug(
          `The property "${deleteAllByMethodName}" was already defined. Not extending it.`,
        );
      }

      const existsByMethodName = `existsBy${camelCasePropertyName}`;
      if (!this._isReservedRepositoryMethodName(proto, existsByMethodName)) {
        proto[existsByMethodName] = function existsByProperty(
        ...
        LOG.debug(
          `The property "${existsByMethodName}" was already defined. Not extending it.`,
        );
      }

      const countByMethodName = `countBy${camelCasePropertyName}`;
      if (!this._isReservedRepositoryMethodName(proto, countByMethodName)) {
        proto[countByMethodName] = function countByProperty(
          propertyValue: unknown,
        ): Promise<number> {
        ...
        LOG.debug(
          `The property "${countByMethodName}" was already defined. Not extending it.`,
        );
      }
      ...
      if (
        !this._isReservedRepositoryMethodName(proto, findAllByMethodNameBetween)
      ) {
        ...
        LOG.debug(
          `The property "${findAllByMethodNameBetween}" was already defined. Not extending it.`,
        );
      }

      const findByMethodNameBetween = `findBy${camelCasePropertyName}Between`;
      if (
        !this._isReservedRepositoryMethodName(proto, findByMethodNameBetween)
      ) {
        ...
        LOG.debug(
          `The property "${findByMethodNameBetween}" was already defined. Not extending it.`,
        );
      }

      const deleteAllByMethodNameBetween = `deleteAllBy${camelCasePropertyName}Between`;
      if (
        !this._isReservedRepositoryMethodName(
          proto,
          deleteAllByMethodNameBetween,
        )
      ) {
        proto[deleteAllByMethodNameBetween] =
          function deleteAllByPropertyBetween(
            startValue: any,
            endValue: any,
            arg2?: Where | undefined,
            arg3?: Where | undefined,
        ...
        LOG.debug(
          `The property "${deleteAllByMethodNameBetween}" was already defined. Not extending it.`,
        );
      }

      const existsByMethodNameBetween = `existsBy${camelCasePropertyName}Between`;
      if (
        !this._isReservedRepositoryMethodName(proto, existsByMethodNameBetween)
      ) {
        proto[existsByMethodNameBetween] = function existsByPropertyBetween(
          startValue: any,
          endValue: unknown,
        ): Promise<boolean> {
          return RepositoryUtils._existsByCondition<T, ID>(
        ...
        LOG.debug(
          `The property "${existsByMethodNameBetween}" was already defined. Not extending it.`,
        );
      }

      const countByMethodNameBetween = `countBy${camelCasePropertyName}Between`;
      if (
        !this._isReservedRepositoryMethodName(proto, countByMethodNameBetween)
      ) {
        proto[countByMethodNameBetween] = function countByPropertyBetween(
          startValue: any,
          endValue: unknown,
        ): Promise<number> {
          return RepositoryUtils._countByCondition<T, ID>(
        ...
        LOG.debug(
          `The property "${countByMethodNameBetween}" was already defined. Not extending it.`,
        );
      }
      ...
      if (
        !this._isReservedRepositoryMethodName(proto, findAllByMethodNameAfter)
      ) {
        ...
        LOG.debug(
          `The property "${findAllByMethodNameAfter}" was already defined. Not extending it.`,
        );
      }
      ...
      if (!this._isReservedRepositoryMethodName(proto, findByMethodNameAfter)) {
        ...
        LOG.debug(
          `The property "${findByMethodNameAfter}" was already defined. Not extending it.`,
        );
      }

      const deleteAllByMethodNameAfter = `deleteAllBy${camelCasePropertyName}After`;
      if (
        !this._isReservedRepositoryMethodName(proto, deleteAllByMethodNameAfter)
      ) {
        ...
        LOG.debug(
          `The property "${deleteAllByMethodNameAfter}" was already defined. Not extending it.`,
        );
      }

      const existsByMethodNameAfter = `existsBy${camelCasePropertyName}After`;
      if (
        !this._isReservedRepositoryMethodName(proto, existsByMethodNameAfter)
      ) {
        proto[existsByMethodNameAfter] = function existsByPropertyAfter(
          value: unknown,
        ): Promise<boolean> {
          return RepositoryUtils._existsByCondition<T, ID>(
            this,
        ...
        LOG.debug(
          `The property "${existsByMethodNameAfter}" was already defined. Not extending it.`,
        );
      }

      const countByMethodNameAfter = `countBy${camelCasePropertyName}After`;
      if (
        !this._isReservedRepositoryMethodName(proto, countByMethodNameAfter)
      ) {
        proto[countByMethodNameAfter] = function countByPropertyAfter(
          value: unknown,
        ): Promise<number> {
          return RepositoryUtils._countByCondition<T, ID>(
            this,
        ...
        LOG.debug(
          `The property "${countByMethodNameAfter}" was already defined. Not extending it.`,
        );
      }
      ...
      if (
        !this._isReservedRepositoryMethodName(proto, findAllByMethodNameBefore)
      ) {
        ...
        LOG.debug(
          `The property "${findAllByMethodNameBefore}" was already defined. Not extending it.`,
        );
      }

      const findByMethodNameBefore = `findBy${camelCasePropertyName}Before`;
      if (
        !this._isReservedRepositoryMethodName(proto, findByMethodNameBefore)
      ) {
        ...
        LOG.debug(
          `The property "${findByMethodNameBefore}" was already defined. Not extending it.`,
        );
      }
      ...
      if (
        ...
        LOG.debug(
          `The property "${deleteAllByMethodNameBefore}" was already defined. Not extending it.`,
        );
      }

      const existsByMethodNameBefore = `existsBy${camelCasePropertyName}Before`;
      if (
        !this._isReservedRepositoryMethodName(proto, existsByMethodNameBefore)
      ) {
        proto[existsByMethodNameBefore] = function existsByPropertyBefore(
          value: unknown,
        ): Promise<boolean> {
          return RepositoryUtils._existsByCondition<T, ID>(
            this,
        ...
        LOG.debug(
          `The property "${existsByMethodNameBefore}" was already defined. Not extending it.`,
        );
      }

      const countByMethodNameBefore = `countBy${camelCasePropertyName}Before`;
      if (
        !this._isReservedRepositoryMethodName(proto, countByMethodNameBefore)
      ) {
        proto[countByMethodNameBefore] = function countByPropertyBefore(
          value: unknown,
        ): Promise<number> {
          return RepositoryUtils._countByCondition<T, ID>(
            this,
        ...
        LOG.debug(
          `The property "${countByMethodNameBefore}" was already defined. Not extending it.`,
        );
      }
    });
  }
   ...
   * @param arg3
   */
  private static async _findAllByCondition<
    T extends Entity,
    ID extends EntityIdTypes,
  >(
    ...
    const [where2, sort]: [Where | undefined, Sort | undefined] =
      this._parseSortAndWhereArgs(arg2, arg3);
   ...
   */
  private static async _findByCondition<
    T extends Entity,
    ID extends EntityIdTypes,
  >(
    self: CrudRepository<T, ID>,
   ...
   * @param entityMetadata
   */
  private static async _deleteAllByCondition<
    T extends Entity,
    ID extends EntityIdTypes,
  >(
    self: CrudRepository<T, ID>,
    ...
    const persister = self.__getPersister();
    const [where2]: [Where | undefined, Sort | undefined] =
      this._parseSortAndWhereArgs(arg2, arg3);
   ...
   * @param entityMetadata
   */
  private static async _existsByCondition<
    T extends Entity,
    ID extends EntityIdTypes,
  >(
    self: CrudRepository<T, ID>,
    ...
    const persister = self.__getPersister();
    return await persister.existsBy(entityMetadata, where);
  }
   ...
   * @param entityMetadata
   */
  private static async _countByCondition<
    T extends Entity,
    ID extends EntityIdTypes,
  >(
    self: CrudRepository<T, ID>,
    ...
    const persister = self.__getPersister();
    return await persister.count(entityMetadata, where);
  }
  ...
  private static _isReservedRepositoryMethodName(
    ...
    return (
      ObjectUtils.isReservedPropertyName(proto, name) ||
      isReservedRepositoryMethodName(name)
    );
  }
}
