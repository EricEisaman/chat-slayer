...
export type ColumnSelectorCallback = (
  tableName: string,
  columnName: string,
  propertyName: string,
) => void;
  ...
  public static includeFields(
    ...
    forEach(fields, (field: EntityField) => {
      ...
      if (columnName && fieldType !== EntityFieldType.JOINED_ENTITY) {
        ...
        if (
          fieldType === EntityFieldType.DATE_TIME ||
          temporalType === TemporalType.TIMESTAMP ||
          columnDefinition === ColumnDefinition.TIMESTAMP ||
          columnDefinition === ColumnDefinition.TIMESTAMPTZ ||
          columnDefinition === ColumnDefinition.DATETIME ||
          columnDefinition === ColumnDefinition.DATETIMETZ
        ) {
          return asTimestamp(tableName, columnName, propertyName);
        }

        if (
          fieldType === EntityFieldType.TIME ||
          temporalType === TemporalType.TIME ||
          columnDefinition === ColumnDefinition.TIME ||
          columnDefinition === ColumnDefinition.TIMETZ
        ) {
          return asTime(tableName, columnName, propertyName);
        }

        if (
          fieldType === EntityFieldType.DATE ||
          temporalType === TemporalType.DATE ||
          columnDefinition === ColumnDefinition.DATE ||
          columnDefinition === ColumnDefinition.DATETZ
        ) {
