...
export class EntityCallbackUtils {
  ...
  private static async _runCallbacks<T extends Entity>(
    ...
    callbacks = filter(callbacks, callback => callback?.callbackType === type);
        ...
        await reduce(
          ...
          async (
            ...
            if (!(entity && (entity as any)[propertyName])) {
              LOG.warn(
                `The entity did not have callback method defined with name "${propertyName.toString()}": `,
                entity,
              );
              throw new TypeError(
                `The entity did not have callback method defined with name "${propertyName.toString()}"`,
            ...
            try {
              ...
              LOG.warn(
                `The callback named "${propertyName.toString()}" for ${type} life cycle method resolved in to an error: `,
                err,
              );
              throw new TypeError(
                `Callback function "${propertyName.toString()}" failed for ${type} life cycle event handler: ${err}`,
              );
            }
          },
          Promise.resolve(),
