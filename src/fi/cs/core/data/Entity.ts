...
export function entityToJSON(
  ...
  let json = reduce(
    ...
    (prev: ReadonlyJsonObject, field: EntityField): ReadonlyJsonObject => {
      ...
      if (!propertyName)
        throw new TypeError('The field did not have propertyName defined');
      const value: unknown = (entity as any)[propertyName];
      if (value === undefined) return prev;
      if (!isReadonlyJsonAny(value)) {
        LOG.warn(
          `Could not convert property "${propertyName}" as JSON: Value not compatible for JSON:`,
          value,
        );
        return prev;
      }
    ...
    (
      ...
      if (propertyName) {
        ...
        if (!isArrayOf<Entity>(value, isEntity)) {
          LOG.warn(
            `Could not convert property "${propertyName}" as JSON: Value not entity array:`,
            value,
          );
          return prev;
        }
        return {
          ...
          [propertyName]: map(
            value,
            (item: Entity): ReadonlyJsonObject => item.toJSON(),
          ),
        };
      }
      return prev;
  ...
  json = reduce(
    ...
    (
      ...
      if (propertyName) {
        ...
        if (!isEntity(value)) {
          LOG.warn(
            `Could not convert property "${propertyName}" as JSON: Value not entity:`,
            value,
          );
          return prev;
        }
 ...
 * @param metadata
 */
export function cloneEntity(entity: Entity, metadata: EntityMetadata): Entity {
  const idPropertyName = metadata?.idPropertyName;
  if (!idPropertyName)
    throw new TypeError(
      'The entity metadata did not have id property name defined',
    );
  const createEntity: CreateEntityLikeCallback | undefined =
    metadata?.createEntity;
  if (!isFunction(createEntity)) {
    throw new TypeError(
      "The entity metadata did not have ability to create new entities. Did you forget '@table()' annotation?",
    );
  }
  ...
  forEach(metadata?.fields, (field: EntityField) => {
    const propertyName = field.propertyName;
    ...
    if (propertyName && field.fieldType !== EntityFieldType.JOINED_ENTITY) {
      (clonedEntity as any)[propertyName] = cloneEntityValue(
