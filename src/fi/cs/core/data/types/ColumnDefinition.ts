...
export function isJsonColumnDefinition(
  value: unknown,
): value is ColumnDefinition.JSON | ColumnDefinition.JSONB {
  return isColumnDefinition(value) && JSON_COLUMN_DEFINITIONS.includes(value);
}

export function isTimeColumnDefinition(
  value: unknown,
): value is
  | ColumnDefinition.TIMESTAMP
  | ColumnDefinition.TIMESTAMPTZ
...
export function isColumnDefinitionOrUndefined(
  value: unknown,
): value is ColumnDefinition | undefined {
...
export function explainColumnDefinitionOrUndefined(value: unknown): string {
  return isColumnDefinitionOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['ColumnDefinition', 'undefined']));
}
