...
export function parseTemporalType(value: unknown): TemporalType | undefined {
  return parseEnum(TemporalType, value) as TemporalType | undefined;
}

export function isTemporalTypeOrUndefined(
  value: unknown,
): value is TemporalType | undefined {
  return isUndefined(TemporalType) || isTemporalType(value);
}

export function explainTemporalTypeOrUndefined(value: unknown): string {
  return isTemporalTypeOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['TemporalType', 'undefined']));
}
