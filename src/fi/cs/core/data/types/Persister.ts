...
export interface Persister extends Disposable {
   ...
   *              counted.
   */
  count(metadata: EntityMetadata, where: Where | undefined): Promise<number>;
   ...
   */
  existsBy(metadata: EntityMetadata, where: Where): Promise<boolean>;
   ...
   *              removed.
   */
  deleteAll(metadata: EntityMetadata, where: Where | undefined): Promise<void>;
   ...
   * @param entity
   */
  update<T extends Entity>(metadata: EntityMetadata, entity: T): Promise<T>;
}
