...
> implements CrudRepository<T, ID> {
  ...
  public constructor(emptyMetadata: EntityMetadata, persister: Persister) {
  ...
  public async findAll(
    ...
    return await this._persister.findAll(this._entityMetadata, where, sort);
  }

  public async findAllById(ids: any[] | any, sort?: Sort): Promise<T[]> {
  ...
  public async findById(id: any, sort?: Sort): Promise<T | undefined> {
  ...
  public async count(where?: Where): Promise<number> {
    return await this._persister.count(this._entityMetadata, where);
  }

  public async existsBy(where: Where): Promise<boolean> {
  ...
  public async deleteAll(
    ...
    if (entities === undefined) {
      LOG.debug('deleteAll');
      return await this._persister.deleteAll(this._entityMetadata, undefined);
    }

    if (isWhere(entities)) {
      LOG.debug('deleteAll: where = ', entities);
      return await this._persister.deleteAll(this._entityMetadata, entities);
    }
  ...
  public async deleteAllById(ids: readonly ID[] | ID): Promise<void> {
    ...
    const idPropertyName: string = EntityUtils.getIdPropertyName(
      this._entityMetadata,
    );
    return await this._persister.deleteAll(
      this._entityMetadata,
  ...
  public async save(entity: T): Promise<T> {
    ...
    if (!current) {
      throw new RepositoryEntityError(
        id,
        RepositoryEntityError.Code.ENTITY_NOT_FOUND,
        `Entity "${id}" not found in table: ${metadata.tableName}`,
      );
    }
  ...
  private _parseArgs(
    ...
    if (isSort(arg1)) {
      ...
      if (arg2 !== undefined)
        throw new TypeError(`Argument 2 is unknown type: ${arg2}`);
