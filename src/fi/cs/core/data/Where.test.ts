...
describe('Where', () => {
  ...
  describe('#or', () => {
    it('combines two Where instances into a new one with an OrCondition', () => {
      ...
      expect(
        (conditions[0] as OrCondition).getWhere().getConditions(),
      ).toStrictEqual([...whereA.getConditions(), ...whereB.getConditions()]);
    });
  ...
  describe('#propertyListEquals', () => {
    it('creates a Where instance with multiple equal conditions', () => {
      ...
      conditions.forEach(condition => {
        ...
        const childConditions = (condition as OrCondition)
          .getWhere()
          .getConditions();
        ...
        childConditions.forEach((childCondition, childIndex) => {
          ...
          expect((childCondition as EqualCondition).getValue()).toStrictEqual(
            values[childIndex],
          );
        });
      });
