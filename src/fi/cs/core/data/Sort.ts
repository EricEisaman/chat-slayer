...
export class Sort {
  ...
  private readonly _sort: (a: any, b: unknown) => number;

  public static DEFAULT_DIRECTION: SortDirection = SortDirection.ASC;

  protected constructor(orders: readonly SortOrder[]) {
    this._orders = orders;
  ...
  public getSortFunction(): (a: any, b: unknown) => number {
    return this._sort;
  }
  ...
  public static by(orders: readonly SortOrder[]): Sort;

  public static by(property1: string, ...properties: readonly string[]): Sort;
  ...
  public static by(order1: SortOrder, ...orders: readonly SortOrder[]): Sort;
  ...
  public static by(
    ...
    if (isSortDirection(arg1)) {
      if (isStringArray(arg2)) {
        return new Sort(
          map(arg2, (item: string): SortOrder => new SortOrder(arg1, item)),
        );
      }
      throw new TypeError(
        `Invalid function signature: ${arg1} ${arg2.join(' ')}`,
      );
    }
    ...
    if (isStringArray(args)) {
      return new Sort(
        map(
          args,
          (item: string): SortOrder =>
            new SortOrder(Sort.DEFAULT_DIRECTION, item),
        ),
      );
    }
