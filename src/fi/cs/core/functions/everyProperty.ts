...
export function assertEveryProperty<K extends keyof any = string, T = any>(
  ...
  if (
    isItem !== undefined &&
    !everyValue<T>(value, (item: T): boolean => isItem(item))
  ) {
    const valueArray: T[] = values(value);
    const itemIndex: number = findIndex(
      valueArray,
      (item: T): boolean => !isItem(item),
    );
    const itemKey: string | symbol | number = keys(value)[itemIndex];
    const itemValue: T = valueArray[itemIndex];
    if (explainValue) {
      throw new TypeError(
        `Property "${itemKey}": value not correct: ${explainValue(itemValue)}`,
      );
    } else {
      throw new TypeError(
        `Property "${itemKey}": value not correct: ${JSON.stringify(itemValue, null, 2)}`,
      );
    }
  }

  const key: string | symbol | number | undefined = find(
    keys(value),
    (key: symbol | string | number): boolean => !isKeyTest(key),
  );

  if (explainKey) {
    throw new TypeError(
      `Property "${String(key)}": key was not correct: ${explainKey(key)}`,
    );
  } else {
    throw new TypeError(
      `Property "${String(key)}": key was not correct: ${JSON.stringify(key, null, 2)}`,
    );
  }
}
