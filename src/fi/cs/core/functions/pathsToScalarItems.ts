 ...
 * @nosideeffects
 */
export function pathsToScalarItems(value: any, baseKey: string = ''): string[] {
  if (isArray(value)) {
    let allKeys: string[] = [];
    forEach(value, (item: any, itemIndex: number) => {
  ...
  if (isObject(value)) {
    let allKeys: string[] = [];
    forEach(keys(value), (itemKey: string) => {
      const itemFullKey = `${baseKey}${baseKey ? '.' : ''}${itemKey}`;

      const item: unknown = value[itemKey];
