...
export class PermissionService<T extends string = string> {
   ...
   * @param manager
   */
  public constructor(manager: PermissionManager<T>) {
  ...
  public async checkEntityPermission(
    ...
    LOG.debug(
      `checkEntityPermission: Checking entity "${entityId}" against permissions "${checkPermissions.join('|')}"`,
    );
    const entityPermissions = await this.getEntityPermissionList(
      entityId,
      targetId,
    );
    LOG.debug(
      `checkEntityPermission: Checking entity "${entityId}" [${entityPermissions.join('|')}] against permissions [${checkPermissions.join('|')}]`,
    );
    ...
    LOG.info(
      `checkEntityPermission: Checking entity "${entityId}" [${entityPermissions.join('|')}] against result [${checkPermissions.join('|')}]: `,
      result,
    );
    return result;
  }
