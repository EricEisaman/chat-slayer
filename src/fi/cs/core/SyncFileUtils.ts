...
export class SyncFileUtils {
  ...
  static readTextFile(sourceFile: string): string {
  ...
  static readJsonFile(sourceFile: string): ReadonlyJsonAny {
    return JSON.parse(SyncFileUtils.readTextFile(sourceFile));
  }

  static writeTextFile(targetPath: string, targetDataString: string) {
    fs.writeFileSync(targetPath, targetDataString, {encoding: 'utf8'});
  }

  static writeJsonFile(targetPath: string, targetData: ReadonlyJsonAny) {
  ...
  static copyTextFileWithReplacementsIfMissing(
    ...
    if (!SyncFileUtils.fileExists(toFile)) {
      SyncFileUtils.copyTextFileWithReplacements(
        sourceFile,
        toFile,
        replacements,
      );
