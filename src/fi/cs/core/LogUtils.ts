...
export class LogUtils {
  ...
  public static stringifyValue(value: unknown): string {
    try {
      ...
      if (value && isObject(value)) {
        ...
        if (
          has(value, 'toString') &&
          isFunction(objectKey(value, 'toString'))
        ) {
  ...
  public static splitStringValue(
    ...
    if (maxSize < prefix.length + suffix.length + 1) {
      throw new TypeError(
        `Max size to splitStringValue() must be greater than the length of prefix and suffix (${maxSize} < ${prefix.length + suffix.length})`,
      );
    }
    ...
    while (str.length) {
      ...
      const chunkSuffix: string =
        str.length <= maxSize - chunkPrefix.length ? '' : suffix;
      const chunkSize: number =
        maxSize - chunkPrefix.length - chunkSuffix.length;
  ...
  public static splitStringArray(
    ...
    return reduce(
      ...
      (prevRows: string[], row: string): string[] => {
        const chunks = LogUtils.splitStringValue(
          row + linebreak,
          maxSize,
          prefix,
          suffix,
  ...
  public static mergeStringArray(
    ...
    const rows = map(value, (item: string): string => item);
    const chunks: string[] = [];
