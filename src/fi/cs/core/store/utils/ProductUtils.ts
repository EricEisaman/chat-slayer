...
import {
  getProductFeatureCategoryTitleTranslationToken,
  getProductFeatureTitleTranslationToken,
} from '../constants/storeTranslation';
...
export class ProductUtils {
    ...
    return [
      ...
      ...map(
        filter(productList, (p): boolean => {
          return map(p?.prices ?? [], pp => pp?.type).includes(
            selectedPriceType,
          );
        }),
        (item: Product, index: number): ProductTableItemModel => {
          const price: ProductPrice | undefined = find(
            item?.prices,
            pp => pp?.type === selectedPriceType,
          );
          if (!price)
            throw new TypeError(
              `Price was not found for ${selectedPriceType}. This should not happen.`,
            );
          return {
            id: index + 1,
            title: item.title,
  ...
  public static createProductTableItemDataModelList(
    ...
    return map(
      categoryList,
      (
        categoryId: ProductFeatureCategory,
        index: number,
      ): ProductTableItemDataModel => {
        const featureIdList: ProductFeatureId[] = has(featureIdMap, categoryId)
          ? featureIdMap[categoryId]
          : [];
        return {
          id: index,
          mainTitle: getProductFeatureCategoryTitleTranslationToken(categoryId),
          title: map(featureIdList, (id: ProductFeatureId) =>
            getProductFeatureTitleTranslationToken(id),
          ),
          product: map(productList, (item: Product): readonly any[] => {
            return map(featureIdList, (featureId: ProductFeatureId) => {
              return (
                find(
                  item.features,
                  (f: unknown) => objectKey(f, 'id') === featureId,
                ) ?? ''
              );
            });
          }),
  ...
  public static createUniquePriceTypeList(
    ...
    return reduce(
      list,
      (a: ProductPriceType[], item: ProductTableItemModel) =>
        uniq([...a, ...item.priceTypeOptions]),
      [],
    );
  }
  ...
  public static calculateCompositeProductFromOptions(
    ...
    const compositeSelections:
      | readonly CompositeProductSelection[]
      | undefined = model?.composite;

    // Check if this is a composite product
    if (!compositeSelections) {
      LOG.warn(
        "Warning! This model doesn't seem to be a composite product. Passing the product on without any changes.",
      );
      return model;
    }
  ...
  public static getBestMatchingNumericCompositeProductOption(
    ...
    const suitableOptions: CompositeProductOption[] = filter(
      list,
      (option: CompositeProductOption) =>
        isNumber(option.value) && preferredValue <= option.value,
    );
    if (suitableOptions.length === 0) return undefined;
    suitableOptions.sort(
      ProductUtils.sortCompositeProductOptionsByNumericValue,
    );
    return suitableOptions[0];
  }

  public static sortCompositeProductOptionsByNumericValue(
    a: CompositeProductOption,
    b: CompositeProductOption,
  ): number {
