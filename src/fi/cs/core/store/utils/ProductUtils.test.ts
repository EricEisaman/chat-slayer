...
import {
  ...
  createCompositeProductOption,
} from '../types/product/CompositeProductOption';
...
describe('ProductUtils', () => {
  describe('#calculateCompositeProductFromOptions', () => {
    it('can composite non-composable product', () => {
      const item: Product = createProduct(
        ...
        [createProductPrice(5, 0.24, ProductPriceType.YEARLY)],
      );

      expect(
        ProductUtils.calculateCompositeProductFromOptions(item, {}, []),
      ).toStrictEqual(item);
    });
    ...
    it.skip('can composite composable product', () => {
      ...
      const extraDiskProduct: Product = createProduct(
        ...
        [createProductPrice(10, 0.24, ProductPriceType.YEARLY)],
      );
      ...
      const compositeProduct =
        ProductUtils.calculateCompositeProductFromOptions(
          item,
          preferredOptions,
          [shellProduct1, extraDiskProduct],
        );
      ...
      expect(compositeProduct?.prices[0]?.type).toStrictEqual(
      ...
      expect(compositeProduct?.features[0]?.id).toStrictEqual(
        ProductFeatureId.DISK_SIZE,
      );
  ...
  describe('#sortCompositeProductOptionsByNumericValue', () => {
    it('can find exact matching product option', () => {
      ...
      const option: CompositeProductOption | undefined =
        ProductUtils.getBestMatchingNumericCompositeProductOption(5, list);
    ...
    it('can find best matching product option', () => {
      ...
      const option: CompositeProductOption | undefined =
        ProductUtils.getBestMatchingNumericCompositeProductOption(15, list);
    ...
    it('can find best matching product option from multiple options (first)', () => {
      ...
      const option: CompositeProductOption | undefined =
        ProductUtils.getBestMatchingNumericCompositeProductOption(10, list);
    ...
    it('can returns undefined if no match can be found', () => {
      ...
      const option: CompositeProductOption | undefined =
        ProductUtils.getBestMatchingNumericCompositeProductOption(100, list);
