...
export function parseInventoryState(
  value: unknown,
): InventoryState | undefined {
  switch (`${value}`.toUpperCase()) {
    case 'UNINITIALIZED':
      return InventoryState.UNINITIALIZED;
    case 'DRAFT':
      return InventoryState.DRAFT;
    case 'WAITING_APPROVAL':
      return InventoryState.WAITING_APPROVAL;
    case 'CHANGING':
      return InventoryState.CHANGING;
    case 'READY':
      return InventoryState.READY;
    case 'ERROR':
      return InventoryState.ERROR;
    default:
      return undefined;
  }
}

export function isInventoryStateOrUndefined(
  value: unknown,
): value is InventoryState | undefined {
  return isUndefined(value) || isInventoryState(value);
}

export function explainInventoryStateOrUndefined(value: unknown): string {
  return isInventoryStateOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['InventoryState', 'undefined']));
}
