...
export function parseNetworkType(value: unknown): NetworkType | undefined {
  return parseEnum(NetworkType, value) as NetworkType | undefined;
}

export function isNetworkTypeOrUndefined(
  value: unknown,
): value is NetworkType | undefined {
  return isUndefined(value) || isNetworkType(value);
}

export function explainNetworkTypeOrUndefined(value: unknown): string {
  return isNetworkTypeOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['NetworkType', 'undefined']));
}
