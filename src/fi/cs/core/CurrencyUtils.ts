...
export class CurrencyUtils {
  public static stringifySum(sum: number): string {
  ...
  public static getSumWithDiscount(
    ...
    return discountPercent !== undefined &&
      discountPercent > 0 &&
      discountPercent <= 1
  ...
  public static roundByAccuracy(value: number, accuracy: number) {
  ...
  public static convertCurrencyAmount(
    ...
    if (toRate === undefined)
      throw new TypeError(`CurrencyService: To: No exchange rate found: ${to}`);
    const fromRate = get(rates, from);
    if (fromRate === undefined)
      throw new TypeError(
        `CurrencyService: From: No exchange rate found: ${from}`,
      );
    return CurrencyUtils.roundByAccuracy(
