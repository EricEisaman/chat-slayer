...
export class TwilioMessageClientImpl implements TwilioMessageClient {
  ...
  public static create(
    ...
    return new TwilioMessageClientImpl(
      ...
      accountSid,
      AuthorizationUtils.createBasicHeaderTokenWithUserAndPassword(
        accountSid,
        authToken,
      ),
      sender,
  ...
  public async sendSms(
    ...
    const bodyString: string = QueryParamUtils.stringifyQueryParamsOnly(
      body as unknown as QueryParams,
    );
    const headers = {
    ...
    if (!isTwilioMessageDTO(dto)) {
      ...
      throw new TypeError(
        `sendSms: Response was not TwilioMessageDTO: ${explainTwilioMessageDTO(dto)}`,
      );
    }
    return dto;
  }
