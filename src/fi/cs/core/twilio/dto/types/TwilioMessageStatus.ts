...
export function stringifyTwilioMessageStatus(
  value: TwilioMessageStatus,
): string {
  return stringifyEnum(TwilioMessageStatus, value);
}

export function parseTwilioMessageStatus(
  value: unknown,
): TwilioMessageStatus | undefined {
  return parseEnum(TwilioMessageStatus, value) as
    | TwilioMessageStatus
    | undefined;
}

export function isTwilioMessageStatusOrUndefined(
  value: unknown,
): value is TwilioMessageStatus | undefined {
...
export function explainTwilioMessageStatusOrUndefined(value: unknown): string {
  return isTwilioMessageStatusOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['TwilioMessageStatus', 'undefined']));
}
