...
import {
  createTwilioCreateMessageContent,
  isTwilioCreateMessageContent,
  explainTwilioCreateMessageContent,
  parseTwilioCreateMessageContent,
  isTwilioCreateMessageContentOrUndefined,
  explainTwilioCreateMessageContentOrUndefined,
} from './TwilioCreateMessageContent';

const mockMediaUrl: TwilioCreateMessageMediaUrl = {
  MediaUrl: 'http://example.com/image.jpg',
};
const mockMessageBody: TwilioCreateMessageBody = {
  Body: 'This is a message body',
};
const mockMessageContentSid: TwilioCreateMessageContentSid = {
  ContentSid: 'SMXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX',
};
...
describe('TwilioCreateMessageContent', () => {
  describe('createTwilioCreateMessageContent function', () => {
    it('returns the same content that was passed in', () => {
      expect(createTwilioCreateMessageContent(mockMediaUrl)).toEqual(
        mockMediaUrl,
      );
      expect(createTwilioCreateMessageContent(mockMessageBody)).toEqual(
        mockMessageBody,
      );
      expect(createTwilioCreateMessageContent(mockMessageContentSid)).toEqual(
        mockMessageContentSid,
      );
    });
  });
  ...
  describe('explainTwilioCreateMessageContent function', () => {
    it('returns a correct explanation for valid and invalid content', () => {
      expect(explainTwilioCreateMessageContent(mockMediaUrl)).toEqual(
        explainOk(),
      );
      expect(explainTwilioCreateMessageContent(mockInvalid)).toEqual(
        explainNot('TwilioCreateMessageContent'),
      );
    });
  });
  ...
  describe('parseTwilioCreateMessageContent function', () => {
    it('returns the content for valid content and undefined for invalid content', () => {
      expect(parseTwilioCreateMessageContent(mockMediaUrl)).toEqual(
        mockMediaUrl,
      );
  ...
  describe('explainTwilioCreateMessageContentOrUndefined function', () => {
    it('returns a correct explanation for valid content, undefined and invalid content', () => {
      expect(
        explainTwilioCreateMessageContentOrUndefined(mockMediaUrl),
      ).toEqual(explainOk());
      expect(
        explainTwilioCreateMessageContentOrUndefined(mockUndefined),
      ).toEqual(explainOk());
      expect(explainTwilioCreateMessageContentOrUndefined(mockInvalid)).toEqual(
        explainNot(explainOr(['TwilioCreateMessageContent', 'undefined'])),
      );
    });
  });
});
