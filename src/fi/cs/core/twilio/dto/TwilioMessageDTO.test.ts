...
import {
  createTwilioMessageDTO,
  explainTwilioMessageDTO,
  explainTwilioMessageDTOOrUndefined,
  isTwilioMessageDTO,
  isTwilioMessageDTOOrUndefined,
  parseTwilioMessageDTO,
} from './TwilioMessageDTO';

// Assuming a mock ReadonlyJsonObject that fits your requirements
const mockSubresourceUris: ReadonlyJsonObject = {
  media:
...
describe('TwilioMessageDTO', () => {
  ...
  describe('explainTwilioMessageDTO function', () => {
    it('returns a correct explanation for valid and invalid DTOs', () => {
      ...
      expect(explainTwilioMessageDTO(mockInvalid)).toContain(
        'property "account_sid" not string',
      );
    });
  });
  ...
  describe('parseTwilioMessageDTO function', () => {
    it('returns the DTO for valid DTOs and undefined for invalid values', () => {
      expect(parseTwilioMessageDTO(mockTwilioMessage)).toEqual(
        mockTwilioMessage,
      );
  ...
  describe('explainTwilioMessageDTOOrUndefined function', () => {
    it('returns a correct explanation for valid DTOs, undefined and invalid values', () => {
      ...
      expect(explainTwilioMessageDTOOrUndefined(mockTwilioMessage)).toEqual(
        explainOk(),
      );
      expect(explainTwilioMessageDTOOrUndefined(undefined)).toEqual(
        explainOk(),
      );
      expect(explainTwilioMessageDTOOrUndefined(mockInvalid)).toEqual(
