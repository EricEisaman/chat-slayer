...
import {
  ...
  explainCreatePaymentRequestDTO,
  isCreatePaymentRequestDTO,
} from './dtos/CreatePaymentRequestDTO';
...
import {
  ...
  explainPaytrailCreatePaymentDTO,
  isPaytrailCreatePaymentDTO,
} from './dtos/PaytrailCreatePaymentDTO';
import {LogService} from '../LogService';
import {
  explainPaytrailPaymentDTO,
  isPaytrailPaymentDTO,
  PaytrailPaymentDTO,
} from './dtos/PaytrailPaymentDTO';
...
import {
  ...
  isPaytrailPaymentProviderListDTO,
  PaytrailPaymentProviderListDTO,
} from './dtos/PaytrailPaymentProviderListDTO';
import {
  explainPaytrailProvider,
  isPaytrailProvider,
} from './types/PaytrailProvider';
import {explainArrayOf, isArrayOf} from '../types/Array';
import {
  isPaytrailLimitedProvider,
  PaytrailLimitedProvider,
} from './types/PaytrailLimitedProvider';
...
export class HttpPaytrailClient implements PaytrailClient {
  ...
  private constructor(account: string, secret: string, url: string) {
  ...
  public async getPayment(transactionId: string): Promise<PaytrailPaymentDTO> {
    ...
    if (!isPaytrailPaymentDTO(result)) {
      throw new Error(
        `Response was not PaytrailPaymentDTO: ${explainPaytrailPaymentDTO(result)}`,
      );
    }
  ...
  public async getMerchantsPaymentProviders(
    ...
    const result = parseJson(resultString);
    if (
      !isArrayOf<PaytrailLimitedProvider>(result, isPaytrailLimitedProvider)
    ) {
      throw new Error(
        `Response was not array of PaytrailLimitedProvider: ${explainArrayOf<PaytrailLimitedProvider>('PaytrailProvider', explainPaytrailProvider, result, isPaytrailProvider)}: ${JSON.stringify(result, null, 2)}`,
      );
    }
  ...
  public async getMerchantsGroupedPaymentProviders(
    ...
    if (!isPaytrailPaymentProviderListDTO(result)) {
      throw new Error(
        `Response was not PaytrailPaymentProviderListDTO: ${explainPaytrailPaymentProviderListDTO(result)}: ${JSON.stringify(result, null, 2)}`,
      );
    }
  ...
  private _createHeaders(
    ...
    return {
      ...
      'checkout-nonce': HttpPaytrailClient._hash(
        'sha256',
        Date.now().toString(),
      ),
  ...
  private async _createPayment(
    ...
    headers['signature'] = this._calculateHmac(
      this._secret,
      headers,
      bodyString,
    );
    ...
    if (!isPaytrailCreatePaymentDTO(result)) {
      throw new Error(
        `Response was not PaytrailCreatePaymentDTO: ${explainPaytrailCreatePaymentDTO(result)}`,
      );
    }
  ...
  private _getCreatePaymentRequestBody(
    ...
    const dto: CreatePaymentRequestDTO = {
      ...
      items:
        items && items.length
          ? map(items, (item: PaytrailItem) => item)
          : undefined,
      redirectUrls,
    ...
    if (!isCreatePaymentRequestDTO(dto)) {
      throw new TypeError(
        `Invalid input CreatePaymentRequestDTO: ${explainCreatePaymentRequestDTO(dto)}`,
      );
    }
    return dto;
  }
  ...
  private _calculateHmac(
    ...
    const includedKeys = Object.keys(params).filter(key =>
      startsWith(key, 'checkout-'),
    );

    // Keys must be sorted alphabetically
    includedKeys.sort();

    const hmacPayload = includedKeys.map(key => `${key}:${params[key]}`);
