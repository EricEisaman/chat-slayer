...
import {
  explainPaytrailPaymentMethodGroupData,
  isPaytrailPaymentMethodGroupData,
  PaytrailPaymentMethodGroupData,
} from '../types/PaytrailPaymentMethodGroupData';
...
export function isPaytrailCreatePaymentDTO(
  value: unknown,
): value is PaytrailCreatePaymentDTO {
  return (
    ...
    isString(objectKey(value, 'terms')) &&
    isArrayOf<PaytrailPaymentMethodGroupData>(
      objectKey(value, 'groups'),
      isPaytrailPaymentMethodGroupData,
    ) &&
    isString(objectKey(value, 'reference')) &&
    isArrayOf<PaytrailProvider>(
      objectKey(value, 'providers'),
      isPaytrailProvider,
    ) &&
    isReadonlyJsonObjectOrUndefined(objectKey(value, 'customProviders'))
  );
}

export function explainPaytrailCreatePaymentDTO(value: unknown): string {
  return explain([
    ...
    explainProperty('terms', explainString(objectKey(value, 'terms'))),
    explainProperty(
      'groups',
      explainArrayOf<PaytrailPaymentMethodGroupData>(
        'PaytrailPaymentMethodGroupData',
        explainPaytrailPaymentMethodGroupData,
        objectKey(value, 'groups'),
        isPaytrailPaymentMethodGroupData,
    ...
    explainProperty(
      'providers',
      explainArrayOf<PaytrailProvider>(
        'PaytrailProvider',
        explainPaytrailProvider,
        objectKey(value, 'providers'),
        isPaytrailProvider,
    ...
    explainProperty(
      'customProviders',
      explainReadonlyJsonObjectOrUndefined(objectKey(value, 'customProviders')),
    ),
  ]);
}

export function parsePaytrailCreatePaymentDTO(
  value: unknown,
): PaytrailCreatePaymentDTO | undefined {
  if (isPaytrailCreatePaymentDTO(value)) return value;
  return undefined;
}

export function isPaytrailCreatePaymentDTOOrUndefined(
  value: unknown,
): value is PaytrailCreatePaymentDTO | undefined {
  return isUndefined(value) || isPaytrailCreatePaymentDTO(value);
}

export function explainPaytrailCreatePaymentDTOOrUndefined(
  value: unknown,
): string {
  return isPaytrailCreatePaymentDTOOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['PaytrailCreatePaymentDTO', 'undefined']));
}
