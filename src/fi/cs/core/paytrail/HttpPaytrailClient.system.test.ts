...
// @ts-ignore
import {CsNode} from '../../node/CsNode';
// @ts-ignore
  ...
  describe('HttpPaytrailClient', () => {
    ...
    describe('#createPayment', () => {
      it('should return a successful response with valid input', async () => {
        ...
        expect(payment?.href).toBe(
          `https://pay.paytrail.com/pay/${payment?.transactionId}`,
        );
        expect(payment?.terms).toBe(
        ...
        expect(
          every(
            ...
            (item: unknown): boolean =>
              isNonEmptyString(item?.name) && isNonEmptyString(item?.value),
          ),
        ).toBe(true);
        ...
        expect(payment?.providers[0]?.icon).toBe(
          'https://resources.paytrail.com/images/payment-method-logos/walley.png',
        );
        expect(payment?.providers[0]?.id).toBe('collectorb2c');
        expect(payment?.providers[0]?.name).toBe('Collector');
        expect(payment?.providers[0]?.svg).toBe(
          'https://resources.paytrail.com/images/payment-method-logos/walley.svg',
        );
        expect(payment?.providers[0]?.url).toBe(
          `https://pay.paytrail.com/payments/${payment?.transactionId}/collectorb2c/`,
        );
    ...
    describe('#getPayment', () => {
      it('should return a successful response with valid input', async () => {
        ...
        expect(fetchedPayment?.href).toBe(
          `https://pay.checkout.fi/pay/${payment?.transactionId}`,
        );
