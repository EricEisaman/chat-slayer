import {
  explainPaytrailStatus,
  explainPaytrailStatusOrUndefined,
  isPaytrailStatus,
  isPaytrailStatusOrUndefined,
  parsePaytrailStatus,
  PaytrailStatus,
  stringifyPaytrailStatus,
} from './PaytrailStatus';
...
describe('PaytrailStatus functions', () => {
  ...
  const validStatusTexts = ['NEW', 'OK', 'FAIL', 'PENDING', 'DELAYED'];
  ...
  describe('explainPaytrailStatus', () => {
    ...
    it('should return an explanation for invalid status', () => {
      expect(explainPaytrailStatus(invalidStatus)).toContain(
        'incorrect enum value "invalid_status" for PaytrailStatus: Accepted values new, ok, fail, pending, delayed',
      );
    });
  });
  ...
  describe('stringifyPaytrailStatus', () => {
    validStatuses.forEach((status, index) => {
      it(`should correctly stringify "${status}"`, () => {
        expect(stringifyPaytrailStatus(status)).toEqual(
          validStatusTexts[index],
        );
      });
    });
  });
  ...
  describe('explainPaytrailStatusOrUndefined', () => {
    ...
    it('should return an explanation for invalid status', () => {
      expect(explainPaytrailStatusOrUndefined(invalidStatus)).toContain(
        'not PaytrailStatus or undefined',
      );
    });
  });
});
