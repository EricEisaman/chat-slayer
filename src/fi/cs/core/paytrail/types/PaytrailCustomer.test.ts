import {
  ...
  isPaytrailCustomer,
  parsePaytrailCustomer,
  PaytrailCustomer,
} from './PaytrailCustomer';
...
describe('PaytrailCustomer functions', () => {
  ...
  describe('isPaytrailCustomer', () => {
    ...
    it('should return false for invalid objects', () => {
      ...
      expect(isPaytrailCustomer({...validCustomer, unknownProp: 'test'})).toBe(
        false,
      );
    });
  });
  ...
  describe('explainPaytrailCustomer', () => {
    ...
    it('should return an explanation for invalid objects', () => {
      expect(explainPaytrailCustomer({...validCustomer, email: 123})).toContain(
        'property "email"',
      );
      expect(
        explainPaytrailCustomer({...validCustomer, unknownProp: 'test'}),
      ).toContain('had extra properties: unknownProp');
    });
  });
  ...
  describe('parsePaytrailCustomer', () => {
    ...
    it('should return undefined for invalid objects', () => {
      expect(
        parsePaytrailCustomer({...validCustomer, email: 123}),
      ).toBeUndefined();
    });
  });
});
