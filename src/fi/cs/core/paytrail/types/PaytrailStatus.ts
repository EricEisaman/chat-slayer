...
export function parsePaytrailStatus(
  value: unknown,
): PaytrailStatus | undefined {
  return parseEnum(PaytrailStatus, value) as PaytrailStatus | undefined;
}

export function isPaytrailStatusOrUndefined(
  value: unknown,
): value is PaytrailStatus | undefined {
  return isUndefined(value) || isPaytrailStatus(value);
}

export function explainPaytrailStatusOrUndefined(value: unknown): string {
  return isPaytrailStatusOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['PaytrailStatus', 'undefined']));
}
