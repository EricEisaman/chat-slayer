...
export function isPaytrailFormField(
  value: unknown,
): value is PaytrailFormField {
  return (
    isRegularObject(value) &&
    hasNoOtherKeysInDevelopment(value, ['name', 'value']) &&
    isString(objectKey(value, 'name')) &&
    isString(objectKey(value, 'value'))
  );
}

export function explainPaytrailFormField(value: unknown): string {
  return explain([
    explainRegularObject(value),
    explainNoOtherKeysInDevelopment(value, ['name', 'value']),
    explainProperty('name', explainString(objectKey(value, 'name'))),
    explainProperty('value', explainString(objectKey(value, 'value'))),
  ]);
}

export function parsePaytrailFormField(
  value: unknown,
): PaytrailFormField | undefined {
  if (isPaytrailFormField(value)) return value;
  return undefined;
}

export function isPaytrailFormFieldOrUndefined(
  value: unknown,
): value is PaytrailFormField | undefined {
...
export function explainPaytrailFormFieldOrUndefined(value: unknown): string {
  return isPaytrailFormFieldOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['PaytrailFormField', 'undefined']));
}
