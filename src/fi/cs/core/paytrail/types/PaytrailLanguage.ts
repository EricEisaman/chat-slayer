...
export function parsePaytrailLanguage(
  value: unknown,
): PaytrailLanguage | undefined {
  return parseEnum(PaytrailLanguage, value) as PaytrailLanguage | undefined;
}

export function isPaytrailLanguageOrUndefined(
  value: unknown,
): value is PaytrailLanguage | undefined {
  return isUndefined(value) || isPaytrailLanguage(value);
}

export function explainPaytrailLanguageOrUndefined(value: unknown): string {
  return isPaytrailLanguageOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['PaytrailLanguage', 'undefined']));
}

export function getPaytrailLanguageFromLanguage(
  value: Language,
): PaytrailLanguage {
  switch (value) {
    case Language.FINNISH:
      return PaytrailLanguage.FI;
    case Language.ENGLISH:
      return PaytrailLanguage.EN;
    default:
      return PaytrailLanguage.EN;
  }
}
