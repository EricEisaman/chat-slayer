import {
  createPaytrailComission,
  explainPaytrailComission,
  isPaytrailComission,
  parsePaytrailComission,
...
describe('PaytrailComission', () => {
  ...
  describe('createPaytrailComission', () => {
    it('should create a valid PaytrailComission object', () => {
      const result = createPaytrailComission(
        validComission.merchant,
        validComission.amount,
      );
  ...
  describe('isPaytrailComission', () => {
    ...
    it('should return false for invalid objects', () => {
      expect(
        isPaytrailComission({...validComission, unknownProperty: 'Test'}),
      ).toBe(false);
  ...
  describe('parsePaytrailComission', () => {
    ...
    it('should return undefined if input is invalid', () => {
      expect(
        parsePaytrailComission({...validComission, unknownProperty: 'Test'}),
      ).toBeUndefined();
  ...
  describe('explainPaytrailComission', () => {
    ...
    it('should return an explanation for invalid objects', () => {
      expect(explainPaytrailComission({})).toContain(
        'property "merchant" not string',
      );
      expect(
        explainPaytrailComission({...validComission, unknownProperty: 'Test'}),
      ).toContain('unknownProperty');
      expect(explainPaytrailComission({merchant: 123, amount: 250})).toContain(
        'property "merchant" not string',
      );
      expect(
        explainPaytrailComission({merchant: '695874', amount: '250'}),
      ).toContain('property "amount" not number');
    });
  });
});
