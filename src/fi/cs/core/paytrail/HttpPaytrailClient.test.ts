...
const TEST_CREATE_PARAMS = {
  ...
  'checkout-nonce':
    'b800e31d15e9262ce69c23a21478ad38494775cf36a895e4fad8075c2e535995',
...
describe('HttpPaytrailClient', () => {
  ...
  let postTextSpy: jest.SpiedFunction<(...args: unknown) => any>;
  let getTextSpy: jest.SpiedFunction<(...args: unknown) => any>;
  ...
  beforeAll(() => {
    ...
    postTextSpy = jest
      .spyOn(RequestClientImpl, 'postText')
      .mockResolvedValue(JSON.stringify(TEST_CREATE_RESPONSE));
    getTextSpy = jest
      .spyOn(RequestClientImpl, 'getText')
      .mockResolvedValue(JSON.stringify(TEST_FETCH_RESPONSE));
  });

  beforeEach(() => {
    client = HttpPaytrailClient.create(
      'mockAccount',
      'mockSecret',
      'http://mockUrl',
    );
  ...
  describe('createPayment', () => {
    it('should return a successful response with valid input', async () => {
      ...
      expect((RequestClientImpl.postText as any).mock.calls[0][0]).toBe(
        'http://mockUrl/payments',
      );
      expect(
        parseJson((RequestClientImpl.postText as any).mock.calls[0][1]),
      ).toStrictEqual(TEST_CREATE_REQUEST);
      expect(
        (RequestClientImpl.postText as any).mock.calls[0][2],
      ).toStrictEqual(TEST_CREATE_PARAMS);
    });
