 ...
 *
 * See also
 * [MatrixCrudRepository](https://github.com//fi.cs.matrix/blob/main/MatrixCrudRepository.ts)
 */
...
> implements SimpleRepository<T> {
  ...
  public constructor(
    ...
    this._explainT =
      ...
      ((value: unknown): string =>
        isT(value) ? explainOk() : explainNot(this._tName));
  }

  public async getAll(): Promise<readonly SimpleRepositoryEntry<T>[]> {
    return map(
      this._items,
      (item): SimpleRepositoryEntry<T> => ({
        id: item.id,
  ...
  public async getAllByProperty(
    ...
    const filteredItems: readonly SimpleMemoryItem<T>[] = filter(
      items,
      (item: SimpleMemoryItem<T>): boolean =>
  ...
  public async createItem(
    ...
    if (existingItem)
      throw new Error(`MemoryRepository: ID "${id}" was not unique`);
  ...
  public async findById(
    ...
    return {
      ...
      deleted: item.deleted,
      members:
        includeMembers && item.members?.length
          ? map(item.members, id => ({id}))
          : undefined,
    };
  }
  ...
  public async findByIdAndUpdate(
    ...
    if (rItem === undefined)
      throw new TypeError(
        `MemoryRepository.findByIdAndUpdate: Could not find item for "${id}"`,
      );
  ...
  public async deleteById(id: string): Promise<SimpleRepositoryEntry<T>> {
    const items = remove(
      [...this._items] as SimpleMemoryItem<T>[],
      item => item.id === id,
    );
  ...
  public async inviteToItem(
    ...
    const nextItem = {
      ...prevItem,
      invited: filter(
        uniq(concat([], prevItem?.invited ?? [], members)),
  ...
  private _setItemByIndex(itemIndex: number, newItem: SimpleMemoryItem<T>) {
    this._items = map(
      this._items,
      (item: SimpleMemoryItem<T>, i: number): SimpleMemoryItem<T> =>
  ...
  public async deleteByIdList(
    ...
    if (!this.isRepositoryEntryList(results)) {
      throw new TypeError(
        `MemoryRepository.getSome: Illegal data from database: Not RepositoryEntryList: ${this.explainRepositoryEntryList(
          results,
        )}`,
      );
    }
