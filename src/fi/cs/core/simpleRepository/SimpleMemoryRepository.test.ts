...
describe('SimpleMemoryRepository', () => {
  describe('#construct', () => {
    it('can create memoryrepository', () => {
      const memoryrepository = new SimpleMemoryRepository(
        isStoredRepositoryItem,
      );
  ...
  describe('#getAll', () => {
    const memoryRepository: SimpleMemoryRepository<StoredTestRepositoryItem> =
      new SimpleMemoryRepository<StoredTestRepositoryItem>(
        isStoredRepositoryItem,
      );
    ...
    it('can create item', async () => {
      const item: SimpleRepositoryEntry<any> =
        await memoryRepository.createItem(test);
      ...
      const item2: SimpleRepositoryEntry<any> =
        await memoryRepository.createItem(test2);
    });
    ...
    it('can find items: getSome, findById, getAllByProperty, findByProperty', async () => {
      ...
      // @ts-ignore @TODO: Why not used?
      const item3: SimpleRepositoryEntry<any> =
        await memoryRepository.createItem(test2);

      //getAllByProperty
      const propertyList = await memoryRepository.getAllByProperty(
        'target',
        item?.data.target,
      );
      ...
      //findByProperty
      const itemUnique: SimpleRepositoryEntry<any> =
        await memoryRepository.createItem(uniqueTest);
      const propertyItemUnique = await memoryRepository.findByProperty(
        'target',
        itemUnique?.data.target,
      );
      ...
      const propertyItemUniqueId = await memoryRepository.findByProperty(
        'id',
        itemUnique?.data.id,
      );
  ...
  describe('modifications', () => {
    ...
    const modificationData = {id: '11', target: 'modification'};
    ...
    it('can update item by it id', async () => {
      const item4: SimpleRepositoryEntry<any> =
        await memoryrepository.createItem(test);
      const modification: SimpleRepositoryEntry<any> =
        await memoryrepository.createItem(modificationData);
      ...
      //update
      updatedItem = await memoryrepository.update(id, test);
      expect(updatedItem?.id).toBe(id);
  ...
  describe('delete operations', () => {
    ...
    it('can delete item by it id', async () => {
      const item5: SimpleRepositoryEntry<any> =
        await memoryrepository.createItem(test);
      id = item5?.id;
