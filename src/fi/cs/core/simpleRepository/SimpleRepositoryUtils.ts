import {
  explainRepositoryEntry,
  isRepositoryEntry,
  SimpleRepositoryEntry,
} from './types/SimpleRepositoryEntry';
...
export class SimpleRepositoryUtils {
  public static filterLatest<T>(
    ...
    return values(
      reduce(
        list,
        (
          cache: {[key: string]: SimpleRepositoryEntry<T>},
          item: SimpleRepositoryEntry<T>,
        ): {[key: string]: SimpleRepositoryEntry<T>} => {
  ...
  public static isRepositoryEntryList<T extends SimpleStoredRepositoryItem>(
    ...
    return isArrayOf(list, (item: SimpleRepositoryEntry<T>): boolean =>
      isRepositoryEntry<T>(item, isT),
    );
  }

  public static explainRepositoryEntryList<
  ...
  >(
    ...
    return explainArrayOf(
      tName,
      (item: SimpleRepositoryEntry<T>): string =>
        explainRepositoryEntry(item, explainT),
      list,
