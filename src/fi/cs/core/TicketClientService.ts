...
import {
  isTicketListDTO,
  TicketListDTO,
} from './store/types/ticket/TicketListDTO';
...
export class TicketClientService {
  ...
  private static _getTicketPath = (ticketId: string): string =>
    `${TicketClientService._ticketListPath}/${encodeURIComponent(ticketId)}`;
  ...
  public static async addTicket(
    ...
    const response: ResponseEntity<JsonAny | undefined> | undefined =
      await HttpService.postJsonEntity(
  ...
  private static _createHeaders(sessionToken: string): {[key: string]: string} {
    return {
      [this._authorizationHeaderName]:
        AuthorizationUtils.createBearerHeader(sessionToken),
    };
  }
}
