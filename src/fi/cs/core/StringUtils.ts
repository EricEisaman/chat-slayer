...
const ACCEPTED_KEYWORD_CHARACTERS =
  'QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm._1234567890';
const ACCEPTED_START_KEYWORD_CHARACTERS =
...
export class StringUtils {
  ...
  public static isValidKeyword(variableKey: string): boolean {
    ...
    return every(variableKey, (item: string) =>
      ACCEPTED_KEYWORD_CHARACTERS.includes(item),
    );
  }
  ...
  public static processVariablesInString(
    ...
    if (!isFunction(resolveVariable)) {
      resolver = (key: string): ReadonlyJsonAny =>
        get(resolveVariable, key, defaultValue) as ReadonlyJsonAny;
    } else {
    ...
    if (startsWith(input, variablePrefix) && endsWith(input, variableSuffix)) {
      let variableKey = input.substr(
        variablePrefix.length,
        input.length - variablePrefix.length - variableSuffix.length,
      );
    ...
    while (index >= 0 && index < input.length) {
      ...
      if (index < 0) {
        ...
        const keyNameEndIndex = input.indexOf(
          variableSuffix,
          keyNameStartIndex,
        );
        if (keyNameEndIndex < 0) {
          throw new TypeError(
            `Parse error near "${input.substr(keyTokenStartIndex).substr(0, 20)}". End of variable not detected.`,
          );
        }

        const keyTokenEndIndex = keyNameEndIndex + variableSuffix.length;

        const variableKey = trim(
          input.substr(keyNameStartIndex, keyNameEndIndex - keyNameStartIndex),
        );
        ...
        if (!StringUtils.isValidKeyword(variableKey)) {
          ...
          const resolvedValue: ReadonlyJsonAny | undefined =
            resolver(variableKey);
  ...
  public static hasOnlyCharacters(
    ...
    return value.length === 0
      ? true
      : every(value, (char: string): boolean => acceptedChars.includes(char));
  }
  ...
  public static validateStringCharacters(
    ...
    return (
      (acceptedStartChars !== undefined
        ? StringUtils.startsWithCharacters(value, acceptedStartChars)
        : true) &&
      (acceptedMiddleChars !== undefined
        ? StringUtils.hasOnlyCharacters(
            value.substring(1, len - 1),
            acceptedMiddleChars,
          )
        : true) &&
      (acceptedEndChars !== undefined
        ? StringUtils.endsWithCharacters(value, acceptedEndChars)
        : true) &&
      len >= minLength &&
  ...
  public static truncateLongString(
    ...
    if (maxLength < suffix.length)
      throw new TypeError(
        'StringUtils.truncateLongString: maxLength must be greater than length of the suffix',
      );
    return value.length <= maxLength
      ? value
      : value.substring(0, maxLength - 3) + suffix;
  }
}
