...
import {
  isMailHogMessagesDTO,
  MailHogMessageListDTO,
} from './dto/MailHogMessageListDTO';
...
export class MailHogClient {
  ...
  public static create(url: string = MailHogClient._defaultUrl): MailHogClient {
    return new MailHogClient(url);
  }

  public constructor(url: string = MailHogClient._defaultUrl) {
    this._url = url;
  }
  ...
  public async getMessages(): Promise<MailHogMessageListDTO> {
    const result = await HttpService.getJson(
      `${this._url}${MAIL_HOG_API_GET_MESSAGES_PATH}`,
    );
  ...
  public async deleteMessages(): Promise<void> {
    await HttpService.deleteText(
      `${this._url}${MAIL_HOG_API_DELETE_MESSAGES_PATH}`,
    );
  }
}
