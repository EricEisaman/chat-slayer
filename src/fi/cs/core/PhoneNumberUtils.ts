...
export class PhoneNumberUtils {
  public static getTelLink(
    phone: string,
    countryCode: string = '+358',
  ): string {
    return `tel:${countryCode}${trimStart(`${phone ?? ''}`.replace(/[^+0-9]*/gi, ''), '0')}`;
  }

  public static getTelLabel(phone: string): string {
    return `${trimStart(`${phone ?? ''}`.replace(/[^+0-9 ]*/gi, ''))}`;
  }
  ...
  public static validatePhoneNumber(phone: string): boolean {
    ...
    return (
      rest.length >= (s === '+' ? 3 : 3) &&
      every(rest, (char: string): boolean => '0123456789'.includes(char))
    );
  }

  public static normalizePhoneAddress(
    value: string,
    ...
    return startsWith(value, '+')
      ? value
      : `${defaultPrefix}${startsWith(value, '0') ? value.substring(1) : value}`;
  }
}
