...
import {
  explainZendeskTicketListDTO,
  isZendeskTicketListDTO,
  ZendeskTicketListDTO,
} from './dto/ZendeskTicketListDTO';
import {isZendeskTicket, ZendeskTicket} from './dto/ZendeskTicket';
import {reduce} from '../functions/reduce';
import {filter} from '../functions/filter';
import {
  explainZendeskTicketDTO,
  isZendeskTicketDTO,
} from './dto/ZendeskTicketDTO';
import {isZendeskComment, ZendeskComment} from './dto/ZendeskComment';
import {
  explainZendeskCommentListDTO,
  isZendeskCommentListDTO,
  ZendeskCommentListDTO,
} from './dto/ZendeskCommentListDTO';
import {isZendeskAttachment} from './dto/ZendeskAttachment';
import {isArray} from '../types/Array';
import {objectKey} from '../types/RegularObject';
import {
  explainZendeskUserListDTO,
  isZendeskUserListDTO,
  ZendeskUserListDTO,
} from './dto/ZendeskUserListDTO';
...
import {
  ...
  ZendeskOrganization,
} from './dto/ZendeskOrganization';
import {
  explainZendeskOrganizationDTO,
  isZendeskOrganizationDTO,
} from './dto/ZendeskOrganizationDTO';
import {
  explainZendeskOrganizationListDTO,
  isZendeskOrganizationListDTO,
  ZendeskOrganizationListDTO,
} from './dto/ZendeskOrganizationListDTO';
...
import {
  explainZendeskGroupListDTO,
  isZendeskGroupListDTO,
  ZendeskGroupListDTO,
} from './dto/ZendeskGroupListDTO';
import {
  explainZendeskOrganizationMembershipListDTO,
  isZendeskOrganizationMembershipListDTO,
  ZendeskOrganizationMembershipListDTO,
} from './dto/ZendeskOrganizationMembershipListDTO';
import {
  isZendeskOrganizationMembership,
  ZendeskOrganizationMembership,
} from './dto/ZendeskOrganizationMembership';
import {
  explainZendeskOrganizationMembershipDTO,
  isZendeskOrganizationMembershipDTO,
} from './dto/ZendeskOrganizationMembershipDTO';
import {
  isZendeskUserIdentity,
  ZendeskUserIdentity,
} from './dto/ZendeskUserIdentity';
import {
  explainZendeskUserIdentityListDTO,
  isZendeskUserIdentityListDTO,
  ZendeskUserIdentityListDTO,
} from './dto/ZendeskUserIdentityListDTO';
import {
  isZendeskGroupMembership,
  ZendeskGroupMembership,
} from './dto/ZendeskGroupMembership';
import {
  explainZendeskGroupMembershipDTO,
  isZendeskGroupMembershipDTO,
} from './dto/ZendeskGroupMembershipDTO';
import {
  explainZendeskGroupMembershipListDTO,
  isZendeskGroupMembershipListDTO,
  ZendeskGroupMembershipListDTO,
} from './dto/ZendeskGroupMembershipListDTO';
import {
  isZendeskSuspendedTicket,
  ZendeskSuspendedTicket,
} from './dto/ZendeskSuspendedTicket';
import {
  explainZendeskSuspendedTicketListDTO,
  isZendeskSuspendedTicketListDTO,
  ZendeskSuspendedTicketListDTO,
} from './dto/ZendeskSuspendedTicketListDTO';
import {
  explainZendeskSuspendedTicketDTO,
  isZendeskSuspendedTicketDTO,
} from './dto/ZendeskSuspendedTicketDTO';
import {
  createDefaultHttpRetryPolicy,
  HttpRetryPolicy,
} from '../request/types/HttpRetryPolicy';
...
export class ZendeskClient {
  ...
  public static create(url: string, authorization: string): ZendeskClient {
  ...
  private _updateRateLimit(headers: Headers): void {
    ...
    const rateLimitRemaining = parseInteger(
      headers.getFirst('X-Rate-Limit-Remaining'),
    );
    const rateLimitResetInSec = parseInteger(
      headers.getFirst('Rate-Limit-Reset'),
    );

    if (
      isNumber(rateLimit) &&
      isNumber(rateLimitRemaining) &&
      isNumber(rateLimitResetInSec)
    ) {
      this._rateLimit = rateLimit;
      this._rateLimitRemaining = rateLimitRemaining;
      this._rateLimitReset = new Date().getTime() + rateLimitResetInSec * 1000;
      LOG.info(
        `Current rate limit ${rateLimitRemaining} / ${rateLimit} and reset in ${rateLimitResetInSec} s`,
      );
    }
  }
  ...
  private async _rateLimitCheck() {
    ...
    if (
      isNumber(rateLimit) &&
      isNumber(rateLimitRemaining) &&
      isNumber(rateLimitReset) &&
      rateLimitRemaining <= this._rateLimitReserve
    ) {
  ...
  public async getTicket(ticketId: number): Promise<ZendeskTicket | undefined> {
    const entity: ResponseEntity<JsonAny | undefined> | undefined =
      await this._getJsonEntityWithRateLimitCheck(
    ...
    if (!isZendeskTicketDTO(result)) {
      LOG.debug(
        `getTicket: Not ZendeskTicketDTO: ${explainZendeskTicketDTO(result)}`,
      );
  ...
  public async processTickets(
    ...
    ) => false | undefined | void | Promise<false | undefined | void>,
    startTime: number = 0,
  ): Promise<boolean> {
    ...
    while (!response?.end_of_stream && response?.after_cursor) {
      LOG.info(
        `Current after cursor for recovery: "${response?.after_cursor}"`,
      );
      response = await this._continueIncrementalTicketExport(
        response.after_cursor,
      );
      if ((await this._processTickets(response.tickets, callback)) === false) {
        LOG.debug('processTickets: Ending because callback requested it');
        return false;
  ...
  private async _startIncrementalTicketExport(
    ...
    if (!isZendeskTicketListDTO(result)) {
      LOG.debug(
        `_startIncrementalTicketExport: Not ZendeskTicketListDTO: ${explainZendeskTicketListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.tickets ?? [],
        (item: unknown): boolean => !isZendeskTicket(item),
      );
  ...
  private async _continueIncrementalTicketExport(
    ...
    if (!isZendeskTicketListDTO(result)) {
      LOG.debug(
        `_continueIncrementalTicketExport: Not ZendeskTicketListDTO: ${explainZendeskTicketListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.tickets ?? [],
        (item: unknown): boolean => !isZendeskTicket(item),
  ...
  public async processTicketComments(
    ...
    size: number,
    callback: (
      ticket: ZendeskComment,
    ) => false | undefined | void | Promise<false | undefined | void>,
  ): Promise<boolean> {
    let response = await this._startCommentExport(ticketId, size);
    if (
      (await this._processTicketComments(response.comments, callback)) === false
    ) {
    ...
    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueCommentExport(
        ticketId,
        size,
        response?.meta?.after_cursor,
      );
      if (
        (await this._processTicketComments(response.comments, callback)) ===
        false
      ) {
        LOG.debug(
          'processTicketComments: Ending because callback requested it',
        );
        return false;
      }
    }
  ...
  private async _startCommentExport(
    ...
    if (!isZendeskCommentListDTO(result)) {
      LOG.debug(
        `_startCommentExport: Not ZendeskCommentListDTO: ${explainZendeskCommentListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.comments ?? [],
        (item: unknown): boolean => !isZendeskComment(item),
      );
      ...
      const attachments = filter(
        reduce(
          list,
          (prev: any[], item: unknown): any[] => {
            const itemAttachments = objectKey(item, 'attachments');
            const attachments = isArray(itemAttachments) ? itemAttachments : [];
            return [
              ...prev,
              ...attachments,
              ...reduce(
                attachments,
                (list: any[], item: unknown): any[] => {
                  return [
                    ...list,
                    ...(isArray(objectKey(item, 'thumbnails'))
                      ? (objectKey(item, 'thumbnails') as unknown[])
                      : []),
                  ];
                },
                [],
        ...
        (item: unknown): boolean => !isZendeskAttachment(item),
      );
      LOG.debug(
        '_startCommentExport: incorrect attachments in comments = ',
        attachments,
      );
  ...
  private async _continueCommentExport(
    ...
    if (!isZendeskCommentListDTO(result)) {
      LOG.debug(
        `_continueCommentExport: Not ZendeskCommentListDTO: ${explainZendeskCommentListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.comments ?? [],
        (item: unknown): boolean => !isZendeskComment(item),
  ...
  public async getUser(userId: number): Promise<ZendeskUser> {
    const entity: ResponseEntity<JsonAny | undefined> | undefined =
      await this._getJsonEntityWithRateLimitCheck(
    ...
    if (!isZendeskUserDTO(result)) {
      LOG.debug(
        `getUser: Not ZendeskUserDTO: ${explainZendeskUserDTO(result)}`,
      );
  ...
  public async processUsers(
    ...
    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueUserExport(
        size,
        response?.meta?.after_cursor,
      );
      if ((await this._processUsers(response.users, callback)) === false) {
  ...
  private async _startUserExport(size: number): Promise<ZendeskUserListDTO> {
    const result = await HttpService.getJson(
    ...
    if (!isZendeskUserListDTO(result)) {
      LOG.debug(
        `_startUserExport: Not ZendeskUserListDTO: ${explainZendeskUserListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.users ?? [],
        (item: unknown): boolean => !isZendeskUser(item),
      );
  ...
  private async _continueUserExport(
    ...
    if (!isZendeskUserListDTO(result)) {
      LOG.debug(
        `_continueUserExport: Not ZendeskUserListDTO: ${explainZendeskUserListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.users ?? [],
        (item: unknown): boolean => !isZendeskUser(item),
  ...
  public async getOrganization(orgId: number): Promise<ZendeskOrganization> {
    const entity: ResponseEntity<JsonAny | undefined> | undefined =
      await this._getJsonEntityWithRateLimitCheck(
    ...
    if (!isZendeskOrganizationDTO(result)) {
      LOG.debug(
        `getOrganization: Not ZendeskOrganizationDTO: ${explainZendeskOrganizationDTO(result)}`,
      );
  ...
  public async processOrganizations(
    size: number,
    callback: (
      ticket: ZendeskOrganization,
    ) => false | undefined | void | Promise<false | undefined | void>,
  ): Promise<boolean> {
    let response = await this._startOrganizationExport(size);
    if (
      (await this._processOrganizations(response.organizations, callback)) ===
      false
    ) {
    ...
    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueOrganizationExport(
        size,
        response?.meta?.after_cursor,
      );
      if (
  ...
  private async _startOrganizationExport(
    ...
    if (!isZendeskOrganizationListDTO(result)) {
      LOG.debug(
        `_startOrganizationExport: Not ZendeskOrganizationListDTO: ${explainZendeskOrganizationListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.organizations ?? [],
        (item: unknown): boolean => !isZendeskOrganization(item),
      );
  ...
  private async _continueOrganizationExport(
    ...
    if (!isZendeskOrganizationListDTO(result)) {
      LOG.debug(
        `_continueOrganizationExport: Not ZendeskOrganizationListDTO: ${explainZendeskOrganizationListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.organizations ?? [],
        (item: unknown): boolean => !isZendeskOrganization(item),
      ...
      LOG.debug(
        '_continueOrganizationExport: incorrect organizations = ',
        list,
      );
      throw new TypeError('Result was not ZendeskOrganizationListDTO');
    }
  ...
  public async getGroup(groupId: number): Promise<ZendeskGroup> {
    const entity: ResponseEntity<JsonAny | undefined> | undefined =
      await this._getJsonEntityWithRateLimitCheck(
    ...
    if (!isZendeskGroupDTO(result)) {
      LOG.debug(
        `getGroup: Not ZendeskGroupDTO: ${explainZendeskGroupDTO(result)}`,
      );
  ...
  public async processGroups(
    ...
    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueGroupExport(
        size,
        response?.meta?.after_cursor,
      );
      if ((await this._processGroups(response.groups, callback)) === false) {
  ...
  private async _startGroupExport(size: number): Promise<ZendeskGroupListDTO> {
    const result = await HttpService.getJson(
    ...
    if (!isZendeskGroupListDTO(result)) {
      LOG.debug(
        `_startGroupExport: Not ZendeskGroupListDTO: ${explainZendeskGroupListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskGroup(item),
      );
  ...
  private async _continueGroupExport(
    ...
    if (!isZendeskGroupListDTO(result)) {
      LOG.debug(
        `_continueGroupExport: Not ZendeskGroupListDTO: ${explainZendeskGroupListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskGroup(item),
  ...
  public async getOrganizationMembership(
    ...
    if (!isZendeskOrganizationMembershipDTO(result)) {
      LOG.debug(
        `getOrganizationMembership: Not ZendeskOrganizationMembershipDTO: ${explainZendeskOrganizationMembershipDTO(result)}`,
      );
      LOG.debug(
        'getOrganizationMembership: incorrect organization_membership = ',
        result,
      );
  ...
  public async processOrganizationMemberships(
    size: number,
    callback: (
      ticket: ZendeskOrganizationMembership,
    ) => false | undefined | void | Promise<false | undefined | void>,
  ): Promise<boolean> {
    let response = await this._startOrganizationMembershipExport(size);
    if (
      (await this._processOrganizationMemberships(
        response.organization_memberships,
        callback,
      )) === false
    ) {
      LOG.debug(
        'processOrganizationMemberships: Ending because callback requested it',
      );
      return false;
    }

    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueOrganizationMembershipExport(
        size,
        response?.meta?.after_cursor,
      );
      if (
    ...
    if (!response?.meta?.has_more) {
      LOG.debug(
        'processOrganizationMemberships: Ending since has no more data',
      );
    } else if (response?.meta?.after_cursor) {
      LOG.debug(
        'processOrganizationMemberships: Ending since no after_cursor detected',
      );
    }
  ...
  private async _startOrganizationMembershipExport(
    ...
    if (!isZendeskOrganizationMembershipListDTO(result)) {
      LOG.debug(
        `_startOrganizationMembershipExport: Not ZendeskOrganizationMembershipListDTO: ${explainZendeskOrganizationMembershipListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskOrganizationMembership(item),
      );
      LOG.debug(
        '_startOrganizationMembershipExport: incorrect groups = ',
        list,
      );

      throw new TypeError(
        'Result was not ZendeskOrganizationMembershipListDTO',
      );
    }
    return result;
  }
  ...
  private async _continueOrganizationMembershipExport(
    ...
    if (!isZendeskOrganizationMembershipListDTO(result)) {
      LOG.debug(
        `_continueOrganizationMembershipExport: Not ZendeskOrganizationMembershipListDTO: ${explainZendeskOrganizationMembershipListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskOrganizationMembership(item),
      ...
      LOG.debug(
        '_continueOrganizationMembershipExport: incorrect groups = ',
        list,
      );
      throw new TypeError(
        'Result was not ZendeskOrganizationMembershipListDTO',
  ...
  public async processUserIdentities(
    ...
    size: number,
    callback: (
      ticket: ZendeskUserIdentity,
    ) => false | undefined | void | Promise<false | undefined | void>,
  ): Promise<boolean> {
    let response = await this._startUserIdentityExport(userId, size);
    if (
      (await this._processUserIdentities(response.identities, callback)) ===
      false
    ) {
    ...
    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueUserIdentityExport(
        userId,
        size,
        response?.meta?.after_cursor,
      );
      if (
        ...
        LOG.debug(
          'processUserIdentities: Ending because callback requested it',
        );
        return false;
      }
    }
  ...
  private async _startUserIdentityExport(
    ...
    if (!isZendeskUserIdentityListDTO(result)) {
      LOG.debug(
        `_startUserIdentityExport: Not ZendeskUserIdentityListDTO: ${explainZendeskUserIdentityListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.identities ?? [],
        (item: unknown): boolean => !isZendeskUserIdentity(item),
      );
  ...
  private async _continueUserIdentityExport(
    ...
    if (!isZendeskUserIdentityListDTO(result)) {
      LOG.debug(
        `_continueUserIdentityExport: Not ZendeskUserIdentityListDTO: ${explainZendeskUserIdentityListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.identities ?? [],
        (item: unknown): boolean => !isZendeskUserIdentity(item),
  ...
  public async getGroupMembership(
    ...
    if (!isZendeskGroupMembershipDTO(result)) {
      LOG.debug(
        `getGroupMembership: Not ZendeskGroupMembershipDTO: ${explainZendeskGroupMembershipDTO(result)}`,
      );
  ...
  public async processGroupMemberships(
    size: number,
    callback: (
      ticket: ZendeskGroupMembership,
    ) => false | undefined | void | Promise<false | undefined | void>,
  ): Promise<boolean> {
    let response = await this._startGroupMembershipExport(size);
    if (
      (await this._processGroupMemberships(
        response.group_memberships,
        callback,
      )) === false
    ) {
      LOG.debug(
        'processGroupMemberships: Ending because callback requested it',
      );
      return false;
    }

    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueGroupMembershipExport(
        size,
        response?.meta?.after_cursor,
      );
      if (
    ...
    if (!response?.meta?.has_more) {
      ...
      LOG.debug(
        'processGroupMemberships: Ending since no after_cursor detected',
      );
    }
  ...
  private async _startGroupMembershipExport(
    ...
    if (!isZendeskGroupMembershipListDTO(result)) {
      LOG.debug(
        `_startGroupMembershipExport: Not ZendeskGroupMembershipListDTO: ${explainZendeskGroupMembershipListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskGroupMembership(item),
      );
  ...
  private async _continueGroupMembershipExport(
    ...
    if (!isZendeskGroupMembershipListDTO(result)) {
      LOG.debug(
        `_continueGroupMembershipExport: Not ZendeskGroupMembershipListDTO: ${explainZendeskGroupMembershipListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskGroupMembership(item),
  ...
  public async getSuspendedTicket(
    ...
    if (!isZendeskSuspendedTicketDTO(result)) {
      LOG.debug(
        `getSuspendedTicket: Not ZendeskSuspendedTicketDTO: ${explainZendeskSuspendedTicketDTO(result)}`,
      );
  ...
  public async processSuspendedTickets(
    size: number,
    callback: (
      ticket: ZendeskSuspendedTicket,
    ) => false | undefined | void | Promise<false | undefined | void>,
  ): Promise<boolean> {
    let response = await this._startSuspendedTicketExport(size);
    if (
      (await this._processSuspendedTickets(
        response.suspended_tickets,
        callback,
      )) === false
    ) {
      LOG.debug(
        'processSuspendedTickets: Ending because callback requested it',
      );
      return false;
    }

    while (response?.meta?.has_more && response?.meta?.after_cursor) {
      response = await this._continueSuspendedTicketExport(
        size,
        response?.meta?.after_cursor,
      );
      if (
    ...
    if (!response?.meta?.has_more) {
      ...
      LOG.debug(
        'processSuspendedTickets: Ending since no after_cursor detected',
      );
    }
  ...
  private async _startSuspendedTicketExport(
    ...
    if (!isZendeskSuspendedTicketListDTO(result)) {
      LOG.debug(
        `_startSuspendedTicketExport: Not ZendeskSuspendedTicketListDTO: ${explainZendeskSuspendedTicketListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.suspended_tickets ?? [],
        (item: unknown): boolean => !isZendeskSuspendedTicket(item),
      );
      LOG.debug(
        '_startSuspendedTicketExport: incorrect suspended_tickets = ',
        list,
      );
  ...
  private async _continueSuspendedTicketExport(
    ...
    if (!isZendeskSuspendedTicketListDTO(result)) {
      LOG.debug(
        `_continueSuspendedTicketExport: Not ZendeskSuspendedTicketListDTO: ${explainZendeskSuspendedTicketListDTO(result)}`,
      );
      const list = filter(
        (result as any)?.groups ?? [],
        (item: unknown): boolean => !isZendeskSuspendedTicket(item),
