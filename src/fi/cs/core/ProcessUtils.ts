...
export class ProcessUtils {
  ...
  public static parseEnvFileLine(
    ...
    if (input[equalIndex + 1] === '"') {
      ...
      if (equalIndexEnd >= 0) {
        ...
        throw new TypeError(
          'ProcessUtils.parseEnvFileLine: No ending for double quote detected',
        );
      }
    }
  ...
  private static _printErrors(reason: string, err?: unknown): void {
    try {
      if (reason === 'exit') {
        if (err) {
