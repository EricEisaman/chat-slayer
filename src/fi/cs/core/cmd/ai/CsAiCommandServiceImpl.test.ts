...
import {CsAiCommandServiceImpl} from './CsAiCommandServiceImpl';
...
import {
  createOpenAiEditResponseDTO,
  OpenAiEditResponseDTO,
} from '../../openai/dto/OpenAiEditResponseDTO';
...
describe('CsAiCommandServiceImpl', () => {
  let service: CsAiCommandServiceImpl;
  ...
  beforeEach(() => {
    CsAiCommandServiceImpl.setLogLevel(LogLevel.NONE);
    client = new MockOpenAiClient();
    service = new CsAiCommandServiceImpl(client);
  ...
  describe('completion', () => {
    ...
    it('should return the correct value from the getCompletion method', async () => {
      const response = createOpenAiCompletionResponseDTO(
        ...
        createOpenAiCompletionResponseUsage(100, 100, 100),
      );

      jest.spyOn(client, 'getCompletion').mockResolvedValue(response);

      const resultCode = await service.completion([
  ...
  describe('edit', () => {
    ...
    it('should return the correct value from the getEdit method', async () => {
      const response: OpenAiEditResponseDTO = createOpenAiEditResponseDTO(
        ...
        createOpenAiEditResponseUsage(100, 100, 100),
      );
...
`;
    ...
    it('should not return CommandExitStatus.OK if the getEdit method returns an error response', async () => {
      ...
      const resultCode = await service.test(['instruction']);
