...
import {CsAiCommandService} from './CsAiCommandService';
...
} from '../../openai/dto/OpenAiEditResponseChoice';
...
} from '../../openai/dto/OpenAiCompletionResponseChoice';
...
const LOG = LogService.createLogger('CsAiCommandServiceImpl');

export class CsAiCommandServiceImpl implements CsAiCommandService {
  ...
  /**
   * Construct a command line service for `cs ai` command
   *
   * @param client
   */
  public constructor(client: OpenAiClient) {
    this._client = client;
  }
   ...
   *
   * It is intended to be called when user calls `cs ai ...` with the remaining
  ...
  public async test(args: readonly string[]): Promise<CommandExitStatus> {
    ...
    if (this._temperature === undefined)
      this.setTemperature(DEFAULT_TEST_TEMPERATURE);
    ...
    const instruction = writeTestsInstruction(
      language,
  ...
  public async document(args: readonly string[]): Promise<CommandExitStatus> {
    ...
    if (this._temperature === undefined)
      this.setTemperature(DEFAULT_DOC_TEMPERATURE);
    if (this._topP === undefined) this.setTopP(DEFAULT_DOC_TOP_P);
    if (this._iterations === undefined)
      this.setIterations(DEFAULT_DOC_ITERATIONS);
    ...
    const result: OpenAiCompletionResponseDTO =
      await this._client.getCompletion(
        describePrompt,
        DEFAULT_DESC_MODEL,
    ...
    const {textChoice, hasText} = this._parseCompletionResponse(result);
  ...
  public async describe(args: readonly string[]): Promise<CommandExitStatus> {
    ...
    if (this._maxTokens === undefined)
      this.setMaxTokens(DEFAULT_DESC_MAX_TOKENS);
    if (this._temperature === undefined)
      this.setTemperature(DEFAULT_DESC_TEMPERATURE);
  ...
  public async changelog(args: readonly string[]): Promise<CommandExitStatus> {
    ...
    if (this._maxTokens === undefined)
      this.setMaxTokens(DEFAULT_CHANGELOG_MAX_TOKENS);
    if (this._temperature === undefined)
      this.setTemperature(DEFAULT_CHANGELOG_TEMPERATURE);
    ...
    const diffChunks = reduce(
      ...
      (chunks: string[], chunk: string) => {
        if (chunk.length > aiChunkSize) {
          return [...chunks, ...splitString(chunk, aiChunkSize)];
        }
        return [...chunks, chunk];
      },
      [],
    ...
    do {
      ...
      if (chunk !== undefined) {
        ...
        if (
          nextAiChunk.length !== 0 &&
          nextAiChunk.length + chunk.length > aiChunkSize
        ) {
          LOG.debug(
            `changelog: Sending "${nextAiChunk.length}" characters to completion`,
          );
  ...
  public async edit(args: readonly string[]): Promise<CommandExitStatus> {
    ...
    try {
      ...
      if (hasAlternativeTexts) {
        console.warn(
          `Alternative choices: ${JSON.stringify(textChoices, null, 2)}`,
        );
      }

      if (hasErrors) {
        console.error(
          `Other items detected: ${JSON.stringify(errorChoices, null, 2)}`,
        );
      }

      return !hasErrors && hasText
    ...
    try {
      ...
      if (hasText) {
        if (textChoice?.finish_reason === 'stop') {
          ...
          console.warn(
            `Warning! Partial response: "${textChoice?.text ?? ''}"`,
          );
          console.error(
            'Error: Please increase "maxTokens" property to get complete response.',
          );
          return CommandExitStatus.GENERAL_ERRORS;
        }
      }
      ...
      if (hasAlternativeTexts) {
        console.warn(
          `Alternative choices: ${JSON.stringify(alternativeTexts, null, 2)}`,
        );
      }
   ...
   */
  private async _populateFiles(
    list: readonly string[],
  ): Promise<readonly string[]> {
    return map(list, (item: string): string => {
      if (existsSync(item)) {
   ...
   * @private
   */
  private _parseCompletionResponse(result: OpenAiCompletionResponseDTO): {
    ...
    const textChoice: OpenAiCompletionResponseChoice | undefined =
      alternativeTexts.shift();
