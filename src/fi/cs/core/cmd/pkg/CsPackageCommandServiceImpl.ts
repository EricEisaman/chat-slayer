...
import {CsPackageCommandService} from './CsPackageCommandService';
import {readFile} from 'node:fs/promises';

export class CsPackageCommandServiceImpl implements CsPackageCommandService {
  public static create(): CsPackageCommandServiceImpl {
    return new CsPackageCommandServiceImpl();
  }
  ...
  public async main(args: readonly string[]): Promise<CommandExitStatus> {
    ...
    const pkgDirectories: string[] = await this._getPkgDirectories(workingDir);
    ...
    if (withNpm) {
      for (let i = 0; i < packageNames.length; i += 1) {
        ...
        const latestVersion: string = trim(
          execSync(`npm info '${packageName.replace(/'/g, "\\'")}' version`, {
            encoding: 'utf8',
          }),
        );
    ...
    for (let i = 0; i < packageNames.length; i += 1) {
      ...
      const pkgVersions = uniq(
        map(subDirs, dir => pkgStatus[packageName][dir].version),
      );
      if (pkgVersions.length >= 2) {
        console.log(
          `Multiple versions available for ${packageName}: ${map(
            subDirs,
            (dir: string): string => {
