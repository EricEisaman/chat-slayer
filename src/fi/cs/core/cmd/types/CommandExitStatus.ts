 ...
 * @nosideeffects
 */
export function isCommandExitStatus(
  value: unknown,
): value is CommandExitStatus {
  if (!isNumber(value)) return false;
  if (value < 0) return false;
  if (value > 255) return false;
  if (
    value >= CommandExitStatus.FATAL_SIGNAL_RANGE_START &&
...
export function stringifyCommandExitStatus(value: CommandExitStatus): string {
  ...
  switch (value) {
    ...
    case CommandExitStatus.CONFIG:
      return 'CONFIG';
    case CommandExitStatus.COMMAND_INVOKED_CANNOT_EXECUTE:
      return 'COMMAND_INVOKED_CANNOT_EXECUTE';
    case CommandExitStatus.COMMAND_NOT_FOUND:
      return 'COMMAND_NOT_FOUND';
    case CommandExitStatus.INVALID_ARGUMENT_TO_EXIT:
      return 'INVALID_ARGUMENT_TO_EXIT';
    case CommandExitStatus.EXIT_STATUS_OUT_OF_RANGE:
 ...
 * @nosideeffects
 */
export function parseCommandExitStatus(
  value: unknown,
  ...
  if (startsWith(valueString, 'FATAL_SIGNAL_')) {
    const int: number | undefined = parseInteger(
      valueString.substring('FATAL_SIGNAL_'.length, valueString.length),
    );
    ...
    if (
      ...
      int <=
        CommandExitStatus.FATAL_SIGNAL_RANGE_END +
          1 -
          CommandExitStatus.FATAL_SIGNAL_RANGE_START
    ) {
  ...
  switch (valueString) {
    ...
    case 'MISUSE_OF_SHELL_BUILTINS':
      return CommandExitStatus.MISUSE_OF_SHELL_BUILTINS;
    ...
    case 'DATAERR':
      return CommandExitStatus.DATAERR;
    case 'NOINPUT':
      return CommandExitStatus.NOINPUT;
    case 'NOUSER':
      return CommandExitStatus.NOUSER;
    case 'NOHOST':
      return CommandExitStatus.NOHOST;
    case 'UNAVAILABLE':
      return CommandExitStatus.UNAVAILABLE;
    case 'SOFTWARE':
      return CommandExitStatus.SOFTWARE;
    case 'OSERR':
      return CommandExitStatus.OSERR;
    case 'OSFILE':
      return CommandExitStatus.OSFILE;
    case 'CANTCREAT':
      return CommandExitStatus.CANTCREAT;
    case 'IOERR':
      return CommandExitStatus.IOERR;
    case 'TEMPFAIL':
      return CommandExitStatus.TEMPFAIL;
    case 'PROTOCOL':
      return CommandExitStatus.PROTOCOL;
    case 'NOPERM':
      return CommandExitStatus.NOPERM;
    case 'CONFIG':
      return CommandExitStatus.CONFIG;
    case 'COMMAND_INVOKED_CANNOT_EXECUTE':
      return CommandExitStatus.COMMAND_INVOKED_CANNOT_EXECUTE;
    case 'COMMAND_NOT_FOUND':
      return CommandExitStatus.COMMAND_NOT_FOUND;
    case 'INVALID_ARGUMENT_TO_EXIT':
      return CommandExitStatus.INVALID_ARGUMENT_TO_EXIT;
    case 'EXIT_STATUS_OUT_OF_RANGE':
      return CommandExitStatus.EXIT_STATUS_OUT_OF_RANGE;
    case 'CONFLICT':
