...
describe('parseSingleArgument', () => {
  describe('with built-in types', () => {
    it('parses boolean arguments correctly', () => {
      const result = parseSingleArgument(
        '--foo',
    ...
    it('parses non-empty string arguments correctly', () => {
      expect(() =>
        parseSingleArgument('--foo', ArgumentType.NON_EMPTY_STRING, undefined),
      ).toThrow(TypeError);
    });

    it('parses number arguments correctly', () => {
      expect(() =>
        parseSingleArgument('--foo', ArgumentType.NUMBER, undefined),
      ).toThrow(TypeError);
    });

    it('parses integer arguments correctly', () => {
      expect(() =>
        parseSingleArgument('--foo', ArgumentType.INTEGER, undefined),
      ).toThrow(TypeError);
    });
  });
    ...
    it('uses the correct custom parser when provided', () => {
      const parserMap = {
        CUSTOM: (
          // @ts-ignore
          value: unknown,
        ): string | undefined => 'Custom parsed: no value',
      };
  ...
  describe('with invalid type', () => {
    it('throws TypeError when an invalid type is provided', () => {
      expect(() =>
        parseSingleArgument('--foo', 'INVALID_TYPE' as any, undefined),
      ).toThrow(TypeError);
    });
  });
});
