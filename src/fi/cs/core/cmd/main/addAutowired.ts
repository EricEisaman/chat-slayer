...
export function addAutowired<T = any>(): MethodDecoratorFunction {
  ...
  return function autowiredMethod(
    ...
    const metadata = AutowireMetadataUtils.getMethodMetadata(
      target,
      propertyKey,
    );
    ...
    const overrideCallback = function (this: T, ...args: any) {
      try {
        return AutowireUtils.autowireApply(target, propertyKey, method, {
          prevArgs: args,
        });
      } catch (err) {
        LOG.warn(
          `Warning! The addAutowired decorator for "${propertyKey}" method had an error: `,
          err,
        );
        throw err;
      }
