...
describe('AutowireUtils', () => {
  ...
  beforeEach(() => {
    ...
    (AutowireServiceImpl.getAutowireService as any).mockReturnValue(
      autowireServiceMock,
    );
    (AutowireMetadataUtils.getMethodMetadata as any).mockReturnValue({
      paramNames: ['name', 'age'],
    });

    (autowireServiceMock.hasName as any).mockImplementation(
      (name: unknown): boolean => name === 'name' || name === 'age',
    );
    (autowireServiceMock.getName as any).mockImplementation((name: unknown) =>
      name === 'name' ? 'John' : 30,
    );
  });
  ...
  describe('autowireValues', () => {
    it('should return autowired values', () => {
      ...
      const autowiredValues = AutowireUtils.autowireValues(
        autowireServiceMock as AutowireService,
        ['name', 'age'],
      );
  ...
  describe('autowireApply', () => {
    it('should apply autowired values to the method', () => {
      ...
      AutowireUtils.autowireApply(
        mockTarget,
        'propertyName',
        mockTarget.propertyName,
        values,
      );
