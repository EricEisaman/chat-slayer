...
export class AutowireUtils {
  ...
  public static autowireValues(
    ...
    return map(paramNames, (name: string | undefined) => {
      return name && autowireService.hasName(name)
        ? autowireService.getName<any>(name)
        : undefined;
    });
  }
  ...
  public static autowireApply(
    ...
    const metadata: AutowireMetadata = AutowireMetadataUtils.getMethodMetadata(
      target,
      propertyName,
    );
    LOG.debug('autowired metadata = ', metadata);
    const autowireService: AutowireService =
      AutowireServiceImpl.getAutowireService();
