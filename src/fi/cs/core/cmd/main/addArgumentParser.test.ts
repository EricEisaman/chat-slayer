...
describe('addArgumentParser', () => {
  ...
  describe('integration testing', () => {
    ...
    const getUsage = jest.fn<() => string>().mockImplementation(() => 'usage');

    // Mock class with a method decorated with `addArgumentParser` and `addAutowired`
    class MyApp {
      @addArgumentParser('testApp', getVersion, getUsage, {
    ...
    it('can parse long version argument', async () => {
      ...
      expect(getVersion).toHaveBeenCalledWith('dist.js', {
        exitStatus: 0,
        extraArgs: [],
        freeArgs: [],
    ...
    it('can parse long help argument', async () => {
      ...
      expect(getUsage).toHaveBeenCalledWith('dist.js', {
        exitStatus: 0,
        extraArgs: [],
        freeArgs: [],
        nodePath: '/usr/bin/node',
    ...
    it('can parse long arguments', async () => {
      ...
      // Check if autowired parameters match expected values
      expect(retrievedArgs).toEqual([
        '/usr/bin/node',
        'dist.js',
        '--backend=myBackend',
      ]);
      expect(retrievedParsedArgs).toBeDefined();
      expect((retrievedParsedArgs as any)?.parseStatus).toEqual(
        ParsedCommandArgumentStatus.OK,
      );
      expect((retrievedParsedArgs as any)?.exitStatus).toEqual(
        CommandExitStatus.OK,
      );
      expect((retrievedParsedArgs as any)?.userArgs?.backend).toEqual(
        'myBackend',
      );
    ...
    it('can parse illegal integer argument', async () => {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        'ERROR: Argument parse error: TypeError: Argument --integer=myBackend: not integer',
      );
