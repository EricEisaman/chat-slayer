  ...
  return createMethodDecorator(
    (method: Function, context: ClassMethodDecoratorContext) => {
      ...
      return async function (this: T, ...args: any[]) {
        ...
        try {
          LOG.debug(
            `Running method decorator for "${propertyName.toString()}"`,
          );

          const {userArgs} =
            autowireService.getName<ParsedCommandArgumentObject>('parsedArgs');
          const stateFileArg: string | undefined =
            parseNonEmptyString(userArgs?.stateFile) ?? undefined;
          const stateConfigFile: string = pathResolve(
            stateFileArg ?? process?.env?.HOME ?? process.cwd(),
            '.nor-agent.json',
          );
          ...
          if (autowireService.hasName('destroyService')) {
            ...
            LOG.warn(
              'Warning! You should include @addDestroyService() for proper StateService destruction support',
            );
          }

          stateServiceChangedDestructor = stateService.on(
            StateServiceEvent.CHANGED,
            () => {
              if (!stateService) {
          ...
          LOG.warn(
            `Warning! The addStateService decorator for "${propertyName.toString()}" method had an error: `,
            err,
          );
          throw err;
        } finally {
...
async function readStateDTOFromFile<DTO>(
  ...
  try {
    ...
    if (!isDTO(data)) {
      throw new TypeError(
        `The file "${file}" is not valid DTO: ${explainDTO(data)}`,
      );
    }
    ...
    if ((err as any)?.code === 'ENOENT') {
      LOG.debug(
        `No configuration file found from ${file}. Creating from fresh.`,
      );
      return createDTO();
    }
