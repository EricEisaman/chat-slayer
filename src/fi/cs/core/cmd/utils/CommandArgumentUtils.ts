...
} from '../types/CommandArgumentType';
...
export class CommandArgumentUtils {
  ...
  public static parseArguments(
    ...
    const argConfigurationMap: ArgumentConfigurationMap = configurationMap
      ? configurationMap
      : {};
    ...
    const userArgs: ArgumentValueMap = {};

    const userLongArgs: {[key: string]: string} = {};
    const userShortArgs: {[key: string]: string} = {};
    const userEnvKeys: {[key: string]: string} = {};
    forEach(keys(argConfigurationMap), (key: string) => {
    ...
    do {
      ...
      const argType: CommandArgumentType | undefined =
        parseCommandArgumentType(argName);
      switch (argType) {
          ...
          if (indexOf(argName, '=') >= 1) {
            ...
            if (has(userLongArgs, argKey)) {
              ...
              try {
                ...
                return CommandArgumentUtils._getArgumentParseError(
                  err,
                  nodePath,
                  scriptNameFromArgs,
                  freeArgs,
    ...
    // Set ENV keys
    forEach(keys(userEnvKeys), (envKey: string) => {
