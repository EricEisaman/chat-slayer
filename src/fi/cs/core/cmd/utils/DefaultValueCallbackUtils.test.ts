...
describe('DefaultValueCallbackUtils', () => {
  ...
  let setNameSpy: jest.SpiedFunction<(...args: unknown) => any>;
  let hasNameSpy: jest.SpiedFunction<(...args: unknown) => any>;
  let getNameSpy: jest.SpiedFunction<(...args: unknown) => any>;
  ...
  describe('#fromAutowired', () => {
    it('returns a DefaultValueCallback that returns autowired value', () => {
      ...
      setNameSpy.mockImplementation((name: any, value: unknown) => {
        if (name === autowiredTo) {
      ...
      const defaultValueCallback =
        DefaultValueCallbackUtils.fromAutowired(autowiredTo);
  ...
  describe('#fromChain', () => {
    ...
    it('chains multiple DefaultValueCallback functions together', async () => {
      ...
      const callback1: DefaultValueCallback = jest.fn(
        value => `${value}_callback1`,
      );
      const callback2: DefaultValueCallback = jest.fn(
        async value => `${value}_callback2`,
      );
      const callback3: DefaultValueCallback = jest.fn(
        value => `${value}_callback3`,
      );
