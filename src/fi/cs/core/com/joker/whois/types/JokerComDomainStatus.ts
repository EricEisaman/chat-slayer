...
export function isJokerComDomainStatus(
  value: unknown,
): value is JokerComDomainStatus {
  switch (value) {
...
export function explainJokerComDomainStatus(value: unknown): string {
  return explainEnum(
    'JokerDomainStatus',
...
export function stringifyJokerDomainStatus(
  value: JokerComDomainStatus,
): string {
  switch (value) {
    case JokerComDomainStatus.FREE:
      return 'FREE';
...
export function parseJokerComDomainStatus(
  value: unknown,
): JokerComDomainStatus | undefined {
  if (value === undefined) return undefined;
  switch (trim(`${value}`).toLowerCase()) {
    case 'free':
      return JokerComDomainStatus.FREE;
    case 'registered':
      return JokerComDomainStatus.REGISTERED;
    case 'unknown':
      return JokerComDomainStatus.UNKNOWN;
    default:
      return undefined;
  }
}
