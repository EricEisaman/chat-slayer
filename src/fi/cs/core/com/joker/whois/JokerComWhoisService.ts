...
import {
  createWhoisServerOptions,
  WhoisServerOptions,
} from '../../../whois/types/WhoisServerOptions';
import {
  JokerComWhoisDTO,
  parseJokerComWhoisDTOFromString,
} from './types/JokerComWhoisDTO';
 ...
 * @see example at https://github.com/example/whois/blob/main/src/controllers/FiCsWhoisBackendController.ts#L62
 */
export class JokerComWhoisService {
   ...
   * @param address
   */
  public async jokerLookup(address: string): Promise<JokerComWhoisDTO> {
    ...
    const dto = response?.data
      ? parseJokerComWhoisDTOFromString(response?.data)
      : undefined;
    if (!dto) {
      LOG.error(
        `jokerLookup: Could not parse for "${address}": payload: `,
        payload,
      );
      throw new Error(
        `lookupJoker: Could not parse whois server response for "${address}": ${payload}`,
      );
    }
    return dto;
  }
