...
import {
  explainWpPageListDTO,
  isWpPageListDTO,
  WpPageListDTO,
} from './dto/WpPageListDTO';
import {
  explainWpPostListDTO,
  isWpPostListDTO,
  WpPostListDTO,
} from './dto/WpPostListDTO';
import {
  explainWpReferenceListDTO,
  isWpReferenceListDTO,
  WpReferenceListDTO,
} from './dto/WpReferenceListDTO';
import {
  explainWpUserProfileListDTO,
  isWpUserProfileListDTO,
  WpUserProfileListDTO,
} from './dto/WpUserProfileListDTO';
import {
...
export class WpClient {
  ...
  public static create(url: string = WpClient._defaultUrl): WpClient {
    return new WpClient(url);
  }

  public constructor(url: string = WpClient._defaultUrl) {
    this._url = url;
  }
  ...
  public async getPages(): Promise<WpPageListDTO> {
    ...
    const result = await HttpService.getJson(
      `${this._url}${WORD_PRESS_API_V2_PAGES}`,
    );
    if (!isWpPageListDTO(result)) {
      LOG.debug('getPages: result = ', result);
      throw new TypeError(
        `Result was not WpPageListDTO: ${explainWpPageListDTO(result)}`,
      );
    }
    return result;
  }
  ...
  public async getPosts(): Promise<WpPostListDTO> {
    if (this._url.length < 1) return [];
    const result = await HttpService.getJson(
      `${this._url}${WORD_PRESS_API_V2_POSTS}`,
    );
    if (!isWpPostListDTO(result)) {
      LOG.debug('getPosts: result = ', result);
      throw new TypeError(
        `Result was not WpPostListDTO: ${explainWpPostListDTO(result)}`,
      );
    }
    return result;
  }
  ...
  public async getReferences(): Promise<WpReferenceListDTO> {
    if (this._url.length < 1) return [];
    const result = await HttpService.getJson(
      `${this._url}${WORD_PRESS_API_V3_REFERENCES}`,
    );
    if (!isWpReferenceListDTO(result)) {
      LOG.debug('getReferences: result = ', result);
      throw new TypeError(
        `Result was not WpReferenceListDTO: ${explainWpReferenceListDTO(result)}`,
      );
    }
    return result;
  }
  ...
  public async getUserProfiles(): Promise<WpUserProfileListDTO> {
    if (this._url.length < 1) return [];
    const result = await HttpService.getJson(
      `${this._url}${WORD_PRESS_API_V3_USERPROFILES}`,
    );
    if (!isWpUserProfileListDTO(result)) {
      LOG.debug('getUserProfiles: result = ', result);
      throw new TypeError(
        `Result was not WpUserProfileListDTO: ${explainWpUserProfileListDTO(result)}`,
      );
    }
    return result;
  }
