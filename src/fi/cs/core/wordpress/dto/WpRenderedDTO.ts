...
export function isWpRenderedDTO(value: unknown): value is WpRenderedDTO {
  return (
    ...
    hasNoOtherKeysInDevelopment(value, ['rendered', 'protected']) &&
    isString(objectKey(value, 'rendered')) &&
    isBooleanOrUndefined(objectKey(value, 'protected'))
  );
}

export function explainWpRenderedDTO(value: unknown): string {
  return explain([
    explainRegularObject(value),
    explainNoOtherKeys(value, ['rendered', 'protected']),
    explainProperty('rendered', explainString(objectKey(value, 'rendered'))),
    explainProperty(
      'protected',
      explainBooleanOrUndefined(objectKey(value, 'protected')),
    ),
  ]);
}
