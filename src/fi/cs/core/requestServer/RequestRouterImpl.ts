...
import {
  parseRequestMethod,
  RequestMethod,
} from '../request/types/RequestMethod';
...
import {
  isResponseEntity,
  ResponseEntity,
} from '../request/types/ResponseEntity';
...
export class RequestRouterImpl implements RequestRouter {
  ...
  private _modelAttributeNames:
    | Map<RequestController, ModelAttributeProperty[]>
    | undefined;
  private _requestMappings:
    | readonly RequestControllerMappingObject[]
    | undefined;
  ...
  public static create(...controllers: readonly any[]): RequestRouterImpl {
    const router = new RequestRouterImpl();
    forEach(controllers, (controller: unknown) => {
      if (!isRequestController(controller)) {
        throw new TypeError(
          `RequestRouter: The provided controller was not RequestController: ${controller}`,
        );
      }
  ...
  public async handleRequest(
    ...
    try {
      ...
      const {pathName, queryParams}: RequestContext =
        parseRequestContextFromPath(urlString);
      LOG.debug(
        `handleRequest: method="${method}", pathName="${pathName}", queryParams=`,
        queryParams,
      );
      ...
      const {routes, bodyRequired, pathVariables}: RequestContext =
        this._getRequestRoutesContext(method, requestPathName);

      if (!parseRequestBody && bodyRequired) {
        LOG.error(
          'handleRequest: parseRequestBody was not provided and body is required',
        );
        return ResponseEntity.internalServerError().body({
          error: 'Internal Server Error',
        });
      ...
      const requestBody =
        parseRequestBody && bodyRequired
          ? await parseRequestBody(requestHeaders)
          : undefined;
      ...
      await reduce(
        routes,
        async (previousPromise, route: RequestRouterMappingPropertyObject) => {
          const routeController: any = route.controller;
          const routePropertyName: string = route.propertyName;
          const routePropertyParams: readonly (RequestParamObject | null)[] =
            route.propertyParams;
          ...
          if (
            ...
            this._modelAttributeNames.has(routeController)
          ) {
            LOG.debug(
              `Populating attributes for property "${routePropertyName}"`,
            );

            const modelAttributeValues: Map<string, any> =
              RequestRouterImpl._getOrCreateRequestModelAttributesForController(
            ...
            const routeAttributeNames: string[] = map(
              filter(
                routePropertyParams,
                (item: unknown): item is RequestModelAttributeParamObject =>
                  isRequestModelAttributeParamObject(item),
              ),
              (item: RequestModelAttributeParamObject): string =>
                item.attributeName,
            );
            LOG.debug('route attributeNames: ', routeAttributeNames);

            const allModelAttributeNamesForRouteController =
              this._modelAttributeNames.get(routeController);
            LOG.debug(
              'all attributeNamePairs: ',
              allModelAttributeNamesForRouteController,
            );

            const attributeNamePairs: ModelAttributeProperty[] = filter(
              allModelAttributeNamesForRouteController ?? [],
              (item: ModelAttributeProperty): boolean =>
                routeAttributeNames.includes(item[0]),
            );
            ...
            await reduce(
              ...
              async (
                ...
                const stepResult: any = await routeController[propertyName](
                  ...stepParams,
                );
          ...
          const stepParams = RequestRouterImpl._bindRequestActionParams(
            ...
            requestHeaders,
            pathVariables,
            requestModelAttributes.get(routeController) ??
              new Map<string, any>(),
          );
          ...
          if (!has(routeController, routePropertyName)) {
            LOG.warn(
              `Warning! No property by name "${routePropertyName}" found in the controller`,
            );
            responseEntity = ResponseEntity.notFound<JsonObject>().body({
              error: '404 - Not Found',
              code: 404,
            });
            return;
          ...
          if (isSynchronized) {
            ...
            LOG.debug(
              `handleRequest: Initialized request synchronizer [${synchronizerKey}]`,
            );

            stepResult = await synchronizer.run(async (): Promise<any> => {
              LOG.debug(
                `handleRequest: Calling route property by name "${routePropertyName}"`,
              );
            ...
            stepResult = await routeController[routePropertyName](
              ...stepParams,
            );
          }
          ...
          if (isRequestStatus(stepResult)) {
            ...
            responseEntity = new ResponseEntity<ReadonlyJsonObject>(
              stepResult.toJSON(),
              stepResult.getStatusCode(),
            );
  ...
  private _initializeRoutes() {
    ...
    if (this._requestMappings?.length) {
      this._routes = RouteUtils.createRoutes(
        RequestRouterImpl._parseMappingObject(this._requestMappings),
      );
    } else {
      this._routes = RouteUtils.createRoutes({});
    }
  ...
  private _initializeRequiredModelAttributeNames() {
    ...
    if (this._requestMappings?.length) {
      values = reduce(
        ...
        (
          arr: [RequestController, ModelAttributeProperty[]][],
          item: RequestControllerMappingObject,
        ) => {
          const controller = item.controller;

          const controllerUniqueAttributeNames: ModelAttributeProperty[] =
            reduce(
              keys(item.controllerProperties),
              (
                arr2: ModelAttributeProperty[],
                propertyKey: string,
              ): ModelAttributeProperty[] => {
                LOG.debug(
                  '_initializeRequiredModelAttributeNames: propertyKey: ',
                  propertyKey,
                );

                const propertyValue: RequestControllerMethodObject =
                  item.controllerProperties[propertyKey];

                const propertyAttributeNames: readonly string[] =
                  propertyValue.modelAttributes;

                LOG.debug(
                  '_initializeRequiredModelAttributeNames: propertyAttributeNames: ',
                  propertyAttributeNames,
                );

                const params: (RequestParamObject | null)[] = [
                  ...propertyValue.params,
                ];

                forEach(propertyAttributeNames, (attributeName: string) => {
                  LOG.debug(
                    '_initializeRequiredModelAttributeNames: attributeName: ',
                    attributeName,
                  );
                  if (
                    find(
                      arr2,
                      (i: ModelAttributeProperty) => i[0] === attributeName,
                    ) === undefined
                  ) {
    ...
    this._modelAttributeNames = new Map<
      RequestController,
      ModelAttributeProperty[]
    >(values);
  }
  ...
  private _getRequestRoutesContext(
    ...
    if (!this._routes || !this._routes.hasRoute(requestPathName)) {
      if (!this._routes) {
        ...
        LOG.debug(
          '_getRequestRoutesContext: Routes did not match: ',
          requestPathName,
        );
      }
      return {
    ...
    if (!routes.length) {
      ...
      LOG.debug(
        '_getRequestRoutesContext: There were matching routes, but not for this method; Method not allowed.',
      );
      return {
        routes: undefined,
        bodyRequired: false,
    ...
    const requestBodyRequired = some(
      routes,
      (item: RequestRouterMappingPropertyObject) =>
        item.requestBodyRequired === true,
    );
    LOG.debug(
      '_getRequestRoutesContext: requestBodyRequired=',
      requestBodyRequired,
    );
    return {
      routes,
  ...
  private _getRequestMappings(): RequestControllerMappingObject[] {
    ...
    return filter(
      map(this._controllers, (controller: RequestController) =>
        getRequestControllerMappingObject(controller),
      ),
        ...
        forEach(rootItem.mappings, (rootMappingItem: RequestMappingObject) => {
          ...
          forEach(rootMappingItem.paths, (rootPath: string) => {
            forEach(controllerPropertyNames, (propertyKey: string) => {
              const propertyValue: RequestControllerMethodObject =
                controllerProperties[propertyKey];
              const propertyParams: readonly (RequestParamObject | null)[] =
                propertyValue.params;

              forEach(
                propertyValue.mappings,
                (propertyMappingItem: RequestMappingObject) => {
                  // Filters away any property routes which do not have common methods
                  const propertyMethods: readonly RequestMethod[] =
                    propertyMappingItem.methods;

                  if (
                    !RequestRouterImpl._matchMethods(
                      rootMethods,
                      propertyMethods,
                    )
                  ) {
                  ...
                  const propertyMethodsCommonWithRoot: readonly RequestMethod[] =
                    RequestRouterImpl._parseCommonMethods(
                      rootMethods,
                      propertyMethods,
                    );

                  const propertyPaths: readonly string[] =
                    propertyMappingItem.paths;

                  forEach(propertyPaths, (propertyPath: string) => {
                    const fullPropertyPath = RequestRouterImpl._joinRoutePaths(
                      rootPath,
                      propertyPath,
                    );
                    ...
                    setRouteMappingResult(fullPropertyPath, {
                      requestBodyRequired:
                        propertyValue?.requestBodyRequired ?? false,
  ...
  private static _parseCommonMethods(
    ...
    return rootMethods.length !== 0
      ? filter(propertyMethods, (propertyMethod: RequestMethod): boolean => {
          return some(rootMethods, (rootMethod: RequestMethod): boolean => {
            return rootMethod === propertyMethod;
          });
        })
    ...
    return map(params, (item: RequestParamObject | null): any => {
      ...
      switch (objectType) {
        ...
        case RequestParamObjectType.QUERY_PARAM: {
          const queryParamItem: RequestQueryParamObject =
            item as RequestQueryParamObject;
          const key = queryParamItem.queryParam;
          if (key === undefined) {
            return RequestRouterImpl._castObjectParam(
              searchParams,
              queryParamItem.valueType,
            );
          }
          if (!has(searchParams, key)) return undefined;
          const value: string | null = searchParams[key];
          if (isNull(value)) return undefined;
          return RequestRouterImpl._castStringParam(
            value,
            queryParamItem.valueType,
          );
        }
        ...
        case RequestParamObjectType.REQUEST_HEADER: {
          const headerItem: RequestHeaderParamObject =
            item as RequestHeaderParamObject;
          ...
          if (!requestHeaders.containsKey(headerName)) {
            if (headerItem.isRequired) {
              throw new RequestError(
                400,
                `Bad Request: Header missing: ${headerName}`,
              );
            }
          ...
          const headerValue: string | undefined =
            requestHeaders.getFirst(headerName);

          if (headerValue === undefined) return undefined;

          return RequestRouterImpl._castStringParam(
            headerValue,
            headerItem.valueType,
          );
        }

        case RequestParamObjectType.REQUEST_HEADER_MAP: {
          const headerItem: RequestHeaderMapParamObject =
            item as RequestHeaderMapParamObject;

          const defaultHeaders: DefaultHeaderMapValuesType | undefined =
            headerItem?.defaultValues;
        ...
        case RequestParamObjectType.PATH_VARIABLE: {
          const pathParamItem: RequestPathVariableParamObject =
            item as RequestPathVariableParamObject;

          const variableName = pathParamItem.variableName;

          const variableValue =
            pathVariables && has(pathVariables, variableName)
        ...
        case RequestParamObjectType.MODEL_ATTRIBUTE: {
          const modelAttributeItem: RequestModelAttributeParamObject =
            item as RequestModelAttributeParamObject;

          const key = modelAttributeItem.attributeName;

          return modelAttributes.has(key)
            ? modelAttributes.get(key)
            : undefined;
        }
      }
  ...
  private static _castObjectParam(
    ...
    switch (type) {
      ...
      case RequestParamValueType.NUMBER:
        throw new TypeError(
          `Incorrect value type for non-string value: ${type}`,
        );
    }
    throw new TypeError(`Unsupported type: ${type}`);
  }
  ...
  private static _getOrCreateRequestModelAttributesForController(
    ...
    let modelAttributeValues: Map<string, any> | undefined =
      requestModelAttributes.get(routeController);
