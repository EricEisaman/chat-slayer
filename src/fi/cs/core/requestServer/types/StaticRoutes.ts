...
type MappingPropertyKeyValuePair = readonly [
  string,
  RequestRouterMappingPropertyObject[],
];
...
export class StaticRoutes implements BaseRoutes {
  ...
  public constructor(routes: RequestRouterMappingObject) {
    ...
    const routePairs: MappingPropertyKeyValuePair[] = map(
      routePaths,
      (itemKey: string): MappingPropertyKeyValuePair => {
    ...
    this._routes = new Map<string, RequestRouterMappingPropertyObject[]>(
      routePairs,
    );
  }
