...
describe('RequestContext', () => {
  describe('#parseRequestContextFromPath', () => {
    ...
    it('can parse URL path with multiple query parameters', () => {
      expect(
        parseRequestContextFromPath('/path/to?world=hello&foo=bar'),
      ).toStrictEqual({
        pathName: '/path/to',
        queryParams: {
          world: 'hello',
    ...
    it('can parse URL path with multiple query parameters with escaping', () => {
      expect(
        parseRequestContextFromPath('/path/to?message=hello+world&foo=bar'),
      ).toStrictEqual({
        pathName: '/path/to',
        queryParams: {
          message: 'hello world',
    ...
    it('can parse URL path with multiple query parameters with encoding', () => {
      expect(
        parseRequestContextFromPath(
          '/path/to?message=hello+world%20%28testing%29&foo=bar',
        ),
      ).toStrictEqual({
        pathName: '/path/to',
        queryParams: {
