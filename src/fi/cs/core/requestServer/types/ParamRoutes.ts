...
export class ParamRoutes implements BaseRoutes {
  ...
  public constructor(routes: RequestRouterMappingObject) {
    ...
    const routeFunctions: CompiledRequestPathCallback[] = map(
      routePaths,
      (itemKey: string): CompiledRequestPathCallback => {
  ...
  public getRoute(pathName: string): GetRouteResultType {
    let lastResult: RequestRouterMappingPropertyObject[] | undefined =
      undefined;
    let lastParams: RouteParamValuesObject | undefined = undefined;

    find(this._routes, (f: CompiledRequestPathCallback) => {
      const [result, params] = f(pathName);
  ...
  public static createCallbackFunction(
    ...
    const paramKeys: (string | null)[] = map(
      formatParts,
      (pathValue: string, pathIndex: number) => {
        if (
          pathValue.length >= 3 &&
          pathValue[0] === '{' &&
        ...
        if (pathValue.length >= 2 && pathValue[0] === ':') {
    ...
    return (requestPath: string): GetRouteResultType => {
      ...
      const paramValues: RouteParamValuesObject = {};

      if (
        every<string>(
          paramKeys,
          (paramKey: string | null, paramIndex: number): boolean => {
            if (isString(paramKey)) {
              ...
              return (
                formatParts[paramIndex] ===
                requestParts[paramIndex].toLowerCase()
              );
            }
          },
        )
