 ...
 * @see https://docs.example.com/common/csv/
 * @file
 */
...
export function isCsv(value: unknown): value is Csv {
 ...
 * @param value
 */
export function stringifyCsvCellValue(value: unknown): string {
...
export function getCsvRowFromJsonObject<T = ReadonlyJsonObject>(
  ...
  return map(properties, (key: string): string => {
...
export function getCsvFromJsonObjectList<T = ReadonlyJsonObject>(
  ...
  const rows: Csv = map(
    list,
    (item: T): CsvRow =>
      getCsvRowFromJsonObject<T>(item, keyList, propertyTransformer),
  );
...
export function parseCsvRow(
  ...
  if (separator?.length !== 1) {
    throw new TypeError(
      `The separator must be exactly 1 character long: ${separator}`,
    );
  }
  ...
  while (lastIndex < value.length) {
    ...
    if (
      piece.length >= 2 &&
      startsWith(piece, quote) &&
      endsWith(piece, quote)
    ) {
      piece = piece
        .substr(1, piece.length - 2)
        .split(piece + piece)
        .join(piece);
    }
    pieces.push(piece);
...
export function parseCsv(
  ...
  return map(
    ...
    (item: unknown): CsvRow => parseCsvRow(item, separator, quote),
  );
}
...
export function stringifyCsv(
  ...
  if (replaceLineBreak !== false) {
    value = replaceCsvContentLineBreaks(value, lineBreak, replaceLineBreak);
  }

  return map(value, (row: CsvRow) =>
    stringifyCsvRow(row, separator, quote),
  ).join(lineBreak);
}
...
export function replaceCsvContentLineBreaks(
  ...
  return map(
    value,
    (row: CsvRow): CsvRow =>
      map(row, (column: string): string =>
        replaceAll(column, lineBreak, replaceTo),
      ),
  );
}
