...
export class AsyncSynchronizerImpl implements AsyncSynchronizer {
  ...
  public async run<T = any>(callback: () => Promise<T>): Promise<T> {
    ...
    LOG.debug(
      `Created a lock. This lock queue has ${this._queue.length} locks.`,
    );
    ...
    while (this._queue.length && first(this._queue) !== lock) {
      LOG.debug(
        `The queue did not have us, waiting ${this._queue.length} locks...`,
      );
    ...
    try {
      ...
      if (first(this._queue) !== lock) {
        LOG.warn(
          'Warning! The lock queue did not have us as the first item. Could not release the lock.',
        );
      } else {
        this._queue.shift();
