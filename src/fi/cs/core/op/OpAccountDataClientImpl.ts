 ...
 */
export class OpAccountDataClientImpl
  implements OpAccountDataClient, OpAuthClient
{
  ...
  public static create(
    ...
    return new OpAccountDataClientImpl(client, auth, url);
  }

  private constructor(client: RequestClient, auth: OpAuthClient, url: string) {
    this._client = client;
    this._auth = auth;
    this._url = url;
  }
  ...
  public async getTransactionListFromTimestampRange(
    ...
    const fullList: OpTransactionDTO[] = [];
    let prevStartTime = fromTimestamp;
    do {
      LOG.debug(
        `Fetching from ${fromTimestamp} to ${toTimestamp} for ${bufferSize} items`,
      );
      ...
      if (list.length > 0) {
        ...
        forEach(list, (item: OpTransactionDTO): void => {
    ...
    // Sort by timestamp
    fullList.sort((a: OpTransactionDTO, b: OpTransactionDTO): number => {
