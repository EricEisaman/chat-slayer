  ...
  (CLIENT_ID ? describe : describe.skip)('OpRefundClientImpl', () => {
    ...
    beforeEach(async () => {
      const requestClient = RequestClientImpl.create(
        NodeRequestClient.create(HTTP, HTTPS, {
          cert: MTLS_CRT,
          key: MTLS_KEY,
        }),
      );
      const authClient = OpAuthClientImpl.create(requestClient, API_SERVER);
      ...
      client = OpRefundClientImpl.create(
        requestClient,
        authClient,
        OpRequestSigner.create(SIGNING_KID, SIGNING_KEY),
        API_SERVER,
      );
      await authClient.authenticate(CLIENT_ID, CLIENT_SECRET);
      ...
      const transactionList: OpTransactionListDTO =
        await accountDataClient.getTransactionListFromTimestamp(
          surrogateId,
          fromTimestamp,
        );
      ...
      if (transactionList.length === 0)
        throw new TypeError('No transactions found');
    ...
    describe('#refundPayment', () => {
      it('should return a successful response with valid archive ID', async () => {
        ...
        const refundResponse: OpRefundResponseDTO =
          await client.refundPayment(refundRequest);
      ...
      it.skip('should return an error response with invalid archive ID format', async () => {
        ...
        await expect(client.refundPayment(refundRequest)).rejects.toThrow(
