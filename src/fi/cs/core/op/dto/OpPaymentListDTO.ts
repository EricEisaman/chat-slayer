...
export function createOpPaymentListDTO(
  ...
  return map(list, (item: OpPaymentResponseDTO): OpPaymentResponseDTO => item);
}
...
export function explainOpPaymentListDTO(value: unknown): string {
  return explainArrayOf<OpPaymentResponseDTO>(
    'OpPaymentResponseDTO',
    explainOpPaymentResponseDTO,
    value,
    isOpPaymentResponseDTO,
...
export function parseOpPaymentListDTO(
  value: unknown,
): OpPaymentListDTO | undefined {
  if (isOpPaymentListDTO(value)) return value;
  return undefined;
}

export function isOpPaymentListDTOOrUndefined(
  value: unknown,
): value is OpPaymentListDTO | undefined {
  return isUndefined(value) || isOpPaymentListDTO(value);
}

export function explainOpPaymentListDTOOrUndefined(value: unknown): string {
  return isOpPaymentListDTOOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpPaymentListDTO', 'undefined']));
}
