...
export function createOpAccountListDTO(
  ...
  return map(list, (item: OpAccountDTO): OpAccountDTO => item);
}
...
export function explainOpAccountListDTO(value: unknown): string {
  return explainArrayOf<OpAccountDTO>(
    'OpAccountDTO',
    explainOpAccountDTO,
    value,
    isOpAccountDTO,
...
export function parseOpAccountListDTO(
  value: unknown,
): OpAccountListDTO | undefined {
  if (isOpAccountListDTO(value)) return value;
  return undefined;
}

export function isOpAccountListDTOOrUndefined(
  value: unknown,
): value is OpAccountListDTO | undefined {
  return isUndefined(value) || isOpAccountListDTO(value);
}

export function explainOpAccountListDTOOrUndefined(value: unknown): string {
  return isOpAccountListDTOOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpAccountListDTO', 'undefined']));
}
