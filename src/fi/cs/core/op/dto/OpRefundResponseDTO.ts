...
import {
  explainOpRefundOriginalObject,
  isOpRefundOriginalObject,
  OpRefundOriginalObject,
} from '../types/OpRefundOriginalObject';
import {
  explainOpRefundRefundObject,
  isOpRefundRefundObject,
  OpRefundRefundObject,
} from '../types/OpRefundRefundObject';
...
export function isOpRefundResponseDTO(
  value: unknown,
): value is OpRefundResponseDTO {
  return (
    isRegularObject(value) &&
    hasNoOtherKeysInDevelopment(value, ['original', 'refund']) &&
    isOpRefundOriginalObject(objectKey(value, 'original')) &&
    isOpRefundRefundObject(objectKey(value, 'refund'))
  );
}

export function explainOpRefundResponseDTO(value: unknown): string {
  return explain([
    explainRegularObject(value),
    explainNoOtherKeysInDevelopment(value, ['original', 'refund']),
    explainProperty(
      'original',
      explainOpRefundOriginalObject(objectKey(value, 'original')),
    ),
    explainProperty(
      'refund',
      explainOpRefundRefundObject(objectKey(value, 'refund')),
    ),
  ]);
}

export function parseOpRefundResponseDTO(
  value: unknown,
): OpRefundResponseDTO | undefined {
  if (isOpRefundResponseDTO(value)) return value;
  return undefined;
}

export function isOpRefundResponseDTOOrUndefined(
  value: unknown,
): value is OpRefundResponseDTO | undefined {
...
export function explainOpRefundResponseDTOOrUndefined(value: unknown): string {
  return isOpRefundResponseDTOOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpRefundResponseDTO', 'undefined']));
}
