...
import {
  ...
  isOpPaymentCreditor,
  OpPaymentCreditor,
} from '../types/OpPaymentCreditor';
import {
  explainOpPaymentDebtor,
  isOpPaymentDebtor,
  OpPaymentDebtor,
} from '../types/OpPaymentDebtor';
import {
  explainOpPaymentInstructedAmount,
  isOpPaymentInstructedAmount,
  OpPaymentInstructedAmount,
} from '../types/OpPaymentInstructedAmount';
import {
  explainOpUltimateDebtorOrUndefined,
  isOpUltimateDebtorOrUndefined,
  OpUltimateDebtor,
} from '../types/OpUltimateDebtor';
import {
  explainOpUltimateCreditorOrUndefined,
  isOpUltimateCreditorOrUndefined,
  OpUltimateCreditor,
} from '../types/OpUltimateCreditor';
...
export function isOpPaymentRequestDTO(
  value: unknown,
): value is OpPaymentRequestDTO {
  return (
    ...
    isString(objectKey(value, 'instructionId')) &&
    isOpPaymentCreditor(objectKey(value, 'creditor')) &&
    isOpPaymentDebtor(objectKey(value, 'debtor')) &&
    isOpPaymentInstructedAmount(objectKey(value, 'instructedAmount')) &&
    isStringOrUndefined(objectKey(value, 'reference')) &&
    isStringOrUndefined(objectKey(value, 'message')) &&
    isStringOrUndefined(objectKey(value, 'endToEndId')) &&
    isOpUltimateDebtorOrUndefined(objectKey(value, 'ultimateDebtor')) &&
    isOpUltimateCreditorOrUndefined(objectKey(value, 'ultimateCreditor'))
  );
}

export function explainOpPaymentRequestDTO(value: unknown): string {
  return explain([
    ...
    explainProperty(
      'instructionId',
      explainString(objectKey(value, 'instructionId')),
    ),
    explainProperty(
      'creditor',
      explainOpPaymentCreditor(objectKey(value, 'creditor')),
    ),
    explainProperty(
      'debtor',
      explainOpPaymentDebtor(objectKey(value, 'debtor')),
    ),
    explainProperty(
      'instructedAmount',
      explainOpPaymentInstructedAmount(objectKey(value, 'instructedAmount')),
    ),
    explainProperty(
      'reference',
    ...
    explainProperty(
      'message',
      explainStringOrUndefined(objectKey(value, 'message')),
    ),
    explainProperty(
      'endToEndId',
      explainStringOrUndefined(objectKey(value, 'endToEndId')),
    ),
    explainProperty(
      'ultimateDebtor',
      explainOpUltimateDebtorOrUndefined(objectKey(value, 'ultimateDebtor')),
    ),
    explainProperty(
      'ultimateCreditor',
      explainOpUltimateCreditorOrUndefined(
        objectKey(value, 'ultimateCreditor'),
      ),
    ),
  ]);
}

export function parseOpPaymentRequestDTO(
  value: unknown,
): OpPaymentRequestDTO | undefined {
  if (isOpPaymentRequestDTO(value)) return value;
  return undefined;
}

export function isOpPaymentRequestDTOOrUndefined(
  value: unknown,
): value is OpPaymentRequestDTO | undefined {
...
export function explainOpPaymentRequestDTOOrUndefined(value: unknown): string {
  return isOpPaymentRequestDTOOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpPaymentRequestDTO', 'undefined']));
}
