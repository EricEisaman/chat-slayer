...
export class OpPaymentEntity extends Entity {
  ...
  public constructor(dto?: OpPaymentDTO) {
    ...
    this.rCreditorAddressLine1 =
      dto?.request?.creditor?.address?.addressLine[0];
    this.rCreditorAddressLine2 =
      dto?.request?.creditor?.address?.addressLine[1];
    ...
    this.rUltimateDebtorIdentificationId =
      dto?.request?.ultimateDebtor?.identification?.id;
    this.rUltimateDebtorIdentificationSchemeName =
      dto?.request?.ultimateDebtor?.identification?.schemeName;
    this.rUltimateDebtorIdentificationIssuer =
      dto?.request?.ultimateDebtor?.identification?.issuer;
    this.rUltimateDebtorAddressCountry =
      dto?.request?.ultimateDebtor?.address?.country;
    this.rUltimateDebtorAddressLine1 =
      dto?.request?.ultimateDebtor?.address?.addressLine[0];
    this.rUltimateDebtorAddressLine2 =
      dto?.request?.ultimateDebtor?.address?.addressLine[1];

    this.rUltimateCreditorName = dto?.request?.ultimateCreditor?.name;
    this.rUltimateCreditorIdentificationId =
      dto?.request?.ultimateCreditor?.identification?.id;
    this.rUltimateCreditorIdentificationSchemeName =
      dto?.request?.ultimateCreditor?.identification?.schemeName;
    this.rUltimateCreditorIdentificationIssuer =
      dto?.request?.ultimateCreditor?.identification?.issuer;
    this.rUltimateCreditorAddressCountry =
      dto?.request?.ultimateCreditor?.address?.country;
    this.rUltimateCreditorAddressLine1 =
      dto?.request?.ultimateCreditor?.address?.addressLine[0];
    this.rUltimateCreditorAddressLine2 =
      dto?.request?.ultimateCreditor?.address?.addressLine[1];
  ...
  public static toDTO(entity: OpPaymentEntity): OpPaymentDTO {
    if (!entity?.rInstructionId)
      throw new TypeError('entity.rInstructionId missing');
    if (!entity?.rCreditorName)
      throw new TypeError('entity.rCreditorName missing');
    if (!entity?.rCreditorAddressCountry)
      throw new TypeError('entity.rCreditorAddressCountry missing');
    if (!entity?.rCreditorAddressLine1)
      throw new TypeError('entity.rCreditorAddressLine1 missing');
    if (!entity?.rCreditorAddressLine2)
      throw new TypeError('entity.rCreditorAddressLine2 missing');
    if (!entity?.rDebtorName) throw new TypeError('entity.rDebtorName missing');
    if (!entity?.rDebtorAddressCountry)
      throw new TypeError('entity.rDebtorAddressCountry missing');
    if (!entity?.rDebtorAddressLine1)
      throw new TypeError('entity.rDebtorAddressLine1 missing');
    if (!entity?.rDebtorAddressLine2)
      throw new TypeError('entity.rDebtorAddressLine2 missing');
    if (!entity?.rInstructedAmountValue)
      throw new TypeError('entity.rInstructedAmountValue missing');
    if (!entity?.rInstructedAmountCurrency)
      throw new TypeError('entity.rInstructedAmountCurrency missing');
    ...
    const dto: OpPaymentDTO = createOpPaymentDTO(
      createOpPaymentRequestDTO(
        ...
        createOpPaymentCreditor(
          ...
          createOpAddress(entity?.rCreditorAddressCountry, [
        ...
        createOpPaymentDebtor(
          entity?.rDebtorName,
          entity?.rDebtorIban,
          createOpAddress(entity?.rDebtorAddressCountry, [
        ...
        entity?.rUltimateDebtorName &&
          ...
          ? createOpUltimateDebtor(
              ...
              createOpAddress(entity?.rUltimateDebtorAddressCountry, [
                entity?.rUltimateDebtorAddressLine1,
                entity?.rUltimateDebtorAddressLine2,
              ]),
            )
        ...
        entity?.rUltimateCreditorName &&
          ...
          ? createOpUltimateCreditor(
              ...
              createOpAddress(entity?.rUltimateCreditorAddressCountry, [
                entity?.rUltimateCreditorAddressLine1,
                entity?.rUltimateCreditorAddressLine2,
              ]),
            )
    ...
    if (!isOpPaymentDTO(dto)) {
      ...
      throw new TypeError(
        `Failed to create valid OpPaymentDTO: ${explainOpPaymentDTO(dto)}`,
      );
    }
    return dto;
  }
