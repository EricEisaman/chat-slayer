...
export class OpTransactionEntity extends Entity {
  ...
  public constructor();
  public constructor(
    dto: OpTransactionDTO & {opAccountId: string; opSurrogateId: string},
  );
  ...
  public static toDTO(entity: OpTransactionEntity): OpTransactionDTO {
    ...
    if (!entity.balanceBefore)
      throw new TypeError('entity.balanceBefore missing');
    if (!entity.balanceAfter)
      throw new TypeError('entity.balanceAfter missing');
    ...
    if (!entity.creditorName)
      throw new TypeError('entity.creditorName missing');
    if (!entity.debtorAccount)
      throw new TypeError('entity.debtorAccount missing');
    if (!entity.timestamp) throw new TypeError('entity.timestamp missing');
    if (!entity.transactionTypeCode)
      throw new TypeError('entity.transactionTypeCode missing');
    if (!entity.transactionTypeDescription)
      throw new TypeError('entity.transactionTypeDescription missing');
    if (!entity.uetr) throw new TypeError('entity.uetr missing');
    const timestamp = parseInteger(entity.timestamp);
    if (!timestamp)
      throw new TypeError('timestamp invalid: ' + entity.timestamp);
    ...
    if (!isOpTransactionDTO(dto)) {
      ...
      throw new TypeError(
        `Failed to create valid OpTransactionDTO: ${explainOpTransactionDTO(dto)}`,
      );
    }
    return dto;
  }
