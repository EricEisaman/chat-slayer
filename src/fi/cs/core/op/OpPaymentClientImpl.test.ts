...
const MOCK_PAYMENT_LIST_RESPONSE: readonly OpPaymentResponseDTO[] = [
  MOCK_PAYMENT_LIST_ITEM_RESPONSE,
];
...
describe('OpPaymentClientImpl', () => {
  ...
  describe('#createPayment', () => {
    it('should initiate payment correctly with unauthenticated auth client', async () => {
      ...
      (requestClient.postText as any).mockReturnValue(
        Promise.resolve(JSON.stringify(MOCK_OP_PAYMENT_RESPONSE_DTO)),
      );
      (requestSigner as any).mockReturnValue(MOCK_SIGNATURE);

      // Act
      await expect(
        client.createPayment(MOCK_OPPAYMENTREQUEST_DTO),
    ...
    it('should initiate payment correctly with authenticated auth client', async () => {
      ...
      expect(requestSigner).toHaveBeenCalledWith(
  ...
  describe('#createInstantPayment', () => {
    it('should initiate payment correctly with unauthenticated auth client', async () => {
      ...
      // Act
      await expect(
        client.createInstantPayment(MOCK_OPPAYMENTREQUEST_DTO),
      ).rejects.toThrow('Not authenticated');
    ...
    it('should initiate payment correctly with authenticated auth client', async () => {
      ...
      // Act
      const result = await client.createInstantPayment(
        MOCK_OPPAYMENTREQUEST_DTO,
      );
  ...
  describe('getInstantPaymentStatus', () => {
    it('should get valid status of an initiated instant payment when auth client is authenticated', async () => {
      ...
      (authClient.isAuthenticated as any).mockReturnValue(true);
      (authClient.getAccessKey as any).mockReturnValue(MOCK_ACCESS_TOKEN);
      (requestClient.getJson as any).mockReturnValue(
        Promise.resolve(MOCK_PAYMENT_LIST_RESPONSE),
      );
      (requestSigner as any).mockReturnValue(MOCK_SIGNATURE);
    ...
    it('should throw an error when auth client is not authenticated', async () => {
      ...
      // Act
      await expect(
        client.getInstantPaymentStatus(MOCK_INSTRUCTION_ID),
      ).rejects.toThrow('Not authenticated');
