import {
  ...
  explainOpPaymentTypeOrUndefined,
  isOpPaymentType,
  isOpPaymentTypeOrUndefined,
  OpPaymentType,
  parseOpPaymentType,
  stringifyOpPaymentType,
} from './OpPaymentType';

describe('OpPaymentType', () => {
  const validTypes = [
    OpPaymentType.SEPA_CREDIT_TRANSFER,
  ...
  describe('explainOpPaymentType', () => {
    it('should return expected explanation for valid and invalid types', () => {
      ...
      expect(explainOpPaymentType(invalidType)).toBe(
        'incorrect enum value "UNKNOWN" for OpPaymentType: Accepted values SEPA_CREDIT_TRANSFER, SCT_INST',
      );
    });
  });
  ...
  describe('explainOpPaymentTypeOrUndefined', () => {
    it('should return expected explanation for valid, invalid, and undefined types', () => {
      ...
      // replace 'OK' with the actual expected explanation for invalid case
      expect(explainOpPaymentTypeOrUndefined(invalidType)).toBe(
        'not OpPaymentType or undefined',
      );
