...
export function parseOpPaymentType(value: unknown): OpPaymentType | undefined {
  return parseEnum(OpPaymentType, value) as OpPaymentType | undefined;
}

export function isOpPaymentTypeOrUndefined(
  value: unknown,
): value is OpPaymentType | undefined {
  return isUndefined(value) || isOpPaymentType(value);
}

export function explainOpPaymentTypeOrUndefined(value: unknown): string {
  return isOpPaymentTypeOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpPaymentType', 'undefined']));
}
