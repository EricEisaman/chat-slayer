...
import {
  createOpRefundOriginalObject,
  explainOpRefundOriginalObject,
  explainOpRefundOriginalObjectOrUndefined,
  isOpRefundOriginalObject,
  isOpRefundOriginalObjectOrUndefined,
  OpRefundOriginalObject,
  parseOpRefundOriginalObject,
} from './OpRefundOriginalObject';
...
describe('OpRefundOriginalObject', () => {
  ...
  describe('explainOpRefundOriginalObject', () => {
    ...
    it('provides explanations for invalid properties', () => {
      ...
      expect(explainOpRefundOriginalObject(invalidObject)).toContain(
        'property "amount"',
      );
    });
  });
  ...
  describe('explainOpRefundOriginalObjectOrUndefined', () => {
    it('explains valid object as OK', () => {
      expect(explainOpRefundOriginalObjectOrUndefined(validObject)).toBe(
        EXPLAIN_OK,
      );
    });

    it('explains undefined as OK', () => {
      expect(explainOpRefundOriginalObjectOrUndefined(undefined)).toBe(
        EXPLAIN_OK,
      );
    });
    ...
    it('provides explanations for invalid properties', () => {
      const invalidObject = {...validObject, amount: 100}; // made amount a number
      expect(explainOpRefundOriginalObjectOrUndefined(invalidObject)).toContain(
        'OpRefundOriginalObject',
      );
    });
  });
