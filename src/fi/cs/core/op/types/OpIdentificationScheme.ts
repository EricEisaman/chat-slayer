...
export function isOpIdentificationScheme(
  value: unknown,
): value is OpIdentificationScheme {
...
export function stringifyOpIdentificationScheme(
  value: OpIdentificationScheme,
): string {
  return stringifyEnum(OpIdentificationScheme, value);
}

export function parseOpIdentificationScheme(
  value: unknown,
): OpIdentificationScheme | undefined {
  return parseEnum(OpIdentificationScheme, value) as
    | OpIdentificationScheme
    | undefined;
}

export function isOpIdentificationSchemeOrUndefined(
  value: unknown,
): value is OpIdentificationScheme | undefined {
  return isUndefined(value) || isOpIdentificationScheme(value);
}

export function explainOpIdentificationSchemeOrUndefined(
  value: unknown,
): string {
  return isOpIdentificationSchemeOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpIdentificationScheme', 'undefined']));
}
