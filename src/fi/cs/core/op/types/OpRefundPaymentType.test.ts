import {
  ...
  explainOpRefundPaymentTypeOrUndefined,
  isOpRefundPaymentType,
  isOpRefundPaymentTypeOrUndefined,
  OpRefundPaymentType,
  parseOpRefundPaymentType,
  stringifyOpRefundPaymentType,
...
describe('OpRefundPaymentType functions', () => {
  describe('isOpRefundPaymentType', () => {
    it('should return true for valid payment types', () => {
      expect(
        isOpRefundPaymentType(OpRefundPaymentType.SEPA_CREDIT_TRANSFER),
      ).toBe(true);
  ...
  describe('explainOpRefundPaymentType', () => {
    it('should explain valid payment types correctly', () => {
      expect(
        explainOpRefundPaymentType(OpRefundPaymentType.SEPA_CREDIT_TRANSFER),
      ).toBe('OK');
    });

    it('should explain invalid payment types correctly', () => {
      const invalidValue = 'RANDOM_VALUE';
      expect(explainOpRefundPaymentType(invalidValue)).toBe(
        `incorrect enum value "${invalidValue}" for OpRefundPaymentType: Accepted values SEPA_CREDIT_TRANSFER, SCT_INST`,
      );
    });
  });
  ...
  describe('stringifyOpRefundPaymentType', () => {
    it('should stringify valid payment types correctly', () => {
      expect(
        stringifyOpRefundPaymentType(OpRefundPaymentType.SEPA_CREDIT_TRANSFER),
      ).toBe('SEPA_CREDIT_TRANSFER');
    });

    it('should throw error for invalid payment types', () => {
      expect(() => stringifyOpRefundPaymentType('RANDOM_VALUE' as any)).toThrow(
        'Unsupported enum value: RANDOM_VALUE',
      );
    });
  });
  ...
  describe('parseOpRefundPaymentType', () => {
    it('should parse valid payment types correctly', () => {
      expect(parseOpRefundPaymentType('SEPA_CREDIT_TRANSFER')).toBe(
        OpRefundPaymentType.SEPA_CREDIT_TRANSFER,
      );
    });
    ...
    it('should parse ignoring dashes', () => {
      expect(parseOpRefundPaymentType('SEPA-CREDIT-TRANSFER')).toBe(
        OpRefundPaymentType.SEPA_CREDIT_TRANSFER,
      );
    });
  });
  ...
  describe('explainOpRefundPaymentTypeOrUndefined', () => {
    ...
    it('should provide explanations for other invalid values', () => {
      expect(explainOpRefundPaymentTypeOrUndefined('RANDOM_VALUE')).toBe(
        'not OpRefundPaymentType or undefined',
      );
    });
  });
});
