import {
  ...
  explainOpPaymentIdentificationOrUndefined,
  isOpPaymentIdentification,
  isOpPaymentIdentificationOrUndefined,
  OpPaymentIdentification,
  parseOpPaymentIdentification,
...
describe('OpPaymentIdentification', () => {
  ...
  describe('explainOpPaymentIdentification', () => {
    it('should provide correct explanation for OpPaymentIdentification objects', () => {
      ...
      expect(explainOpPaymentIdentification(wrongObject)).toContain(
        'property "id" not string',
      );
      expect(explainOpPaymentIdentification(wrongObject)).toContain(
        'property "schemeName" incorrect enum value "WrongScheme" for OpIdentificationScheme: Accepted values BIC, COID, TXID, SOSE, UNSTRUCTURED_ORG, UNSTRUCTURED_PERSON',
      );
      expect(explainOpPaymentIdentification(wrongObject)).toContain(
        'property "issuer" not string or undefined',
      );
    });
  });
  ...
  describe('explainOpPaymentIdentificationOrUndefined', () => {
    it('should provide correct explanation for OpPaymentIdentification or undefined', () => {
      expect(explainOpPaymentIdentificationOrUndefined(testObject)).toEqual(
        'OK',
      );
      expect(explainOpPaymentIdentificationOrUndefined(undefined)).toEqual(
        'OK',
      );
      expect(explainOpPaymentIdentificationOrUndefined(wrongObject)).toEqual(
        'not OpPaymentIdentification or undefined',
      );
    });
  });
});
