...
export function parseOpPaymentStatus(
  value: unknown,
): OpPaymentStatus | undefined {
  return parseEnum(OpPaymentStatus, value) as OpPaymentStatus | undefined;
}

export function isOpPaymentStatusOrUndefined(
  value: unknown,
): value is OpPaymentStatus | undefined {
  return isUndefined(value) || isOpPaymentStatus(value);
}

export function explainOpPaymentStatusOrUndefined(value: unknown): string {
  return isOpPaymentStatusOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['OpPaymentStatus', 'undefined']));
}
