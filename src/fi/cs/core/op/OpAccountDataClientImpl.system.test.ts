 ...
 * other times.
 */
const TRANSACTION_TEST_LIMIT =
  parseInteger(process.env.OP_TRANSACTION_TEST_LIMIT ?? '5') ?? 5;
        ...
        it('should return details for a all accounts', async () => {
          for (let i = 0; i < surrogateIdList.length; i += 1) {
            ...
            const accountDetails =
              await accountDataClient.getAccountDetails(surrogateId);
      ...
      describe('#getTransactionListFromObjectId', () => {
        it('should return a list of transactions', async () => {
          for (let i = 0; i < surrogateIdList.length; i += 1) {
            ...
            // We test only 5
            const objectIdList = slice(
              shuffle(map(firstTransactionList, item => item.objectId)),
              0,
              TRANSACTION_TEST_LIMIT,
            );

            for (let i = 0; i < objectIdList.length; i += 1) {
              const objectId = objectIdList[i];
              const transactionList =
                await accountDataClient.getTransactionListFromObjectId(
                  surrogateId,
                  objectId,
                );
              expect(transactionList).toBeDefined();
