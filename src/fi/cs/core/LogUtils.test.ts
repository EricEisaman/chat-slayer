...
describe('LogUtils', () => {
  describe('#stringifyArray', () => {
    ...
    it('converts an array of objects to a string', () => {
      expect(LogUtils.stringifyArray([{name: 'John'}, {name: 'Jane'}])).toEqual(
        '{"name":"John"} {"name":"Jane"}',
      );
    });

    it('converts an array of mixed types to a string', () => {
      expect(LogUtils.stringifyArray([1, 'two', {name: 'John'}, true])).toEqual(
  ...
  describe('#stringifyValue', () => {
    ...
    it('converts an object to a string', () => {
      expect(LogUtils.stringifyValue({name: 'John'})).toEqual(
        '{"name":"John"}',
      );
    });
    ...
    it('converts an TypeError object to a string', () => {
      expect(LogUtils.stringifyValue(new TypeError('Fail'))).toEqual(
        'TypeError: Fail',
      );
    });
  ...
  describe('#splitStringValue', () => {
    ...
    it('can split long string to one row 1', () => {
      const rows = LogUtils.splitStringValue('123456789\n', 10, '>>>', '...\n');
      expect(rows).toBeArray();
    ...
    it('can split long string to one row 1', () => {
      const rows = LogUtils.splitStringValue('12345\n', 10, '>>>', '...\n');
      expect(rows).toBeArray();
    ...
    it('cannot split too short rows', () => {
      expect(() =>
        LogUtils.splitStringValue('12345\n', 4, '> ', '<\n'),
      ).toThrow(
        new TypeError(
          'Max size to splitStringValue() must be greater than the length of prefix and suffix (4 < 4)',
        ),
      );
    });
  });
  ...
  describe('#splitStringArray', () => {
    ...
    it('splits an array of strings into chunks of maximum length', () => {
      ...
      const result = LogUtils.splitStringArray(
        input,
        maxSize,
        prefix,
        suffix,
    ...
    it('handles an array of short strings', () => {
      ...
      expect(result).toStrictEqual(['Short string.', 'Another short word.']);
    });
    ...
    it('returns an unchanged array if maxSize is large enough for all strings', () => {
      const input = ['Short string.', 'Another short string.'];
      maxSize = 50;
      const result = LogUtils.splitStringArray(
        input,
        maxSize,
