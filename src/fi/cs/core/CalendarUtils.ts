...
import {
  createInternetCalendarLine,
  InternetCalendarLine,
  isInternetCalendarLine,
} from './types/InternetCalendarLine';
import {
  createInternetCalendarParam,
  InternetCalendarParam,
} from './types/InternetCalendarParam';
...
export type ReadonlyInternetCalendarLineBlockList = readonly (
  | InternetCalendarLine
  | ReadonlyInternetCalendarLineList
)[];

export type InternetCalendarLineList = InternetCalendarLine[];
export type InternetCalendarLineBlockList = (
  | InternetCalendarLine
  | InternetCalendarLineList
)[];
...
export class CalendarUtils {
  ...
  public static parseInternetCalendarLine(line: string): InternetCalendarLine {
    ...
    if (index < 0) {
      throw new TypeError(
        'CalendarUtils.parseInternetCalendarLine: No name found',
      );
    }
    ...
    if (paramIndex < 0) {
      return createInternetCalendarLine(nameParams, value);
    } else {
   ...
   * @param rows
   */
  public static unfoldInternetCalendarLines(
    ...
    return reduce(
      ...
      (list: string[], row: string): string[] => {
        if (row.length) {
          ...
          LOG.warn(
            'Warning: Empty line parsed at parseInternetCalendarRows().',
          );
        }
  ...
  public static groupInternetCalendarLines(
    value: ReadonlyInternetCalendarLineList,
  ): ReadonlyInternetCalendarLineBlockList {
    const stack: InternetCalendarLineBlockList[] = [];

    return reduce(
      value,
      (
        list: InternetCalendarLineBlockList,
        item: InternetCalendarLine,
      ): InternetCalendarLineBlockList => {
        ...
        if (itemName === 'END') {
          ...
          if (stack.length === 0) {
            ...
            LOG.warn(
              `Warning! Ended block "${blockName}" without previous START`,
            );
            return list;
          }

          const lastBlock: InternetCalendarLineBlockList =
            stack[stack.length - 1];
          const lastBegin:
            | InternetCalendarLine
            | InternetCalendarLineList
            | undefined = lastBlock.length ? lastBlock[0] : undefined;
          if (!isInternetCalendarLine(lastBegin)) {
            throw new TypeError(
              `CalendarUtils.groupInternetCalendarLines: Could not detect BEGIN for END (${blockName})`,
            );
          }
          if (lastBegin.value !== blockName) {
            throw new TypeError(
              `CalendarUtils.groupInternetCalendarLines: Found wrong block for END (${blockName})`,
            );
          }
  ...
  public static parseCalendarDTOFromInternetCalendar(
    value: string,
  ): CalendarDTO {
    ...
    const parsedRows: InternetCalendarLine[] = map(
      unfoldRows,
      (item: string): InternetCalendarLine =>
        CalendarUtils.parseInternetCalendarLine(item),
    );

    const grouped: ReadonlyInternetCalendarLineBlockList =
      CalendarUtils.groupInternetCalendarLines(parsedRows);

    // LOG.debug(`grouped = `, grouped);

    const eventBlocks: ReadonlyInternetCalendarLineList[] = filter(
      grouped,
      (
        item: InternetCalendarLine | ReadonlyInternetCalendarLineList,
      ): boolean => {
  ...
  public static parseCalendarEventFromInternetCalendarLines(
    list: ReadonlyInternetCalendarLineList,
  ): CalendarEvent {
    // LOG.debug(`parseCalendarEventFromInternetCalendarLines: `, list);

    const start: string =
      CalendarUtils._findCalendarLineAsTime('DTSTART', list) ?? '';
    const end: string =
      CalendarUtils._findCalendarLineAsTime('DTEND', list) ?? '';
    const repeatRule: string =
      CalendarUtils._findCalendarLine('RRULE', list)?.value ?? '';
    const stamp: string =
      CalendarUtils._findCalendarLineAsTime('DTSTAMP', list) ?? '';
    const uid: string =
      CalendarUtils._findCalendarLine('UID', list)?.value ?? '';
    const created: string =
      CalendarUtils._findCalendarLineAsTime('CREATED', list) ?? '';
    const description: string =
      CalendarUtils._findCalendarLine('DESCRIPTION', list)?.value ?? '';
    const lastModified: string =
      CalendarUtils._findCalendarLineAsTime('LAST-MODIFIED', list) ?? '';
    const location: string =
      CalendarUtils._findCalendarLine('LOCATION', list)?.value ?? '';
    const sequence: string =
      CalendarUtils._findCalendarLine('SEQUENCE', list)?.value ?? '';
    const status: string =
      CalendarUtils._findCalendarLine('STATUS', list)?.value ?? '';
    const summary: string =
      CalendarUtils._findCalendarLine('SUMMARY', list)?.value ?? '';
    const transparency: string =
      CalendarUtils._findCalendarLine('TRANSP', list)?.value ?? '';
  ...
  private static _findCalendarLine(
    name: string,
    list: ReadonlyInternetCalendarLineList,
  ...
  private static _findCalendarLineAsTime(
    name: string,
    list: ReadonlyInternetCalendarLineList,
  ): string | undefined {
    const item: InternetCalendarLine | undefined =
      CalendarUtils._findCalendarLine(name, list);
    ...
    if (item) {
      ...
      if (tzId) {
        const tzValue =
          CalendarUtils.parseWindowsTimeZoneToIANA(tzId.value) ?? tzId.value;
