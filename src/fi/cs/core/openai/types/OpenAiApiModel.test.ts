...
describe('OpenAiApiModel', () => {
  ...
  describe('explainOpenApiModel', () => {
    it("should return 'incorrect enum value' message for invalid values", () => {
      expect(explainOpenAiModel('invalid')).toMatch(
        /incorrect enum value "invalid" for OpenApiModel/,
      );
      expect(explainOpenAiModel(123)).toMatch(
        /incorrect enum value "123" for OpenApiModel/,
      );
      expect(explainOpenAiModel(null)).toMatch(
        /incorrect enum value "null" for OpenApiModel/,
      );
      expect(explainOpenAiModel(undefined)).toMatch(
        /incorrect enum value "undefined" for OpenApiModel/,
      );
    });
  ...
  describe('stringifyOpenApiModel', () => {
    it('should return the string representation of the OpenAiApiModel', () => {
      expect(stringifyOpenAiModel(OpenAiModel.DAVINCI)).toEqual(
        'text-davinci-003',
      );
      expect(stringifyOpenAiModel(OpenAiModel.CURIE)).toEqual('text-curie-001');
      expect(stringifyOpenAiModel(OpenAiModel.BABBAGE)).toEqual(
        'text-babbage-001',
      );
