...
export function createOpenAiErrorDTO(error: OpenAiError): OpenAiErrorDTO {
  return {
    error,
  };
}
...
export function isOpenAiErrorDTO(value: unknown): value is OpenAiErrorDTO {
  return (
    ...
    hasNoOtherKeysInDevelopment(value, ['error']) &&
    isOpenAiError(objectKey(value, 'error'))
  );
}

export function explainOpenAiErrorDTO(value: unknown): string {
  return explain([
    explainRegularObject(value),
    explainNoOtherKeysInDevelopment(value, ['error']),
    explainProperty('error', explainOpenAiError(objectKey(value, 'error'))),
  ]);
}
...
export function parseOpenAiErrorDTO(
  value: unknown,
): OpenAiErrorDTO | undefined {
  if (isString(value)) {
