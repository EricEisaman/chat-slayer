 ...
 */
export function isOpenAiEditResponseDTO(
  value: unknown,
): value is OpenAiEditResponseDTO {
  return (
    ...
    isArrayOf<OpenAiEditResponseChoice>(
      objectKey(value, 'choices'),
      isOpenAiEditResponseChoice,
    ) &&
    isOpenAiEditResponseUsage(objectKey(value, 'usage'))
  );
}
 ...
 */
export function explainOpenAiEditResponseDTO(value: unknown): string {
  return explain([
    explainRegularObject(value),
    explainNoOtherKeys(value, ['object', 'created', 'choices', 'usage']),
    explainProperty('object', explainString(objectKey(value, 'object'))),
    explainProperty('created', explainNumber(objectKey(value, 'created'))),
    explainProperty(
      'choices',
      explainArrayOf<OpenAiEditResponseChoice>(
        'OpenAiEditResponseChoice',
        explainOpenAiEditResponseChoice,
        objectKey(value, 'choices'),
        isOpenAiEditResponseChoice,
    ...
    explainProperty(
      'usage',
      explainOpenAiEditResponseUsage(objectKey(value, 'usage')),
    ),
  ]);
}
 ...
 */
export function stringifyOpenAiEditResponseDTO(
  value: OpenAiEditResponseDTO,
): string {
 ...
 */
export function parseOpenAiEditResponseDTO(
  value: unknown,
): OpenAiEditResponseDTO | undefined {
  if (isString(value)) {
    if (startsWith(value, 'OpenAiEditResponseDTO(')) {
      value = value.substring(
        'OpenAiEditResponseDTO('.length,
        value.length - 1,
      );
    }
