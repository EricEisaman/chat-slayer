...
export function createOpenAiEditRequestDTO(
  ...
  if (!isString(instruction))
    throw new TypeError(
      `Invalid OpenAiEditRequestDTO.instruction: ${instruction}`,
    );
  if (!isStringOrUndefined(input))
    throw new TypeError(`Invalid OpenAiEditRequestDTO.input: ${input}`);
  if (!isStringOrUndefined(model))
    throw new TypeError(`Invalid OpenAiEditRequestDTO.model: ${model}`);
  if (!isNumberOrUndefined(n))
    throw new TypeError(`Invalid OpenAiEditRequestDTO.n: ${n}`);
  if (!isNumberOrUndefined(temperature))
    throw new TypeError(
      `Invalid OpenAiEditRequestDTO.temperature: ${temperature}`,
    );
  if (!isNumberOrUndefined(top_p))
    throw new TypeError(`Invalid OpenAiEditRequestDTO.top_p: ${top_p}`);
  return {
 ...
 */
export function isOpenAiEditRequestDTO(
  value: unknown,
): value is OpenAiEditRequestDTO {
  return (
    ...
    isString(objectKey(value, 'instruction')) &&
    isStringOrUndefined(objectKey(value, 'input')) &&
...
export function explainOpenAiEditRequestDTO(value: unknown): string {
  return explain([
    ...
    explainProperty(
      'temperature',
      explainNumberOrUndefined((value as any)?.temperature),
    ),
 ...
 */
export function stringifyOpenAiEditRequestDTO(
  value: OpenAiEditRequestDTO,
): string {
 ...
 */
export function parseOpenAiEditRequestDTO(
  value: unknown,
): OpenAiEditRequestDTO | undefined {
  if (isString(value)) {
