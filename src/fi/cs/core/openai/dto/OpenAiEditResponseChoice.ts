 ...
 */
export function isOpenAiEditResponseChoice(
  value: unknown,
): value is OpenAiEditResponseChoice {
  return (
    ...
    isStringOrUndefined(objectKey(value, 'finish_reason'))
  );
}
 ...
 */
export function explainOpenAiEditResponseChoice(value: unknown): string {
  return explain([
    ...
    explainProperty(
      'finish_reason',
      explainStringOrUndefined(objectKey(value, 'finish_reason')),
    ),
  ]);
}
 ...
 */
export function stringifyOpenAiEditResponseChoice(
  value: OpenAiEditResponseChoice,
): string {
 ...
 */
export function parseOpenAiEditResponseChoice(
  value: unknown,
): OpenAiEditResponseChoice | undefined {
  if (isString(value)) {
    if (startsWith(value, 'OpenAiEditResponseChoice(')) {
      value = value.substring(
        'OpenAiEditResponseChoice('.length,
        value.length - 1,
      );
    }
