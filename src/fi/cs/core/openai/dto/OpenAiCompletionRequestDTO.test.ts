import {
  ...
  createOpenAiCompletionRequestDTO,
  isOpenAiCompletionRequestDTO,
  explainOpenAiCompletionRequestDTO,
  stringifyOpenAiCompletionRequestDTO,
  parseOpenAiCompletionRequestDTO,
} from './OpenAiCompletionRequestDTO';
...
describe('OpenAiCompletionRequestDTO', () => {
  ...
  describe('isOpenAiCompletionRequestDTO', () => {
    ...
    it('returns false for invalid OpenAiCompletionRequestDTO objects', () => {
      ...
      expect(isOpenAiCompletionRequestDTO({frequency_penalty: 'string'})).toBe(
        false,
      );
      expect(isOpenAiCompletionRequestDTO({presence_penalty: 'string'})).toBe(
        false,
      );
    });
  ...
  describe('explainOpenAiCompletionRequestDTO', () => {
    it('returns a human-readable string explaining why the value is not a regular object', () => {
      expect(explainOpenAiCompletionRequestDTO(undefined)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO(null)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO(false)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO(true)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO(0)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO(1)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO('')).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO('foo')).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO([])).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO([1, 2, 3])).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiCompletionRequestDTO(() => {})).toMatch(
        /not regular object/,
    ...
    it('returns a human-readable string explaining why the value has a non-number max_tokens property', () => {
      ...
      for (const nonNumberMaxTokensValue of nonNumberMaxTokensValues) {
        ...
        expect(explainOpenAiCompletionRequestDTO(value)).toMatch(
          /property "max_tokens" not number/,
        );
      }
    });
  ...
  describe('stringifyOpenAiCompletionRequestDTO', () => {
    it('returns a string representation of the OpenAiCompletionRequestDTO object', () => {
      ...
      expect(stringifyOpenAiCompletionRequestDTO(request)).toEqual(
        `OpenAiCompletionRequestDTO(${JSON.stringify(request)})`,
      );
    });
  });
  ...
  describe('parseOpenAiCompletionRequestDTO', () => {
    ...
    it('returns undefined for invalid OpenAiCompletionRequestDTO strings', () => {
      ...
      expect(
        parseOpenAiCompletionRequestDTO(
          'OpenAiCompletionRequestDTO(invalid json)',
        ),
      ).toBeUndefined();
    });
  });
