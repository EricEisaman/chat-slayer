...
export function createOpenAiCompletionRequestDTO(
  ...
  if (!isString(prompt))
    throw new TypeError(`Invalid OpenAiCompletionRequestDTO.prompt: ${prompt}`);
  if (!(isString(model) || isUndefined(model)))
    throw new TypeError(`Invalid OpenAiCompletionRequestDTO.model: ${model}`);
  if (!isNumberOrUndefined(max_tokens))
    throw new TypeError(
      `Invalid OpenAiCompletionRequestDTO.max_tokens: ${max_tokens}`,
    );
  if (!isNumberOrUndefined(temperature))
    throw new TypeError(
      `Invalid OpenAiCompletionRequestDTO.temperature: ${temperature}`,
    );
  if (!isNumberOrUndefined(top_p))
    throw new TypeError(`Invalid OpenAiCompletionRequestDTO.top_p: ${top_p}`);
  if (!isNumberOrUndefined(frequency_penalty))
    throw new TypeError(
      `Invalid OpenAiCompletionRequestDTO.frequency_penalty: ${frequency_penalty}`,
    );
  if (!isNumberOrUndefined(presence_penalty))
    throw new TypeError(
      `Invalid OpenAiCompletionRequestDTO.presence_penalty: ${presence_penalty}`,
    );
  return {
    prompt,
 ...
 */
export function isOpenAiCompletionRequestDTO(
  value: unknown,
): value is OpenAiCompletionRequestDTO {
  return (
    ...
    isString(objectKey(value, 'prompt')) &&
    isOpenAiModel(objectKey(value, 'model')) &&
    isNumber(objectKey(value, 'max_tokens')) &&
    isNumber(objectKey(value, 'temperature')) &&
    isNumber(objectKey(value, 'top_p')) &&
    isNumber(objectKey(value, 'frequency_penalty')) &&
    isNumber(objectKey(value, 'presence_penalty'))
  );
}
...
export function explainOpenAiCompletionRequestDTO(value: unknown): string {
  return explain([
    ...
    explainProperty(
      'frequency_penalty',
      explainNumber((value as any)?.frequency_penalty),
    ),
    explainProperty(
      'presence_penalty',
      explainNumber((value as any)?.presence_penalty),
    ),
  ]);
}
 ...
 */
export function stringifyOpenAiCompletionRequestDTO(
  value: OpenAiCompletionRequestDTO,
): string {
 ...
 */
export function parseOpenAiCompletionRequestDTO(
  value: unknown,
): OpenAiCompletionRequestDTO | undefined {
  if (isString(value)) {
    if (startsWith(value, 'OpenAiCompletionRequestDTO(')) {
      value = value.substring(
        'OpenAiCompletionRequestDTO('.length,
        value.length - 1,
      );
    }
