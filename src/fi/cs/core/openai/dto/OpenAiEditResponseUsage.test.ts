...
describe('OpenAiEditResponseUsage', () => {
  ...
  describe('explainOpenAiEditResponseUsage', () => {
    ...
    it('returns a human-readable string explaining why the value is not a valid OpenAiEditResponseUsage', () => {
      expect(explainOpenAiEditResponseUsage({extra: 'key'})).toMatch(
        /Value had extra properties: extra/,
      );
      expect(explainOpenAiEditResponseUsage('foo')).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseUsage(undefined)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseUsage(null)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseUsage(true)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseUsage(false)).toMatch(
        /not regular object/,
      );
      ...
      expect(explainOpenAiEditResponseUsage('test')).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseUsage([])).toMatch(/not regular object/);
      expect(explainOpenAiEditResponseUsage(['test'])).toMatch(
      ...
      expect(explainOpenAiEditResponseUsage(() => {})).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseUsage(Symbol())).toMatch(
        /not regular object/,
      );
    });
  });
  ...
  describe('stringifyOpenAiEditResponseUsage', () => {
    it("should return the string 'OpenAiEditResponseUsage({...})'", () => {
      ...
      const expected =
        'OpenAiEditResponseUsage({"prompt_tokens":1,"completion_tokens":2,"total_tokens":3})';
