import {
  ...
  isOpenAiEditRequestDTO,
  explainOpenAiEditRequestDTO,
  stringifyOpenAiEditRequestDTO,
  parseOpenAiEditRequestDTO,
} from './OpenAiEditRequestDTO';
...
describe('OpenAiEditRequestDTO', () => {
  ...
  describe('isOpenAiEditRequestDTO', () => {
    ...
    it('returns false for invalid OpenAiEditRequestDTO objects', () => {
      ...
      expect(isOpenAiEditRequestDTO({})).toBe(false);
      expect(isOpenAiEditRequestDTO({instruction: 123})).toBe(false);
      expect(isOpenAiEditRequestDTO({input: 123})).toBe(false);
      expect(isOpenAiEditRequestDTO({model: 123})).toBe(false);
      expect(isOpenAiEditRequestDTO({n: 'string'})).toBe(false);
      expect(isOpenAiEditRequestDTO({temperature: 'string'})).toBe(false);
      expect(isOpenAiEditRequestDTO({top_p: 'string'})).toBe(false);
    });
  ...
  describe('explainOpenAiEditRequestDTO', () => {
    it('returns a human-readable string explaining why the value is not a regular object', () => {
      expect(explainOpenAiEditRequestDTO(undefined)).toMatch(
        /not regular object/,
      );
      ...
      expect(explainOpenAiEditRequestDTO([])).toMatch(/not regular object/);
      expect(explainOpenAiEditRequestDTO([1, 2, 3])).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditRequestDTO(() => {})).toMatch(
        /not regular object/,
      );
    });
    ...
    it('returns a human-readable string explaining why the value has a non-number n property', () => {
      ...
      for (const nonNumberMaxTokensValue of nonNumberMaxTokensValues) {
        ...
        expect(explainOpenAiEditRequestDTO(value)).toMatch(
          /property "n" not number/,
        );
      }
    });
  ...
  describe('stringifyOpenAiEditRequestDTO', () => {
    it('returns a string representation of the OpenAiEditRequestDTO object', () => {
      ...
      expect(stringifyOpenAiEditRequestDTO(request)).toEqual(
        `OpenAiEditRequestDTO(${JSON.stringify(request)})`,
      );
    });
  });
  ...
  describe('parseOpenAiEditRequestDTO', () => {
    ...
    it('returns undefined for invalid OpenAiEditRequestDTO strings', () => {
      ...
      expect(
        parseOpenAiEditRequestDTO('OpenAiEditRequestDTO(invalid json)'),
      ).toBeUndefined();
    });
  });
});
