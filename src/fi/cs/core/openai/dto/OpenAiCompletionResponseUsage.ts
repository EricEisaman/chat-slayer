 ...
 */
export function isOpenAiCompletionResponseUsage(
  value: unknown,
): value is OpenAiCompletionResponseUsage {
  return (
    ...
    isNumber(objectKey(value, 'prompt_tokens')) &&
    isNumber(objectKey(value, 'completion_tokens')) &&
    isNumber(objectKey(value, 'total_tokens'))
  );
}
 ...
 */
export function explainOpenAiCompletionResponseUsage(value: unknown): string {
  return explain([
    ...
    explainProperty(
      'prompt_tokens',
      explainNumber(objectKey(value, 'prompt_tokens')),
    ),
    explainProperty(
      'completion_tokens',
      explainNumber(objectKey(value, 'completion_tokens')),
    ),
    explainProperty(
      'total_tokens',
      explainNumber(objectKey(value, 'total_tokens')),
    ),
  ]);
}
 ...
 */
export function stringifyOpenAiCompletionResponseUsage(
  value: OpenAiCompletionResponseUsage,
): string {
 ...
 */
export function parseOpenAiCompletionResponseUsage(
  value: unknown,
): OpenAiCompletionResponseUsage | undefined {
  if (isString(value)) {
    if (startsWith(value, 'OpenAiCompletionResponseUsage(')) {
      value = value.substring(
        'OpenAiCompletionResponseUsage('.length,
        value.length - 1,
      );
    }
