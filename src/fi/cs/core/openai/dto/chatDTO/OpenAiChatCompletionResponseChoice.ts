 ...
 */
export function isOpenAiChatCompletionResponseChoice(
  value: unknown,
): value is OpenAiChatCompletionResponseChoice {
  return (
    isRegularObject(value) &&
    hasNoOtherKeysInDevelopment(value, ['index', 'message', 'finish_reason']) &&
    isNumber(objectKey(value, 'index')) &&
    isOpenAiChatCompletionMessage(objectKey(value, 'message')) &&
 ...
 */
export function explainOpenAiChatCompletionResponseChoice(
  value: unknown,
): string {
  return explain([
    ...
    explainProperty(
      'message',
      explainOpenAiChatCompletionMessage(objectKey(value, 'message')),
    ),
    explainProperty(
      'finish_reason',
 ...
 */
export function isOpenAiChatCompletionResponseChoiceOrError(
  value: unknown,
): value is OpenAiChatCompletionResponseChoice | OpenAiError {
 ...
 */
export function explainOpenAiChatCompletionResponseChoiceOrError(
  value: unknown,
): string {
  return isOpenAiChatCompletionResponseChoiceOrError(value)
    ? explainOk()
    : 'Not OpenAiError or OpenAiChatCompletionResponseChoice';
}
 ...
 */
export function stringifyOpenAiChatCompletionResponseChoice(
  value: OpenAiChatCompletionResponseChoice,
): string {
 ...
 */
export function parseOpenAiChatCompletionResponseChoice(
  value: unknown,
): OpenAiChatCompletionResponseChoice | undefined {
  if (isString(value)) {
    if (startsWith(value, 'OpenAiChatCompletionResponseChoice(')) {
      value = value.substring(
        'OpenAiChatCompletionResponseChoice('.length,
        value.length - 1,
      );
    }
