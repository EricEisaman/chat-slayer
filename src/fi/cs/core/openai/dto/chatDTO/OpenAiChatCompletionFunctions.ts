...
export function isOpenAiChatCompletionFunctions(
  value: unknown,
): value is OpenAiChatCompletionFunctions {
  return (
    isRegularObject(value) &&
    hasNoOtherKeysInDevelopment(value, ['name', 'parameters', 'description']) &&
    isString(objectKey(value, 'name')) &&
    isObject(objectKey(value, 'parameters')) &&
...
export function explainOpenAiChatCompletionFunctions(value: unknown): string {
  return explain([
    ...
    explainNoOtherKeys(value, ['name', 'parameters', 'description']),
    explainProperty('name', explainString((value as any)?.name)),
    explainProperty('parameters', explainObject((value as any)?.parameters)),
    explainProperty(
      'description',
      explainStringOrUndefined((value as any)?.description),
    ),
  ]);
}

export function isOpenAiChatCompletionFunctionsOrUndefined(
  value: unknown,
): value is OpenAiChatCompletionFunctions | undefined {
  return isOpenAiChatCompletionFunctions(value) || isUndefined(value);
}

export function explainOpenAiChatCompletionFunctionsOrUndefined(
  value: unknown,
): string {
  return isOpenAiChatCompletionFunctionsOrUndefined(value)
    ? explainOk()
    : explainNot('OpenAiChatCompletionFunctions or undefined');
}
