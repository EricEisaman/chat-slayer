...
describe('OpenAiEditResponseChoice', () => {
  ...
  describe('explainOpenAiEditResponseChoice', () => {
    ...
    it('returns a human-readable string explaining why the value is not a valid OpenAiEditResponseChoice', () => {
      expect(explainOpenAiEditResponseChoice({extra: 'key'})).toMatch(
        /Value had extra properties: extra/,
      );
      expect(explainOpenAiEditResponseChoice('foo')).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseChoice(undefined)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseChoice(null)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseChoice(true)).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseChoice(false)).toMatch(
        /not regular object/,
      );
      ...
      expect(explainOpenAiEditResponseChoice('test')).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseChoice([])).toMatch(/not regular object/);
      expect(explainOpenAiEditResponseChoice(['test'])).toMatch(
      ...
      expect(explainOpenAiEditResponseChoice(() => {})).toMatch(
        /not regular object/,
      );
      expect(explainOpenAiEditResponseChoice(Symbol())).toMatch(
        /not regular object/,
      );
    });
  });
  ...
  describe('stringifyOpenAiEditResponseChoice', () => {
    it("should return the string 'OpenAiEditResponseChoice({...})'", () => {
      ...
      const expected =
        'OpenAiEditResponseChoice({"text":"This is a test","index":2,"logprobs":null,"finish_reason":"length"})';
  ...
  describe('parseOpenAiEditResponseChoice', () => {
    it('parses a valid OpenAiEditResponseChoice string representation', () => {
      const string =
        'OpenAiEditResponseChoice({"text":"text","index":1,"logprobs":null,"finish_reason":"length"})';
