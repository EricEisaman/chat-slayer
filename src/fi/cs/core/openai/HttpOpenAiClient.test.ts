...
import {
  HttpOpenAiClient,
  OPENAI_API_POST_COMPLETIONS_PATH,
  OPENAI_API_POST_EDITS_PATH,
} from './HttpOpenAiClient';
...
describe('HttpOpenAiClient', () => {
  ...
  let postJsonSpy: jest.SpiedFunction<(...args: unknown) => any>;

  beforeAll(() => {
    HttpOpenAiClient.setLogLevel(LogLevel.NONE);

    postJsonSpy = jest
      .spyOn(HttpService, 'postJson')
      .mockResolvedValue(
    ...
    it("should make a request to the OpenAI API's text edit endpoint and return the response", async () => {
      ...
      jest
        ...
        .mockImplementation(
          (
            _url: string,
            _body?: ReadonlyJsonAny | undefined,
            _headers?: ReadonlyJsonAny | undefined,
      ...
      const response = await client.getEdit(
        instruction,
        input,
        model,
        n,
