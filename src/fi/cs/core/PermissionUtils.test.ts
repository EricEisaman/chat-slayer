...
describe('PermissionUtils', () => {
  describe('#checkPermission', () => {
    ...
    test('can test valid permission with multiple items', () => {
      ...
      expect(
        PermissionUtils.checkPermission('FOO', ['BAR', 'FOO', 'HELLO']),
      ).toBe(true);
      expect(
        PermissionUtils.checkPermission('FOO', ['WORLD', 'BAR', 'FOO']),
      ).toBe(true);
      expect(
        PermissionUtils.checkPermission('FOO', ['FOO', 'HELLO', 'WORLD']),
      ).toBe(true);
    });
    ...
    test('can test invalid permission when permission list has multiple types', () => {
      expect(
        PermissionUtils.checkPermission('FOO', ['BAR', 'HELLO', 'WORLD']),
      ).toBe(false);
    });
  });
  ...
  describe('#checkPermissionList', () => {
    ...
    test('can test empty invalid permission list when no permissions are accepted', () => {
      expect(PermissionUtils.checkPermissionList([], [])).toStrictEqual({});
    });

    test('can test empty invalid permission list when one permission is accepted', () => {
      expect(PermissionUtils.checkPermissionList(['BAR'], [])).toStrictEqual({
        BAR: false,
      });
    });
    ...
    test('can test empty invalid permission list when some permissions are accepted', () => {
      expect(
        PermissionUtils.checkPermissionList(['BAR', 'FOO'], []),
      ).toStrictEqual({BAR: false, FOO: false});
    });

    test('can test valid permission list with single permission', () => {
      expect(
        PermissionUtils.checkPermissionList(['FOO', 'BAR'], ['FOO']),
      ).toStrictEqual({FOO: true, BAR: false});
    });

    test('can test valid permission list with two permissions', () => {
      expect(
        PermissionUtils.checkPermissionList(['FOO', 'BAR'], ['FOO', 'BAR']),
    ...
    test('can test valid permission list with two permissions against single accepted permission', () => {
      expect(
        PermissionUtils.checkPermissionList(['FOO'], ['FOO', 'BAR']),
    ...
    test('can test valid permission list with multiple permissions', () => {
      ...
      ).toStrictEqual({FOO: true, BAR: true, HELLO: true});
    });

    test('can test invalid permission list with single permission', () => {
      expect(
        PermissionUtils.checkPermissionList(['HELLO', 'WORLD'], ['FOO']),
      ).toStrictEqual({HELLO: false, WORLD: false});
    });

    test('can test invalid permission list with two permissions', () => {
      expect(
        PermissionUtils.checkPermissionList(['HELLO', 'WORLD'], ['FOO', 'BAR']),
      ).toStrictEqual({HELLO: false, WORLD: false});
    });
    ...
    test('can test invalid permission list with multiple permissions when only some match', () => {
      ...
      ).toStrictEqual({HELLO: true, WORLD: false, XXX: false});
    });
  });
  ...
  describe('#everyPermissionAccepted', () => {
    ...
    test('can test that every permission matches on on permission', () => {
      expect(
        PermissionUtils.everyPermissionAccepted({FOO: true}),
      ).toStrictEqual(true);
    });

    test('can test that every permission matches on two permissions', () => {
      expect(
        PermissionUtils.everyPermissionAccepted({FOO: true, BAR: true}),
      ).toStrictEqual(true);
    });
  ...
  describe('#getAcceptedPermissionList', () => {
    test('it returns accepted permissions', () => {
      ...
      expect(
        PermissionUtils.getAcceptedPermissionList({FOO: false}),
      ).toStrictEqual([]);
      expect(
        PermissionUtils.getAcceptedPermissionList({FOO: false, BAR: false}),
      ).toStrictEqual([]);
      expect(
        PermissionUtils.getAcceptedPermissionList({
          FOO: false,
          BAR: false,
          WORLD: false,
        }),
      ...
      expect(
        PermissionUtils.getAcceptedPermissionList({FOO: true}),
      ).toStrictEqual(['FOO']);
      expect(
        PermissionUtils.getAcceptedPermissionList({FOO: true, BAR: false}),
      ).toStrictEqual(['FOO']);
      expect(
        PermissionUtils.getAcceptedPermissionList({FOO: true, BAR: true}),
      ).toStrictEqual(['FOO', 'BAR']);
      expect(
        PermissionUtils.getAcceptedPermissionList({FOO: false, BAR: true}),
      ).toStrictEqual(['BAR']);
      expect(
        PermissionUtils.getAcceptedPermissionList({
          FOO: true,
  ...
  describe('#somePermissionAccepted', () => {
    ...
    test('can test that permission matches on single permission', () => {
      expect(PermissionUtils.somePermissionAccepted({FOO: true})).toStrictEqual(
        true,
      );
    });
    ...
    test('can test that every permission matches on two permissions', () => {
      expect(
        PermissionUtils.somePermissionAccepted({FOO: true, BAR: true}),
      ).toStrictEqual(true);
    });
