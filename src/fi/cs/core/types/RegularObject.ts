...
export function isRegularObject(
  ...
  return everyProperty<string, unknown>(value, isString, undefined);
}

export function explainRegularObject(value: unknown) {
  return isRegularObject(value) ? explainOk() : 'not regular object';
}

/** Read a property from an unknown value after a regular-object check. */
export function objectKey(value: unknown, key: string): unknown {
...
export function objectKeyAs<T>(value: unknown, key: string): T | undefined {
  const item = objectKey(value, key);
  return item === undefined ? undefined : (item as T);
}
...
export function assertRegularObjectOf<K extends keyof any = string, T = any>(
  ...
  const isKeyTest: TestCallbackNonStandardOf<K> =
    isKey === undefined ? (isString as TestCallbackNonStandardOf<K>) : isKey;
 ...
 * @nosideeffects
 */
export function isRegularObjectOrUndefinedOf<
 ...
 * @nosideeffects
 */
export function isRegularObjectOrUndefined(
  value: unknown,
): value is {[P in string]: any} | undefined {
  if (value === undefined) return true;
 ...
 * @nosideeffects
 */
export function explainRegularObjectOrUndefined(value: unknown): string {
  return isRegularObjectOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['regular object', 'undefined']));
}
