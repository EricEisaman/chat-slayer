 ...
 *
 * ```typescript
 * import { SiType } from "./fi/cs/core/types/SiType";
 * ```
 *
 ...
 * @nosideeffects
 */
export function isSiType(value: unknown): value is SiType {
  switch (value) {
    case SiType.NONE:
    case SiType.KILO:
    case SiType.MEGA:
 ...
 * @nosideeffects
 */
export function parseSiType(value: unknown): SiType | undefined {
  if (isArray(value)) return undefined;
  const raw = isString(value) ? value : `${value}`;
  const normalized = raw.length === 1 ? raw : toUpper(raw);
  switch (normalized) {
    case '':
    case 'NONE':
      return SiType.NONE;
