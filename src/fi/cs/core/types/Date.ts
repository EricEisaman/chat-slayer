...
export function isValidDate(time: unknown): time is Date {
  try {
...
export function isIsoDateStringWithMilliseconds(
  value: unknown,
): value is IsoDateStringWithMilliseconds {
...
export function createIsoDateStringWithMilliseconds(
  ...
  if (!parsedValue)
    throw new TypeError(`Value is not ISO data string: '${value}'`);
  return parsedValue;
}

export function explainIsoDateStringWithMilliseconds(value: unknown): string {
  return isIsoDateStringWithMilliseconds(value)
    ? explainOk()
    : explainNot(`Expected '${value}' to be a valid IsoDateString`);
}

export function isIsoDateStringWithMillisecondsOrUndefined(
  value: unknown,
): value is IsoDateStringWithMilliseconds | undefined {
  return isUndefined(value) || isIsoDateStringWithMilliseconds(value);
}

export function explainIsoDateStringWithMillisecondsOrUndefined(
  value: unknown,
): string {
  return isIsoDateStringWithMillisecondsOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['IsoDateStringWithMilliseconds', 'undefined']));
}
...
export function isIsoDateStringOrUndefined(
  value: unknown,
): value is IsoDateString | undefined {
  return isUndefined(value) || isIsoDateString(value);
}

export function explainIsoDateStringOrUndefined(value: unknown): string {
  return isIsoDateStringOrUndefined(value)
    ? explainOk()
    : explainNot(explainOr(['IsoDateString', 'undefined']));
}
