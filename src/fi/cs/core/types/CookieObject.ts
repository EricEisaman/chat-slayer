...
import {
  explainSameSiteOrUndefined,
  isSameSiteOrUndefined,
  parseSameSite,
  SameSite,
} from './SameSite';
import {
...
export function isCookieObject(value: unknown): value is CookieObject {
  return (
    ...
    isStringOrUndefined(objectKey(value, 'value')) &&
    isStringOrUndefined(objectKey(value, 'path')) &&
    isStringOrUndefined(objectKey(value, 'domain')) &&
    isBooleanOrUndefined(objectKey(value, 'httpOnly')) &&
    isBooleanOrUndefined(objectKey(value, 'secure')) &&
    isStringOrUndefined(objectKey(value, 'expires')) &&
    isNumberOrUndefined(objectKey(value, 'maxAge')) &&
    isSameSiteOrUndefined(objectKey(value, 'sameSite'))
  );
}

export function explainCookieObject(value: unknown): string {
  return explain([
    ...
    explainProperty('path', explainStringOrUndefined(objectKey(value, 'path'))),
    explainProperty('name', explainString(objectKey(value, 'name'))),
    explainProperty(
      'value',
      explainStringOrUndefined(objectKey(value, 'value')),
    ),
    explainProperty(
      'domain',
      explainStringOrUndefined(objectKey(value, 'domain')),
    ),
    explainProperty(
      'httpOnly',
      explainBooleanOrUndefined(objectKey(value, 'httpOnly')),
    ),
    explainProperty(
      'secure',
      explainBooleanOrUndefined(objectKey(value, 'secure')),
    ),
    explainProperty(
      'expires',
      explainStringOrUndefined(objectKey(value, 'expires')),
    ),
    explainProperty(
      'maxAge',
      explainNumberOrUndefined(objectKey(value, 'maxAge')),
    ),
    explainProperty(
      'sameSite',
      explainSameSiteOrUndefined(objectKey(value, 'sameSite')),
    ),
  ]);
}
 ...
 * @param line
 */
export function parseCookiePair(line: string): [string, string | undefined] {
