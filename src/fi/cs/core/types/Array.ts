...
export function isArrayOrUndefined(
  value: unknown,
): value is any[] | readonly any[] | undefined {
...
export function explainArrayOf<T = any>(
  ...
  return `${explainNot(itemTypeName)}: ${explain(
    map(value, (item: unknown): string => itemExplain(item)),
  )}`;
}
...
export function explainPairArrayOf<T1 = any, T2 = T1>(
  ...
  return isPairArrayOf<T1, T2>(value, isItemA, isItemB)
    ? explainOk()
    : explainNot(`[${itemTypeNameA}, ${itemTypeNameB}]`);
}
...
export function isTripletArrayOf<T1, T2, T3>(
  ...
  const [a, b, c] = value;
  return isItemA(a, 0, value) && isItemB(b, 1, value) && isItemC(c, 2, value);
}
...
export function explainTripletArrayOf<T1, T2, T3>(
  ...
  return isTripletArrayOf<T1, T2, T3>(value, isItemA, isItemB, isItemC)
    ? explainOk()
    : explainNot(`[${itemTypeNameA}, ${itemTypeNameB}, ${itemTypeNameC}]`);
}
...
export function explainArrayOfOrUndefined<T = any>(
  ...
  if (isArrayOfOrUndefined<T>(value, isItem, minLength, maxLength))
    return explainOk();
  if (!isArray(value)) return explainNot(itemTypeName);
  if (value.length < 1) return explainNot(itemTypeName);
  return `${explainNot(itemTypeName)}: ${explain(
    map(value, (item: unknown): string => {
