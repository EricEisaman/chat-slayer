...
export function isPermissionListDTO<T extends string>(
  ...
  return (
    ...
    hasNoOtherKeys(value, ['permissions']) &&
    isPermissionList<T>(objectKey(value, 'permissions'), isT)
  );
}
...
export function explainPermissionListDTO<T extends string>(
  ...
  return explain([
    ...
    explainNoOtherKeys(value, ['permissions']),
    explainProperty(
      'permissions',
      explainPermissionList<T>(
        permissionName,
        permissionExplain,
        objectKey(value, 'permissions'),
        isPermission,
      ),
    ),
  ]);
