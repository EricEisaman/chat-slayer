...
} from './types/RequestPathVariableOptions';
...
export function PathVariable(
  ...
  function _setPathVariableMap(
    ...
    const requestController: RequestController | undefined =
      RequestControllerUtils.findController(target);
    ...
    LOG.warn(
      '_setPathVariableMap: Unrecognized configuration: ',
      '; target=',
      target,
      '; propertyKey=',
      propertyKey,
  ...
  const variableName: string | RequestPathVariableListOptions | undefined =
    arg1;

  if (isString(variableName)) {
    if (!isRequestPathVariableOptionsOrUndefined(arg2)) {
      throw new TypeError(
        `PathVariable: Argument 2 is not type of RequestPathVariableOptions: ${arg2}`,
      );
    }
    ...
    return (
      ...
      LOG.warn(
        'pathVariable: Unrecognized configuration: ',
        '; target=',
        target,
        '; propertyKey=',
        propertyKey,
  ...
  const opts: RequestPathVariableListOptions | undefined = variableName;
  if (opts === undefined || isObject(opts?.defaultValues)) {
  } else {
    throw new TypeError('PathVariable: Invalid type of options');
  }
  const defaultValues: DefaultPathVariableMapValuesType | undefined = opts
    ? (opts?.defaultValues ?? undefined)
    : undefined;
  return (
    target: any | Function,
    propertyKey?: string,
