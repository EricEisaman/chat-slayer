...
import {
  ...
  isRequestMappingObject,
} from './RequestMappingObject';
import {asRecord} from '../../types/castBoundaries';
...
export function isRequestControllerMappingObject(
  value: unknown,
): value is RequestControllerMappingObject {
  return (
    RequestInterfaceUtils.isObject(value) &&
    RequestInterfaceUtils.hasPropertyMappings(value) &&
    isArray(objectKey(value, 'mappings')) &&
    every(objectKey(value, 'mappings'), isRequestMappingObject) &&
    RequestInterfaceUtils.hasPropertyControllerProperties(value) &&
    isObject(objectKey(value, 'controllerProperties')) &&
    RequestInterfaceUtils.everyPropertyIs<RequestControllerMethodObject>(
      asRecord(objectKey(value, 'controllerProperties')),
      isRequestControllerMethodObject,
    )
  );
}

export function explainRequestControllerMappingObject(value: unknown): string {
  ...
  if (!isArray(objectKey(value, 'mappings'))) {
    return 'Property "mappings" was not valid: Was not array';
  }

  if (!every(objectKey(value, 'mappings'), isRequestMappingObject)) {
  ...
  if (!isObject(objectKey(value, 'controllerProperties'))) {
    return 'Property "controllerProperties" was not valid: Property was not object';
  }

  const controllerProperties = asRecord(
    objectKey(value, 'controllerProperties'),
  );
  if (
    !RequestInterfaceUtils.everyPropertyIs<RequestControllerMethodObject>(
    ...
    return `Property "controllerProperties" was not valid: Some properties were not valid: ${RequestInterfaceUtils.explainEveryPropertyIs(controllerProperties, isRequestControllerMethodObject, explainRequestControllerMethodObject)}`;
  }

  return 'ok';
}

const LOG = LogService.createLogger(
...
export function getOpenApiDocumentFromRequestControllerMappingObject(
  ...
  const paths: OpenAPIV3.PathsObject = reduce(
    ...
    (
      ...
      item: RequestControllerMethodObject,
      ...
      forEach(item.mappings, (mapping: RequestMappingObject) => {
        forEach(mapping.methods, (method: RequestMethod) => {
          const openApiMethod = getOpenApiMethodFromRequestMethod(method);
          forEach(mapping.paths, (path: string) => {
  ...
  return reduce(
    ...
    (
      info: OpenAPIV3.Document,
      item: Partial<OpenAPIV3.Document>,
    ): OpenAPIV3.Document => {
      return {
        ...info,
        ...item,
...
getOpenApiDocumentFromRequestControllerMappingObject.setLogLevel = (
  level: LogLevel,
): void => {
...
function setOpenApiDocumentPathOperationsObject(
  ...
  if (!origObjPath)
    throw new TypeError('Path was undefined; this should not happen');
  obj = {
    ...obj,
    [path]: {
      ...origObjPath,
