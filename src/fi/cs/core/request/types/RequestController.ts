...
export function isRequestController(
  value: unknown,
): value is RequestController {
  if (!value) return false;
  const hasInternalValue =
    RequestInterfaceUtils.hasProperty__requestMappings(value);
  ...
  LOG.warn(
    'The internal mapping object was not correct: ' +
      JSON.stringify(internalValue, null, 2) +
      ': ' +
      explainRequestControllerMappingObject(internalValue),
  );
  return false;
}
...
export function getOpenApiDocumentFromRequestController(
  ...
  const data: RequestControllerMappingObject | undefined =
    getRequestControllerMappingObject(controller);
  if (data === undefined) {
    throw new TypeError(
      'getOpenApiDocumentFromRequestController: Could not detect request mapping in the controller',
    );
  }
