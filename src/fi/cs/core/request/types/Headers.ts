...
export class Headers {
  ...
  public add(headerName: string, headerValue: string) {
    ...
    const originalHeader: string | readonly string[] | undefined =
      this._value && has(this._value, headerName)
  ...
  public addAll(arg1: HeadersObject | string, arg2?: readonly string[]) {
    ...
    if (isString(arg1)) {
      ...
      if (!isArray(headerValues))
        throw new TypeError(
          'Headers.addAll signature must be (string, string[]) or (HeadersObject)',
        );
      ...
      forEach(keys(values), (headerKey: string) => {
        const headerValue: string | readonly string[] | undefined =
          values[headerKey];
...
export function isHeaders(value: unknown): value is Headers {
  return !!value && value instanceof Headers;
}
