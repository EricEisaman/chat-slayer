...
function filterRetryForStatusRange(value: RetryForAny): readonly NumberPair[] {
  return filter(
    map(normalizeAsArray(value), normalizeStatusRange),
    isNumberPair,
  );
}
...
function normalizeAsArray(value: unknown): readonly any[] {
...
function filterRetryForCode(value: RetryForAny): readonly ErrorCode[] {
  return filter(
    map(normalizeAsArray(value), normalizeAsErrorCode),
    isErrorCode,
  );
}
...
export function isHttpRetryPolicy(value: unknown): value is HttpRetryPolicy {
  return (
    ...
    isArrayOf<string>(objectKey(value, 'onCode'), isString) &&
    isArrayOf<string>(objectKey(value, 'onMethod'), isString) &&
    isArrayOf<NumberPair>(objectKey(value, 'onStatus'), isNumberPair) &&
    isNumber(objectKey(value, 'maxAttempts')) &&
    isNumber(objectKey(value, 'baseDelay')) &&
    isNumber(objectKey(value, 'increasingDelayStep')) &&
    isNumberOrUndefined(objectKey(value, 'maxDelay'))
  );
}

export function explainHttpRetryPolicy(value: unknown): string {
  return explain([
    ...
    explainProperty(
      'onCode',
      explainArrayOf<string>(
        'string',
        explainString,
        objectKey(value, 'onCode'),
        isString,
      ),
    ),
    explainProperty(
      ...
      explainArrayOf<string>(
        'string',
        explainString,
        objectKey(value, 'onMethod'),
        isString,
      ),
    ),
    explainProperty(
      'onStatus',
      explainArrayOf<NumberPair>(
        'NumberPair',
        explainNumberPair,
        objectKey(value, 'onStatus'),
        isNumberPair,
      ),
    ),
    explainProperty(
      'maxAttempts',
      explainNumber(objectKey(value, 'maxAttempts')),
    ),
    explainProperty('baseDelay', explainNumber(objectKey(value, 'baseDelay'))),
    explainProperty(
      'increasingDelayStep',
      explainNumber(objectKey(value, 'increasingDelayStep')),
    ),
    explainProperty(
      'maxDelay',
      explainNumberOrUndefined(objectKey(value, 'maxDelay')),
    ),
  ]);
}
...
export function parseHttpRetryPolicy(
  value: unknown,
): HttpRetryPolicy | undefined {
...
export function shouldRetry(
  ...
  return (
    ...
    (some(value.onStatus, (item: NumberPair): boolean => {
      ...
      (code !== undefined &&
        some(value.onCode, (item: string) => item === code)))
  );
}
...
export function createDefaultHttpRetryPolicy(): HttpRetryPolicy {
  return createHttpRetryPolicy(getDefaultRetryFor(), 10, 1000, 2500);
}
