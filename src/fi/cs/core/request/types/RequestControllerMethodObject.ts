...
export function isRequestControllerMethodObject(
  value: unknown,
): value is RequestControllerMethodObject {
  return (
    RequestInterfaceUtils.isObject(value) &&
    RequestInterfaceUtils.hasPropertyMappings(value) &&
    isArrayOf<RequestMappingObject>(
      objectKey(value, 'mappings'),
      isRequestMappingObject,
    ) &&
    RequestInterfaceUtils.hasPropertyParams(value) &&
    isArrayOf(
      objectKey(value, 'params'),
      RequestInterfaceUtils.createOrFunction(isRequestParamObject, isNull),
    ) &&
    RequestInterfaceUtils.hasPropertyModelAttributes(value) &&
    isStringArray(objectKey(value, 'modelAttributes')) &&
    RequestInterfaceUtils.hasPropertySynchronized(value) &&
    isBoolean(objectKey(value, 'synchronized'))
  );
}

export function explainRequestControllerMethodObject(value: unknown): string {
  ...
  if (!isArray(objectKey(value, 'params'))) {
    return 'Property "params" was not an array';
  }
  const test = RequestInterfaceUtils.createOrFunction(
    isRequestParamObject,
    isNull,
  );
  if (!every(objectKey(value, 'params'), test)) {
    return `Property "params" had some elements which were not RequestParamObject or null: ${filter(
      map(
        objectKey(value, 'params') as unknown[],
  ...
  if (!isStringArray(objectKey(value, 'modelAttributes'))) {
  ...
  if (!isBoolean(objectKey(value, 'synchronized'))) {
