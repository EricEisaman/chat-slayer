...
export function isEntityStatusType(value: unknown): value is EntityStatusTypes {
...
export class ResponseEntity<T> {
  ...
  constructor(
    ...
    if (
      isEntityStatusType(arg1) &&
      !isEntityStatusType(arg2) &&
      !isEntityStatusType(arg3)
    ) {
      status = arg1;
      if (arg2 !== undefined)
        throw new TypeError(
          'ResponseEntity signature is [body, ][headers, ]status',
        );
      if (arg3 !== undefined)
        throw new TypeError(
          'ResponseEntity signature is [body, ][headers, ]status',
  ...
  public allow(...methods: (RequestMethod | string)[]): ResponseEntity<T> {
    ...
    const stringMethods: string[] = map(parsedMethods, (item: RequestMethod) =>
      stringifyRequestMethod(item).toUpperCase(),
    );
  ...
  public toString(): string {
    ...
    const headers: string = headersObject.isEmpty()
      ? ''
      : StringUtils.toString(headersObject);
  ...
  public static accepted<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.Accepted);
  }
  public static badRequest<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.BadRequest);
  }
  public static created<T>(location: string): ResponseEntity<T> {
  ...
  public static noContent(): ResponseEntity<void> {
    return new ResponseEntity<void>(RequestStatus.NoContent);
  }
  public static notFound<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.NotFound);
  }
  public static notImplemented<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.NotImplemented);
  }
  public static internalServerError<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.InternalServerError);
  }
  public static methodNotAllowed<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.MethodNotAllowed);
  }
  public static unprocessableEntity<T>(): ResponseEntity<T> {
    return new ResponseEntity<T>(RequestStatus.UnprocessableEntity);
  }

  public static ok<T>(body?: T): ResponseEntity<T> {
    return body !== undefined
      ? new ResponseEntity<T>(body, RequestStatus.OK)
      : new ResponseEntity<T>(RequestStatus.OK);
  }
}

export function isResponseEntity(value: unknown): value is ResponseEntity<any> {
