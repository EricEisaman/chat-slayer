...
export function stringifyRequestMethod(value: RequestMethod): string {
  ...
  throw new TypeError(
    `Unsupported value for stringifyRequestMethod(): ${value}`,
  );
}
...
export function isRequestMethod(value: unknown): value is RequestMethod {
  return (
    isNumber(value) &&
    value >= FIRST_REQUEST_METHOD_NUMBER &&
    value <= LAST_REQUEST_METHOD_NUMBER
  );
}

export function explainRequestMethod(value: unknown): string {
  return isRequestMethod(value) ? explainOk() : explainNot('RequestMethod');
}

export function parseRequestMethod(value: unknown): RequestMethod {
