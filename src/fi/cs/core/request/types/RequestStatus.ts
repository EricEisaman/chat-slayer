...
export function parseRequestStatus(value: unknown): RequestStatus {
  ...
  if (isString(value)) {
    ...
    const integerValue = parseInt(trimmed, 10);
    if (isRequestStatus(integerValue)) return integerValue;

    const normalized = normaliseStatusString(trimmed);

    const statusKey: string | undefined = find(
      keys(RequestStatus),
      (key: string) => {
        // @ts-ignore
        const item: RequestStatus = RequestStatus[key];
        return (
          normaliseStatusString(stringifyRequestStatus(item)) === normalized
        );
      },
    );
