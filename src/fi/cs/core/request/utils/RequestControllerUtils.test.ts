...
describe('RequestControllerUtils', () => {
  describe('#parseRequestMappings', () => {
    test('can parse single path', () => {
      const config: (RequestMethod | string)[] = ['/path/to'];
      const parsedObject: RequestMappingObject =
        RequestControllerUtils.parseRequestMappings(config);
    ...
    test('can parse multiple paths', () => {
      const config: (RequestMethod | string)[] = ['/path', '/path/to'];
      const parsedObject: RequestMappingObject =
        RequestControllerUtils.parseRequestMappings(config);
      expect(parsedObject).toBeDefined();
      expect(parsedObject).toStrictEqual({
    ...
    test('can parse GET method', () => {
      const config: (RequestMethod | string)[] = [RequestMethod.GET];
      const parsedObject: RequestMappingObject =
        RequestControllerUtils.parseRequestMappings(config);
      expect(parsedObject).toBeDefined();
      expect(parsedObject).toStrictEqual({
    ...
    test('can parse POST method', () => {
      const config: (RequestMethod | string)[] = [RequestMethod.POST];
      const parsedObject: RequestMappingObject =
        RequestControllerUtils.parseRequestMappings(config);
      expect(parsedObject).toBeDefined();
      expect(parsedObject).toStrictEqual({
  ...
  describe('#attachControllerMapping', () => {
    test('can attach request mapping to the controller', () => {
      ...
      RequestControllerUtils.attachControllerMapping(
        TestController as RequestController,
        config,
      );
      const internalMapping = getInternalRequestMappingObject(
        TestController as RequestController,
        TestController,
      );
  ...
  describe('#attachControllerMethodMapping', () => {
    test("can attach mapping for the controller's method", () => {
      ...
      RequestControllerUtils.attachControllerMethodMapping(
        TestController as RequestController,
        config,
        'getList',
      );
      ...
      expect(internalMapping?.controller).toStrictEqual(TestController);
      expect(
        internalMapping?.controllerProperties?.getList?.mappings?.length,
      ).toBe(1);
      expect(
        internalMapping?.controllerProperties?.getList?.mappings[0],
      ).toStrictEqual({
        methods: [RequestMethod.GET],
        paths: ['/path/to'],
      });
      expect(
        internalMapping?.controllerProperties?.getList?.modelAttributes,
      ).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getList?.params,
      ).toStrictEqual([]);
    ...
    test("can attach mappings for the same controller's method", () => {
      ...
      expect(
        internalMapping?.controllerProperties?.getList?.mappings[1],
      ).toStrictEqual({
        methods: [RequestMethod.GET],
        paths: ['/path/to'],
      });
    ...
    test('can attach multiple mappings to the same controller', () => {
      ...
      expect(
        internalMapping?.controllerProperties?.getItem?.mappings?.length,
      ).toBe(1);
      expect(
        internalMapping?.controllerProperties?.getItem?.mappings[0],
      ).toStrictEqual({
        methods: [RequestMethod.GET],
        paths: ['/different'],
      });
      expect(
        internalMapping?.controllerProperties?.getItem?.modelAttributes,
      ).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getItem?.params,
      ).toStrictEqual([]);
    });
  });
  ...
  describe('#findController', () => {
    ...
    test('cannot find instance from undefined', () => {
      expect(RequestControllerUtils.findController(undefined)).toStrictEqual(
        undefined,
      );
    });
    ...
    test('cannot find instance from string', () => {
      expect(
        RequestControllerUtils.findController('hello world'),
      ).toStrictEqual(undefined);
    });

    test('cannot find instance from null', () => {
      expect(RequestControllerUtils.findController(null)).toStrictEqual(
        undefined,
      );
    });

    test('cannot find instance from number', () => {
      expect(RequestControllerUtils.findController(123)).toStrictEqual(
        undefined,
      );
    });
  });
  ...
  describe('#setControllerMethodModelAttributeParam', () => {
    test("can attach string attribute mapping for the controller's method", () => {
      ...
      expect(internalMapping?.controller).toStrictEqual(TestController);
      expect(
        internalMapping?.controllerProperties?.getEcho?.mappings?.length,
      ).toBe(0);
      expect(
        internalMapping?.controllerProperties?.getEcho?.modelAttributes,
      ).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getEcho?.params,
      ).toStrictEqual([
        {
          attributeName: 'test',
  ...
  describe('#setControllerMethodQueryParam', () => {
    test("can attach attribute mapping for the controller's method", () => {
      ...
      expect(
        internalMapping?.controllerProperties?.getEcho?.modelAttributes?.length,
      ).toBe(0);
      //expect(internalMapping?.controllerProperties?.getEcho?.modelAttributes[0]).toStrictEqual({});
      expect(
        internalMapping?.controllerProperties?.getEcho?.params?.length,
      ).toBe(1);
      expect(
        internalMapping?.controllerProperties?.getEcho?.params[0],
      ).toStrictEqual({
  ...
  describe('#setControllerMethodBodyParam', () => {
    test("can attach body argument configuration for the controller's method", () => {
      ...
      expect(internalMapping?.controller).toStrictEqual(TestController);
      expect(
        internalMapping?.controllerProperties?.getEcho?.requestBodyRequired,
      ).toBe(true);
      expect(
        internalMapping?.controllerProperties?.getEcho?.mappings?.length,
      ).toBe(0);
  ...
  describe('#attachControllerOperation', () => {
    test("can attach openAPI operation info for the controller's method", () => {
      ...
      expect(internalMapping?.mappings).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getEcho?.mappings,
      ).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getEcho?.modelAttributes,
      ).toStrictEqual([]);
      ...
      expect(
        internalMapping?.controllerProperties?.getEcho?.operations?.length,
      ).toBe(1);
      expect(
        (internalMapping?.controllerProperties?.getEcho?.operations ?? [])[0],
      ).toStrictEqual({
        operationId: 'getEcho',
        summary: 'Returns echo of the parameter provided',
      });
    ...
    test("can attach openAPI operation info for the controller's method with request mapping with method mappings1", () => {
      ...
      const config1: (RequestMethod | string)[] = [RequestMethod.GET, '/echo'];
      RequestControllerUtils.attachControllerMethodMapping(
        TestController as RequestController,
        config1,
        'getEcho',
      ...
      const config2: (RequestMethod | string)[] = [RequestMethod.GET, '/foo'];
      RequestControllerUtils.attachControllerMethodMapping(
        TestController as RequestController,
        config2,
        'getFoo',
      ...
      expect(
        internalMapping?.controllerProperties?.getFoo?.mappings?.length,
      ).toBe(1);
      expect(
        internalMapping?.controllerProperties?.getFoo?.mappings[0],
      ).toStrictEqual({
        methods: [RequestMethod.GET],
        paths: ['/foo'],
      });
      expect(
        internalMapping?.controllerProperties?.getFoo?.modelAttributes,
      ).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getFoo?.params,
      ).toStrictEqual([]);
      expect(
        internalMapping?.controllerProperties?.getFoo?.operations,
      ).not.toBeDefined();
      ...
      expect(
        internalMapping?.controllerProperties?.getEcho?.mappings[0],
      ).toStrictEqual({
        methods: [RequestMethod.GET],
        paths: ['/echo'],
      });
