...
export class RequestControllerUtils {
   ...
   */
  public static parseRequestMappings(
    value: readonly RequestMappingValue[],
  ): RequestMappingObject {
    return {
  ...
  public static attachControllerMapping(
    ...
    const origMapping: RequestControllerMappingObject | undefined =
      getInternalRequestMappingObject(controller, controller);
   ...
   * @param target
   */
  public static findController(target: unknown): RequestController | undefined {
    ...
    if (
      ...
      isRequestController(target.constructor)
    ) {
  ...
  public static setControllerMethodModelAttributeParam(
    ...
    RequestControllerUtils._setControllerMethodParam(
      controller,
      propertyKey,
      paramIndex,
      newParam,
  ...
  public static attachControllerOpenApiDocument(
    ...
    const mappingObject: RequestControllerMappingObject | undefined =
      getInternalRequestMappingObject(controller, controller);
  ...
  public static attachControllerOperation(
    ...
    let operations: readonly Partial<OpenAPIV3.OperationObject>[] =
      mappingObject?.controllerProperties[propertyKey]?.operations ?? [];

    if (operationId) {
      let operation: any = find(
        operations,
      ...
      if (operation) {
        operations = filter(
          operations,
          (op: Partial<OpenAPIV3.OperationObject>): boolean =>
            op.operationId !== operationId,
        );
        ...
        operation = {
          ...(operation?.tags || config?.tags
            ? {
                tags: uniq(
                  concat([], config?.tags ?? [], operation?.tags ?? []),
                ),
              }
            : {}),
          ...(config?.summary ? {summary: config?.summary} : {}),
          ...(config?.description ? {description: config?.description} : {}),
          ...(operation?.externalDocs || config?.externalDocs
            ? {
                externalDocs: merge(
                  {},
                  config?.externalDocs ?? {},
          ...
          ...(operation?.parameters || config?.parameters
            ? {
                parameters: mergeConcatByProperty(
                  'name',
                  config?.parameters ?? [],
          ...
          ...(operation?.responses || config?.responses
            ...
            : {}),
          ...(operation?.callbacks || config?.callbacks
            ? {
                callbacks: merge(
                  {},
                  config?.callbacks ?? {},
            ...
            : {}),
          ...(operation?.deprecated || config?.deprecated
            ? {deprecated: config?.deprecated ?? operation?.deprecated}
            : {}),
          ...(operation?.security || config?.security
            ...
            : {}),
          ...(operation?.servers || config?.servers
            ? {
                servers: mergeConcatByProperty(
                  'url',
                  config?.servers ?? [],
                  operation?.servers ?? [],
                ),
  ...
  public static attachControllerSynchronizedRequest(
    ...
    if (propertyKey === undefined) {
      throw new TypeError(
        'Synchronizing all controller methods is not supported yet',
      );
    }
  ...
  private static _setControllerMethodParam(
    ...
    const origMapping: RequestControllerMappingObject | undefined =
      getInternalRequestMappingObject(controller, controller);
    if (origMapping === undefined) {
      const params: readonly (RequestParamObject | null)[] =
        RequestControllerUtils._initializeParams(paramIndex, newParam);
      ...
      const params: (RequestParamObject | null)[] =
        RequestControllerUtils._reinitializeParams(
          origMapping,
          propertyKey,
          paramIndex,
          newParam,
  ...
  private static _reinitializeParams(
    ...
    const params: (RequestParamObject | null)[] = [
      ...origMapping.controllerProperties[propertyKey].params,
    ];
    while (paramIndex >= params.length) {
      params.push(null);
    }
...
function mergeConcatByProperty<T = any>(
  propertyName: string,
  newValues: T[],
  prevValues: T[],
): T[] {
  const allPropertyValues: string[] = uniq([
    ...map(newValues, (item: T) =>
      has(item, propertyName) ? (item as any)[propertyName] : undefined,
    ),
    ...map(prevValues, (item: T) =>
      has(item, propertyName) ? (item as any)[propertyName] : undefined,
    ),
  ]);
  return reduce(
    ...
    (list: T[], propertyValue: string): T[] => {
      const newValue: T | undefined = find(newValues, (item: T): boolean =>
        has(item, propertyName)
          ? (item as any)[propertyName] === propertyValue
          : false,
      );
      const prevValue: T | undefined = find(prevValues, (item: T): boolean =>
        has(item, propertyName)
          ? (item as any)[propertyName] === propertyValue
          : false,
      );
