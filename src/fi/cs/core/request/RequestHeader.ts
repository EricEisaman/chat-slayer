import {
  isRequestHeaderListOptions,
  RequestHeaderListOptions,
} from './types/RequestHeaderListOptions';
import {
...
export function RequestHeader(
  ...
  function _setMethodHeaderMap(
    ...
    if (requestController !== undefined) {
      ...
      LOG.warn(
        '_setMethodHeaderMap: Unrecognized configuration: ',
        '; target=',
        target,
        '; propertyKey=',
        propertyKey,
  ...
  if (isString(arg1)) {
    ...
    if (!isRequestHeaderOptionsOrUndefined(arg2)) {
      throw new TypeError(
        `RequestHeader: Argument 2 is not type of RequestHeaderOptions: ${arg2}`,
      );
    }
    ...
    return (
      ...
      LOG.warn(
        'header: Unrecognized configuration: ',
        '; target=',
        target,
        '; propertyKey=',
        propertyKey,
  ...
  const opts: RequestHeaderListOptions | undefined = arg1;
  if (!(opts === undefined || isRequestHeaderListOptions(opts))) {
    throw new TypeError('RequestHeader: Invalid type of options');
  }
  const defaultValues: DefaultHeaderMapValuesType | undefined =
    opts?.defaultValues;
  return (
    target: any | Function,
    propertyKey?: string,
