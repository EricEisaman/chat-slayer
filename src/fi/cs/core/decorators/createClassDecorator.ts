...
export function createClassDecorator(
  ...
  return (target: any): void | Function => {
    if (!isFunction(target)) {
      throw new TypeError(
        `The constructor is not a function: ${typeof target}`,
      );
    }
    const overrideConstructor: void | Function = esDecorator(target, {
      kind: 'class',
      name: undefined,
      addInitializer: (
        // @ts-ignore
    ...
    if (isFunction(overrideConstructor)) {
      ...
      LOG.warn(
        `The return value was not void or Function: ${typeof overrideConstructor}`,
      );
    }
  };
}
