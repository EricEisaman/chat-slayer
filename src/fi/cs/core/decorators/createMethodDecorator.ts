...
export function createMethodDecorator<T>(
  ...
  return function methodDecorator(
    ...
    if (!descriptor) {
      throw new TypeError(
        `createMethodDecorator: descriptor missing from the property "${propertyName}"`,
      );
    }
    ...
    if (!isFunction(method)) {
      throw new TypeError(
        `createMethodDecorator: The property "${propertyName}" is not a function: ${typeof method}`,
      );
    }

    const overrideCallback = esDecorator(method, {
      kind: 'method',
      name: propertyName,
      static: false,
      private: false,
    ...
    if (isFunction(overrideCallback)) {
      ...
      LOG.warn(
        `The return value was not void or Function: ${typeof overrideCallback}`,
      );
    }
  };
}
