...
describe('TranslationUtils', () => {
  ...
  describe('getLanguageStringForI18n', () => {
    it('should return i18n compatible language string', () => {
      expect(
        TranslationUtils.getLanguageStringForI18n(Language.ENGLISH),
      ).toEqual('en');
      expect(
        TranslationUtils.getLanguageStringForI18n(Language.FINNISH),
      ).toEqual('fi');
    });
    ...
    it('should return undefined for unsupported language', () => {
      // @ts-ignore
      expect(
        TranslationUtils.getLanguageStringForI18n('unsupported_language'),
      ).toEqual(undefined);
    });
  });
  ...
  describe('translateKeys', () => {
    it('should translate keys', () => {
      const t = jest
        .fn<any>()
        .mockImplementation(
          (key: any, params: unknown): string => `${key}:${params.key}`,
        );
